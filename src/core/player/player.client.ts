...
import { HuaShenInfo, Player } from "./player";
...
export class ClientPlayer extends Player {
  ...
  unsetVisibleOutsideArea(areaName: string) {
    const index = this.visibleOutsideAreas.findIndex(
      (area) => area === areaName
    );
    if (index >= 0) {
  ...
  getAllVisibleTags(viewer: PlayerId) {
    ...
    for (const name of Object.keys(this.visiblePlayerTags)) {
      if (
        this.visiblePlayerTags[name] &&
        (!this.visiblePlayers[name] ||
          this.visiblePlayers[name].includes(viewer))
      ) {
