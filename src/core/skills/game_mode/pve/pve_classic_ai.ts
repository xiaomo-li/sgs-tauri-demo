  ...
  public canUse(
    ...
    switch (EventPacker.getIdentifier(content)) {
      case GameEventIdentifiers.DrawCardEvent:
        const markPlayer = room.AlivePlayers.find((player) =>
          player.getMark(MarkEnum.PveJian)
        );
        const drawCardEvent =
      ...
      case GameEventIdentifiers.DamageEvent:
        ...
        const hasMarkPlayer = room.AlivePlayers.find((player) =>
          player.getMark(MarkEnum.PveYu)
        );
        return (
          owner.Id === damageEvent.toId &&
          hasMarkPlayer !== undefined &&
          hasMarkPlayer.Role === damagedPlayer.Role
        );

      case GameEventIdentifiers.LevelBeginEvent:
        return (
          owner.getMark(MarkEnum.PveYing) > 0 ||
          owner.getMark(MarkEnum.PveJi) > 0
        );
      default:
        return false;
    }
    ...
    switch (identifier) {
      ...
      case GameEventIdentifiers.LevelBeginEvent:
        ...
        if (owner.getMark(MarkEnum.PveJi) > 0) {
          const partners = room.AlivePlayers.filter(
            (player) => player.Role === owner.Role
          );
...
@ShadowSkill
@CompulsorySkill({
  name: PveClassicAi.Name,
  description: PveClassicAi.Description,
})
