...
import { AllStage } from "../game/stage_processor";
...
export class SkillLifeCycle {
  ...
  public static async executeHookOnObtainingSkill(
    skill: Skill,
    room: Room,
    owner: Player
  ) {
    const hookedSkill = skill as unknown as OnDefineReleaseTiming;
    hookedSkill.whenObtainingSkill &&
      (await hookedSkill.whenObtainingSkill(room, owner));
  }
  public static async executeHookOnLosingSkill(
    skill: Skill,
    room: Room,
    owner: Player
  ) {
    const hookedSkill = skill as unknown as OnDefineReleaseTiming;
    hookedSkill.whenLosingSkill &&
      (await hookedSkill.whenLosingSkill(room, owner));
  }
  public static async executeHookedOnDead(
    skill: Skill,
    room: Room,
    owner: Player
  ) {
  ...
  public static async executeHookedOnNullifying(
    skill: Skill,
    room: Room,
    owner: Player
  ) {
    const hookedSkill = skill as unknown as OnDefineReleaseTiming;
    hookedSkill.whenNullifying &&
      (await hookedSkill.whenNullifying(room, owner));
  }
  public static async executeHookedOnEffecting(
    skill: Skill,
    room: Room,
    owner: Player
  ) {
