...
export class JueXiang extends TriggerSkill {
  ...
  public async onEffect(
    ...
    await room.obtainSkill(
      event.toIds[0],
      this.RelatedSkills[Math.floor(Math.random() * this.RelatedSkills.length)]
    );
...
@PersistentSkill()
@CommonSkill({
  name: "s_juexiang_prohibited",
  description: "s_juexiang_description",
})
...
@PersistentSkill()
@CommonSkill({
  name: "s_juexiang_remover",
  description: "s_juexiang_remover_description",
})
export class JueXiangRemover
