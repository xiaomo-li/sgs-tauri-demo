...
export class PianChong extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const redCards = room.findCardsByMatcherFrom(
      new CardMatcher({ suit: [CardSuit.Diamond, CardSuit.Heart] })
    );
    const blackCards = room.findCardsByMatcherFrom(
      new CardMatcher({ suit: [CardSuit.Club, CardSuit.Spade] })
    );

    const toGain: CardId[] = [];
    redCards.length > 0 &&
      toGain.push(redCards[Math.floor(Math.random() * redCards.length)]);
    blackCards.length > 0 &&
      toGain.push(blackCards[Math.floor(Math.random() * blackCards.length)]);
    ...
    const color =
      response.selectedOption === options[1] ? CardColor.Red : CardColor.Black;
      ...
      originalColors.length > 1
        ? TranslationPack.translationJsonPatcher(
            ...
            ...originalColors.map((color) =>
              Functional.getCardColorRawText(color)
            )
          ).toString()
        : TranslationPack.translationJsonPatcher(
            "pianchong: {0}",
    ...
    if (identifier === GameEventIdentifiers.PhaseChangeEvent) {
      ...
      for (const info of (
        ...
        for (const cardInfo of info.movingCards) {
          ...
          if (
            ...
            color.includes(CardColor.Black)
          ) {
            blackNum++;
          } else if (
            Sanguosha.getCardById(cardInfo.card).isRed() &&
            color.includes(CardColor.Red)
          ) {
            redNum++;
          }
        }
        ...
        if (redCards.length > 0) {
          toGain.push(
            ...Algorithm.randomPick(
              Math.min(redCards.length, blackNum),
              redCards
            )
          );
        }
      ...
      if (redNum > 0) {
        ...
        if (blackCards.length > 0) {
          toGain.push(
            ...Algorithm.randomPick(
              Math.min(blackCards.length, redNum),
              blackCards
            )
          );
        }
