...
@PersistentSkill()
@CommonSkill({
  name: "s_zhuihuan_buff",
  description: "s_zhuihuan_buff_description",
})
export class ZhuiHuanBuff
  ...
  implements OnDefineReleaseTiming
{
  public async whenDead(room: Room, player: Player) {
    ...
    player.getFlag<boolean>(ZhuiHuan.Name) !== undefined &&
      room.removeFlag(player.Id, ZhuiHuan.Name);
    ...
    if (
      ...
      if (!sources.includes(source)) {
        ...
        room
          ...
          .setFlag<PlayerId[]>(this.Name, sources);
      }
    } else {
      const sources = room.getFlag<PlayerId[]>(event.fromId, this.Name) || [];
      if (sources.length > 0) {
        ...
        for (const source of sources) {
          ...
          if (sourcePlayer.Hp > room.getPlayerById(event.fromId).Hp) {
            ...
            sourcePlayer.getCardIds(PlayerCardsArea.HandArea).length > 0
          ) {
            let toDiscard = sourcePlayer
              .getCardIds(PlayerCardsArea.HandArea)
              .filter((cardId) => room.canDropCard(source, cardId));
            toDiscard.length > 2 &&
              (toDiscard = Algorithm.randomPick(2, toDiscard));
      ...
      room.getFlag<boolean>(event.fromId, ZhuiHuan.Name) !== undefined &&
        room.removeFlag(event.fromId, ZhuiHuan.Name);
