  ...
  public async onEffect(
    ...
    for (let i = 0; i < room.getPlayerById(fromId).LostHp + 1; i++) {
      ...
      room
        ...
        .find((player) => player.getCardIds().length > 0) &&
        secOptions.push("yuyun:prey");
      room
        .getOtherPlayers(fromId)
        .find(
          (player) =>
            player.getCardIds(PlayerCardsArea.HandArea).length <
            Math.min(player.MaxHp, 5)
        ) && secOptions.push("yuyun:letDraw");

      secOptions = secOptions.filter(
      ...
      if (resp.selectedOption === "yuyun:draw2") {
        ...
        if (resp.selectedOption === "yuyun:discard") {
          ...
          targets = room.AlivePlayers.filter(
            (player) =>
              player.getCardIds(PlayerCardsArea.HandArea).length < player.MaxHp
          );
        }
        ...
        if (resp.selectedOption === "yuyun:damage") {
          ...
          const originalTargets =
            room.getFlag<PlayerId[]>(fromId, YuYun.YuYunTargets) || [];
          ...
          const options: CardChoosingOptions = {
            ...
            [PlayerCardsArea.HandArea]: to.getCardIds(PlayerCardsArea.HandArea)
              .length,
            [PlayerCardsArea.JudgeArea]: to.getCardIds(
