...
export class ShanJia extends TriggerSkill implements OnDefineReleaseTiming {
      ...
      const lostNum =
        room.Analytics.getRecordEvents<GameEventIdentifiers.MoveCardEvent>(
          ...
          if (event.infos.length === 1) {
            return (
              sum +
              event.infos[0].movingCards.filter((card) =>
                Sanguosha.getCardById(card.card).is(CardType.Equip)
              ).length
            );
          } else {
            ...
            for (const info of infos) {
              sum += info.movingCards.filter((card) =>
                Sanguosha.getCardById(card.card).is(CardType.Equip)
              ).length;
            }
          }
  ...
  public async onEffect(
    ...
    if (
      ...
      (dropCards.length > 0 &&
        !dropCards.find(
          (card) => !Sanguosha.getCardById(card).is(CardType.Equip)
        ))
    ) {
      const targets = room
    ...
    if (identifier === GameEventIdentifiers.GameStartEvent) {
      ...
      if (recordNum > 0) {
        ...
        if (moveCardEvent.infos.length === 0) {
          ...
          for (const info of infos) {
            num += info.movingCards.filter((card) =>
              Sanguosha.getCardById(card.card).is(CardType.Equip)
            ).length;
          }
        }
