...
export class TiQi extends TriggerSkill {
  ...
  public canUse(
    ...
    const drawnNum =
      room.Analytics.getRecordEvents<GameEventIdentifiers.DrawCardEvent>(
        (event) =>
  ...
  public async onEffect(
    ...
    const diff = Math.abs(
      2 - room.getPlayerById(event.fromId).getFlag<number>(this.Name)
    );
    ...
    if (response.selectedOption !== "cancel") {
      room.syncGameCommonRules(currentPlayer, (user) => {
        const changedValue =
          response.selectedOption === options[0] ? diff : -diff;
        user.addInvisibleMark(
          this.Name,
          user.getInvisibleMark(this.Name) + changedValue
        );
...
@CommonSkill({
  name: "s_tiqi_remover",
  description: "s_tiqi_remover_description",
})
