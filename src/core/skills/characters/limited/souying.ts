...
export class SouYing extends TriggerSkill implements OnDefineReleaseTiming {
  public async whenObtainingSkill(room: Room, player: Player) {
    const records =
      room.Analytics.getRecordEvents<GameEventIdentifiers.AimEvent>(
        (event) =>
    ...
    for (const record of records) {
      if (
        ...
        !player.getFlag<PlayerId[]>(this.Name)?.includes(record.fromId)
      ) {
        ...
        EventPacker.addMiddleware(
          { tag: this.Name, data: record.fromId },
          record
        );
      } else if (
        record.fromId === player.Id &&
        !!AimGroupUtil.getAllTargets(record.allTargets).find(
          (playerId) =>
            !player.getFlag<PlayerId[]>(SouYingShadow.Name)?.includes(playerId)
        )
      ) {
        const originalPlayers =
          player.getFlag<PlayerId[]>(SouYingShadow.Name) || [];
        originalPlayers.push(
          ...Algorithm.unique(
            AimGroupUtil.getAllTargets(record.allTargets),
            originalPlayers
          )
        );
        ...
        EventPacker.addMiddleware(
          { tag: SouYingShadow.Name, data: originalPlayers },
          record
        );
      }
    }
  ...
  implements OnDefineReleaseTiming
{
    ...
    return (
      ...
      (event.fromId === owner.Id &&
        ...
        !!AimGroupUtil.getAllTargets(event.allTargets).find(
          (playerId) =>
            !owner.getFlag<PlayerId[]>(this.Name)?.includes(playerId)
        ))
    );
  }
  ...
  public async onEffect(
    ...
    if (aimEvent.fromId === event.fromId) {
      ...
      originalPlayers.push(
        ...Algorithm.unique(
          AimGroupUtil.getAllTargets(aimEvent.allTargets),
          originalPlayers
        )
      );
      room
      ...
      EventPacker.addMiddleware(
        { tag: this.Name, data: originalPlayers },
        aimEvent
      );
    } else {
      const originalPlayers =
      ...
      room
        .getPlayerById(event.fromId)
        .setFlag<PlayerId[]>(this.GeneralName, originalPlayers);

      EventPacker.addMiddleware(
        { tag: this.GeneralName, data: aimEvent.fromId },
        aimEvent
      );
    }
...
@PersistentSkill()
@CommonSkill({
  name: SouYingShadow.Name,
  description: SouYingShadow.Description,
})
