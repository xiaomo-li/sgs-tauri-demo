...
import { CardColor, CardId, CardSuit } from "../../../cards/libs/card_props";
import {
  CardMoveArea,
  ...
  public async onEffect(
    ...
    for (const info of (
      ...
      const originalMappers =
        room.getFlag<JiaoYingMapper>(toId, JiaoYing.UsedTimesMappers) || {};
      originalMappers[event.fromId] = originalMappers[event.fromId] || 0;
      originalMappers[event.fromId]++;
      room
        .getPlayerById(toId)
        .setFlag<JiaoYingMapper>(JiaoYing.UsedTimesMappers, originalMappers);

      for (const skillName of [JiaoYingBlocker.Name, JiaoYingRemover.Name]) {
        room.getPlayerById(toId).hasShadowSkill(skillName) ||
          (await room.obtainSkill(toId, skillName));
      }
    }
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    return (
      ...
      !!room.getOtherPlayers(owner.Id).find((player) => {
        const jiaoYingMappers = room.getFlag<JiaoYingMapper>(
          player.Id,
          JiaoYing.UsedTimesMappers
        );
  ...
  public async beforeUse(
    ...
    const availableNum = room
      .getOtherPlayers(event.fromId)
      .reduce<number>((sum, player) => {
        const originalMappers = player.getFlag<JiaoYingMapper>(
          JiaoYing.UsedTimesMappers
        );
        if (
          originalMappers &&
          originalMappers[event.fromId] &&
          originalMappers[event.fromId] > 0
        ) {
    ...
    const targets = room.AlivePlayers.filter(
      (player) => player.getCardIds(PlayerCardsArea.HandArea).length < 5
...
@PersistentSkill()
@CommonSkill({
  name: "s_jiaoying_blocker",
  description: "s_jiaoying_blocker_description",
})
...
@PersistentSkill()
@CommonSkill({
  name: "s_jiaoying_remover",
  description: "s_jiaoying_remover_description",
})
export class JiaoYingRemover
  extends TriggerSkill
  implements OnDefineReleaseTiming
{
  public async whenLosingSkill(room: Room, player: Player) {
    player.getFlag<CardColor[]>(JiaoYing.Name) &&
      room.removeFlag(player.Id, JiaoYing.Name);
    player.removeFlag(JiaoYing.UsedTimesMappers);
    player.hasShadowSkill(JiaoYingBlocker.Name) &&
      (await room.loseSkill(player.Id, JiaoYingBlocker.Name));
  }
  ...
  public canUse(
    ...
    content: ServerEventFinder<
      GameEventIdentifiers.PhaseChangeEvent | GameEventIdentifiers.CardUseEvent
    >
  ): boolean {
    ...
    if (identifier === GameEventIdentifiers.PhaseChangeEvent) {
      return (
        (content as ServerEventFinder<GameEventIdentifiers.PhaseChangeEvent>)
          .from === PlayerPhase.PhaseFinish
      );
    } else if (identifier === GameEventIdentifiers.CardUseEvent) {
      return (
        (content as ServerEventFinder<GameEventIdentifiers.CardUseEvent>)
          .fromId === owner.Id &&
