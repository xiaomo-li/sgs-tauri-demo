    ...
    const { selectedCards } =
      await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingCardEvent>(
        GameEventIdentifiers.AskForChoosingCardEvent,
        {
          ...
          toId: fromId,
          customTitle:
            "xingzuo: please select cards to put on draw stack bottom",
        },
        fromId
      );
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async onEffect(
    ...
    await room.moveCards(
      ...
      {
        movingCards: toExchange.map((card) => ({
          card,
          fromArea: CardMoveArea.HandArea,
        })),
        fromId: toIds[0],
...
@PersistentSkill()
@CommonSkill({
  name: XingZuoShadow.Name,
  description: XingZuoShadow.Description,
})
export class XingZuoClear
  extends TriggerSkill
  implements OnDefineReleaseTiming
{
  public afterLosingSkill(
