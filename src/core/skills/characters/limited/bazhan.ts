...
import { ActiveSkill, SwitchSkillState } from "../../skill";
import { CommonSkill, SwitchSkill } from "../../skill_wrappers";
...
export class BaZhan extends ActiveSkill {
  ...
  public isAvailableTarget(
    ...
    return (
      target !== owner &&
      !(
        room.getPlayerById(owner).getSwitchSkillState(this.Name, true) ===
  ...
  public async onEffect(
    ...
    const currentSkillState = room
      .getPlayerById(event.fromId)
      .getSwitchSkillState(this.Name);
    ...
    if (currentSkillState === SwitchSkillState.Yang) {
      ...
      response.selectedCards = Algorithm.randomPick(
        response.selectedCardsIndex.length,
        handCards
      );
    ...
    if (hasHeartOrAlcohol) {
      ...
      const handler =
        currentSkillState === SwitchSkillState.Yang
          ? event.toIds[0]
          : event.fromId;
