...
export class WeiYi extends TriggerSkill {
  ...
  public isAutoTrigger(
    ...
    return (
      event !== undefined && room.getPlayerById(event.toId).Hp === owner.Hp
    );
  }
  ...
  public canUse(
    ...
    return (
      !owner.getFlag<PlayerId>(this.Name)?.includes(content.toId) &&
  ...
  public async onEffect(
    ...
    if (chosen) {
      ...
      room.getPlayerById(event.fromId).Hp < room.getPlayerById(victim).Hp
    ) {
      await room.loseHp(victim, 1);
    } else {
      await room.recover({
