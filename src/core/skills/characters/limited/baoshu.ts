...
export class BaoShu extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const markNum =
      room.getPlayerById(event.fromId).MaxHp + 1 - event.toIds.length;
    for (const toId of event.toIds) {
      room.addMark(toId, MarkEnum.Shu, markNum);
      room.getPlayerById(toId).hasShadowSkill(BaoShuBuff.Name) ||
        (await room.obtainSkill(toId, BaoShuBuff.Name));
    }
...
@CommonSkill({
  name: "s_baoshu_buff",
  description: "s_baoshu_buff_description",
})
export class BaoShuBuff extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public async onEffect(
    ...
    if (room.getMark(event.fromId, MarkEnum.Shu) > 0) {
      ...
      ).drawAmount += room.getMark(event.fromId, MarkEnum.Shu);
