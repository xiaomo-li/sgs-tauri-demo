...
export class BiFa extends TriggerSkill {
  ...
  public async onEffect(
    ...
    room.getPlayerById(toIds[0]).hasShadowSkill(BiFaEffect.Name) ||
      (await room.obtainSkill(toIds[0], BiFaEffect.Name));
...
@CommonSkill({
  name: "s_bifa_effect",
  description: "s_bifa_effect_description",
})
export class BiFaEffect extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public async onEffect(
    ...
    const bifa = room
      ...
      .getCardIds(PlayerCardsArea.OutsideArea, BiFa.Name);
    if (bifa.length > 0) {
            ...
            GameEventIdentifiers.AskForCardEvent,
            {
              ...
              conversation: TranslationPack.translationJsonPatcher(
                ...
                TranslationPack.patchPlayerInTranslation(
                  room.getPlayerById(bifaUser)
                )
              ).extract(),
              cardMatcher: new CardMatcher({
                type: [Sanguosha.getCardById(bifa[0]).BaseType],
              }).toSocketPassenger(),
      ...
      if (selectedCard) {
        await room.moveCards({
          movingCards: [
            { card: selectedCard, fromArea: CardMoveArea.HandArea },
          ],
          fromId: event.fromId,
          toId: bifaUser,
