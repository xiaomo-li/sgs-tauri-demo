...
export class JinJian extends TriggerSkill {
  ...
  public async onEffect(
    ...
    if (
      room.getFlag<AllStage>(event.fromId, this.JinJianStage) ===
      DamageEffectStage.DamageEffect
    ) {
      damageEvent.damage++;
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async whenDead(room: Room, player: Player) {
    for (const flagName of [this.GeneralName, JinJian.Debuff]) {
      player.getFlag<boolean>(flagName) !== undefined &&
        room.removeFlag(player.Id, flagName);
    }
  }
      ...
      if (
        stage === DamageEffectStage.DamageEffect
          ...
          : damageEvent.toId === owner.Id &&
            owner.getFlag<boolean>(JinJian.Debuff)
      ) {
  ...
  public async onEffect(
    ...
    if (
      ...
      if (
        room.getFlag<AllStage>(event.fromId, this.JinJianShadowStage) ===
      ...
      for (const flagName of [this.GeneralName, JinJian.Debuff]) {
        room.getFlag<boolean>(event.fromId, flagName) !== undefined &&
          room.removeFlag(event.fromId, flagName);
      }
    }
