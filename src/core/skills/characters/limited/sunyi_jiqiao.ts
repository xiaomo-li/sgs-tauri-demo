  ...
  public async onEffect(
    ...
    if (
      ...
      const response =
        await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingCardWithConditionsEvent>(
          GameEventIdentifiers.AskForChoosingCardWithConditionsEvent,
          {
            toId: event.fromId,
            customCardFields: {
              [Functional.getCardColorRawText(CardColor.Black)]: jiqiao.filter(
                (cardId) => Sanguosha.getCardById(cardId).isBlack()
              ),
              [Functional.getCardColorRawText(CardColor.Red)]: jiqiao.filter(
                (cardId) => Sanguosha.getCardById(cardId).isRed()
              ),
            },
      ...
      response.selectedCards = response.selectedCards || [
        jiqiao[Math.floor(Math.random() * jiqiao.length)],
      ];
      ...
      if (
        jiqiao.filter((cardId) => Sanguosha.getCardById(cardId).isBlack())
          .length ===
