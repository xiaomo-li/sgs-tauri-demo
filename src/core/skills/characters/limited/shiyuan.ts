...
export class ShiYuan extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      ...
      !(
        ...
        ((user.Hp > owner.Hp &&
          (yuweiEffected
            ? flag.includes(ShiYuanEffect.MoreHpSec)
            : flag.includes(ShiYuanEffect.MoreHp))) ||
          (user.Hp === owner.Hp &&
            (yuweiEffected
              ? flag.includes(ShiYuanEffect.SameHpSec)
              : flag.includes(ShiYuanEffect.SameHp))) ||
          (user.Hp < owner.Hp &&
            (yuweiEffected
              ? flag.includes(ShiYuanEffect.LesserHpSec)
              : flag.includes(ShiYuanEffect.LesserHp))))
      )
    );
  }
  ...
  public async onEffect(
    ...
    let num = 1;
    const originalFlags =
      room.getFlag<ShiYuanEffect[]>(event.fromId, this.Name) || [];
    let flag = originalFlags.includes(ShiYuanEffect.LesserHp)
      ? ShiYuanEffect.LesserHpSec
      : ShiYuanEffect.LesserHp;
    if (from.Hp > room.getPlayerById(event.fromId).Hp) {
      num = 3;
      flag = originalFlags.includes(ShiYuanEffect.MoreHp)
        ? ShiYuanEffect.MoreHpSec
        : ShiYuanEffect.MoreHp;
    } else if (from.Hp === room.getPlayerById(event.fromId).Hp) {
      num = 2;
      flag = originalFlags.includes(ShiYuanEffect.SameHp)
        ? ShiYuanEffect.SameHpSec
        : ShiYuanEffect.SameHp;
    }

    originalFlags.includes(flag) || originalFlags.push(flag);
    room
      .getPlayerById(event.fromId)
      .setFlag<ShiYuanEffect[]>(this.Name, originalFlags);
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    return (
      ...
      owner.getFlag<ShiYuanEffect[]>(this.GeneralName) !== undefined
    );
  }
