...
import { LeBuSiShu } from "../../../cards/standard/lebusishu";
...
export class LiMu extends ActiveSkill {
  ...
  public async onEffect(
    ...
    await room.useCard({
      fromId,
      targetGroup: [[fromId]],
      cardId: VirtualCard.create<LeBuSiShu>(
        { cardName: "lebusishu", bySkill: this.Name },
        cardIds
      ).Id,
    });
...
export class LiMuShadow extends RulesBreakerSkill {
  public breakCardUsableDistanceTo(
    ...
    if (
      ...
      room.withinAttackDistance(owner, target)
    ) {
