...
export class KanNan extends ActiveSkill {
  ...
  public canUse(room: Room, owner: Player): boolean {
    return (
      owner.hasUsedSkillTimes(this.Name) < owner.Hp &&
      !owner.getFlag<PlayerId[]>(this.Name)?.includes(owner.Id)
    );
  }
  ...
  public async onEffect(
    ...
    if (winner) {
      const originalNum =
        room.getFlag<number>(winner, KanNan.KanNanDamage) || 0;
      room.setFlag<number>(
        winner,
        KanNan.KanNanDamage,
        originalNum + 1,
...
@CommonSkill({
  name: "s_kannan_buff",
  description: "s_kannan_buff_description",
})
export class KanNanBuff extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public async onEffect(
    ...
    const additionalDMG = room.getFlag<number>(
      event.fromId,
      KanNan.KanNanDamage
    );
    if (additionalDMG) {
