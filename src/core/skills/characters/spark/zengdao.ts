...
import { ActiveSkill, LimitSkill, TriggerSkill } from "../../skill";
...
@CommonSkill({
  name: "s_zengdao_buff",
  description: "s_zengdao_buff_description",
})
export class ZengDaoBuff extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public canUse(
    ...
    return (
      ...
      owner.getCardIds(PlayerCardsArea.OutsideArea, ZengDao.Name).length > 0
    );
  }
  ...
  public async onEffect(
    ...
    const dao = room
      ...
      .getCardIds(PlayerCardsArea.OutsideArea, ZengDao.Name);
    const response =
    ...
    if (
      room
        .getPlayerById(fromId)
        .getCardIds(PlayerCardsArea.OutsideArea, ZengDao.Name).length === 0
    ) {
