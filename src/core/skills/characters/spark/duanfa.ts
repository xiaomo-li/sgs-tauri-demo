...
export class DuanFa extends ActiveSkill implements OnDefineReleaseTiming {
  public async whenObtainingSkill(room: Room, player: Player) {
    const num = room.Analytics.getCardDropRecord(
      player.Id,
      "phase"
    ).reduce<number>((sum, event) => {
      const infos = event.infos.filter(
        (info) => info.movedByReason === this.Name
      );
      for (const info of infos) {
        sum += info.movingCards
          .filter((card) => !Sanguosha.isVirtualCardId(card.card))
          .map((card) => card.card).length;
      }
  ...
  public canUse(room: Room, owner: Player): boolean {
    return (
      ...
      (owner.getFlag<number>(this.Name) || 0) < owner.MaxHp
    );
  }
  ...
  public cardFilter(room: Room, owner: Player, cards: CardId[]): boolean {
    return (
      cards.length > 0 &&
      cards.length <= owner.MaxHp - (owner.getFlag<number>(this.Name) || 0)
    );
  }
  ...
  public isAvailableCard(owner: PlayerId, room: Room, cardId: CardId): boolean {
    return (
      Sanguosha.getCardById(cardId).isBlack() && room.canDropCard(owner, cardId)
    );
  }
