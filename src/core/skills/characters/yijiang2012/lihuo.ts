    ...
    const players = room
      ...
      .filter(
        (player) =>
          ...
          !TargetGroupUtil.includeRealTarget(
...
export class LiHuoPut extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const realCardId = card.isVirtualCard()
      ? (card as VirtualCard).getRealActualCards()[0]
      : cardUseEvent.cardId;
    await room.moveCards({
      movingCards: [
        { card: realCardId, fromArea: CardMoveArea.ProcessingArea },
      ],
      toId: fromId,
