...
export class ZiShouReforge extends TriggerSkill {
    ...
    return (
      ...
      room.Analytics.getRecordEvents<GameEventIdentifiers.CardUseEvent>(
        (event) =>
          ...
          TargetGroupUtil.getRealTargets(event.targetGroup).find(
  ...
  public isAvailableCard(
    ...
    return (
      (selectedCards.length === 0 ||
        selectedCards.find(
          (card) =>
            Sanguosha.getCardById(card).Suit ===
            Sanguosha.getCardById(cardId).Suit
        ) === undefined) &&
  ...
  public async onEffect(
    ...
    event.cardIds = Precondition.exists(
      event.cardIds,
      "Unable to get zishou cards"
    );
...
@ShadowSkill
@CommonSkill({
  name: ZiShouReforge.Name,
  description: ZiShouReforge.Description,
})
export class ZiShouPrevent
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async onTrigger(
    ...
    content.translationsMessage = TranslationPack.translationJsonPatcher(
      ...
      this.GeneralName,
      TranslationPack.patchPlayerInTranslation(
        room.getPlayerById(damageEvent.toId!)
      )
    ).extract();
...
@ShadowSkill
@CommonSkill({
  name: ZiShouPrevent.Name,
  description: ZiShouPrevent.Description,
})
export class ZiShouShadow
  extends TriggerSkill
  implements OnDefineReleaseTiming
{
  public afterLosingSkill(
