...
import { PersistentSkill, ShadowSkill } from "../../skill_wrappers";
      ...
      if (targets.length > 0) {
        const { selectedPlayers } =
          await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
            GameEventIdentifiers.AskForChoosingPlayerEvent,
            {
              ...
              requiredAmount: 1,
              conversation:
                "gongqi: do you want to drop one card of another player?",
        ...
        if (selectedPlayers && selectedPlayers.length === 1) {
          ...
          const options: CardChoosingOptions = {
            [PlayerCardsArea.EquipArea]: to.getCardIds(
  ...
  implements OnDefineReleaseTiming
{
  ...
  public breakFinalAttackRange(room: Room, owner: Player): number {
    return owner.getFlag<CardSuit[]>(this.GeneralName)
      ? INFINITE_ATTACK_RANGE
      : -1;
  }
  ...
  public breakCardUsableTimes(
    ...
    if (cardId instanceof CardMatcher) {
      match = cardId.match(
        new CardMatcher({ generalName: ["slash"], suit: suits })
      );
    } else {
      match =
        suits.includes(Sanguosha.getCardById(cardId).Suit) &&
