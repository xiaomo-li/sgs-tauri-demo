...
export class TongYe extends TriggerSkill {
  ...
  public async onEffect(
      ...
      await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingOptionsEvent>(
        GameEventIdentifiers.AskForChoosingOptionsEvent,
        {
          ...
          conversation: TranslationPack.translationJsonPatcher(
            "{0}: please choose tongye options",
            this.Name
          ).extract(),
          toId: event.fromId,
    ...
    const currentEquipsNum = room.AlivePlayers.reduce<number>(
      (sum, player) =>
        sum + player.getCardIds(PlayerCardsArea.EquipArea).length,
      0
    );
    room.setFlag<string>(
      event.fromId,
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async onEffect(
    ...
    if (
      ...
      const tongyeRecord = room
        .getFlag<string>(event.fromId, this.GeneralName)
        .split("+");
      ...
      if (
        (tongyeRecord[0] === TongYe.Options[0] &&
          parseInt(tongyeRecord[1], 10) !== currentEquipsNum) ||
        (tongyeRecord[0] === TongYe.Options[1] &&
          parseInt(tongyeRecord[1], 10) === currentEquipsNum)
      ) {
        room.getMark(event.fromId, MarkEnum.Ye) < 2 &&
          room.addMark(event.fromId, MarkEnum.Ye, 1);
      } else {
        room.getMark(event.fromId, MarkEnum.Ye) > 0 &&
          room.addMark(event.fromId, MarkEnum.Ye, -1);
      }
    } else {
