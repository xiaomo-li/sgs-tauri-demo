...
@PersistentSkill()
@CommonSkill({
  name: DuoJingShadow.Name,
  description: DuoJingShadow.Description,
})
export class DuoJingRemover
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      return (
        EventPacker.getMiddleware<PlayerId>(this.GeneralName, event) ===
          owner.Id &&
