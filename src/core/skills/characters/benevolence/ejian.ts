  ...
  public async onEffect(
    ...
    for (const info of (
      ...
      if (
        info.toId &&
        !(
          room.getPlayerById(event.fromId).getFlag<PlayerId[]>(this.Name) || []
        ).includes(info.toId) &&
        info.triggeredBySkills?.includes(BoMing.Name)
      ) {
        const sameTypeCards = room
          ...
          .filter(
            (cardId) =>
              ...
              Sanguosha.getCardById(info.movingCards[0].card).BaseType ===
                Sanguosha.getCardById(cardId).BaseType
          );
