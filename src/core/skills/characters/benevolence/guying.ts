...
export class GuYing extends TriggerSkill {
      ...
      return (
        ...
        !!(
          ...
          (info) =>
            ...
            VirtualCard.getActualCards([info.movingCards[0].card]).length ===
              1 &&
  ...
  public async onEffect(
    ...
    if (
      ...
      const cardId = VirtualCard.getActualCards([
        moveCardInfo.movingCards[0].card,
      ])[0];
      ...
      if (
        ...
        room.isCardOnProcessing(cardId) &&
          (currentArea = CardMoveArea.ProcessingArea);
      } else {
        room.isCardInDropStack(cardId) &&
          (currentArea = CardMoveArea.DropStack);
      }
      ...
      if (options.length > 0) {
        ...
        if (response.selectedOption === "guying:giveRandomly") {
          const randomCard =
            currentPlayerCards[
              Math.floor(Math.random() * currentPlayerCards.length)
            ];
          await room.moveCards({
            movingCards: [
              {
                card: randomCard,
                fromArea: room.CurrentPlayer.cardFrom(randomCard),
              },
            ],
