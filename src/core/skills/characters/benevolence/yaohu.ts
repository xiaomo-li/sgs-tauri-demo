...
import { DamageCardEnum } from "../../../cards/libs/card_props";
...
export class YaoHu extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const nationalities = room.AlivePlayers.reduce<CharacterNationality[]>(
      (nas, player) => {
    ...
    const index = options.findIndex(
      (nationalityText) => nationalityText === response.selectedOption
    );
...
export class YaoHuShadow extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      ...
      room.getPlayerById(event.playerId).Nationality ===
        owner.getFlag<CharacterNationality>(this.GeneralName) &&
  ...
  public async onEffect(
    ...
    const shengs = room
      ...
      .getCardIds(PlayerCardsArea.OutsideArea, JuTu.Name);
    const response =
      await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingCardEvent>(
    ...
    response.selectedCards = response.selectedCards || [
      shengs[Math.floor(Math.random() * shengs.length)],
    ];
    await room.moveCards({
      movingCards: [
        { card: response.selectedCards[0], fromArea: CardMoveArea.OutsideArea },
      ],
      fromId: event.fromId,
      toId,
    ...
    const slashTargets = room
      ...
      .filter(
        (player) =>
          room.withinAttackDistance(room.getPlayerById(toId), player) &&
          player.Id !== event.fromId
      )
    ...
    if (slashTargets.length > 0) {
      const resp =
        await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
          GameEventIdentifiers.AskForChoosingPlayerEvent,
          {
            ...
            requiredAmount: 1,
            conversation:
              "yaohu: please choose a target to be the target of the slash",
            triggeredBySkills: [this.GeneralName],
          },
          event.fromId,
          true
      ...
      resp.selectedPlayers = resp.selectedPlayers || [
        slashTargets[Math.floor(Math.random() * slashTargets.length)],
      ];
      const slashResponse = await room.askForCardUse(
        {
          ...
          conversation: TranslationPack.translationJsonPatcher(
            ...
            TranslationPack.patchPlayerInTranslation(
              room.getPlayerById(resp.selectedPlayers[0])
            )
          ).extract(),
          triggeredBySkills: [this.GeneralName],
        },
    ...
    room.getPlayerById(toId).hasShadowSkill(YaoHuDebuff.Name) ||
      (await room.obtainSkill(toId, YaoHuDebuff.Name));
...
@CommonSkill({
  name: "s_yaohu_debuff",
  description: "s_yaohu_debuff_description",
})
export class YaoHuDebuff extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.AimEvent) {
      const aimEvent =
        event as ServerEventFinder<GameEventIdentifiers.AimEvent>;
      return (
        aimEvent.fromId === owner.Id &&
        (owner.getFlag<PlayerId[]>(YaoHuShadow.Name) || []).includes(
          aimEvent.toId
        ) &&
        (Object.values(DamageCardEnum) as string[]).includes(
          Sanguosha.getCardById(aimEvent.byCardId).GeneralName
        )
      );
