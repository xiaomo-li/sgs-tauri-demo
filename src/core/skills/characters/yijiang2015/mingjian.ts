...
@PersistentSkill()
@CommonSkill({
  name: "s_mingjian_buff",
  description: "s_mingjian_buff_description",
})
...
@PersistentSkill()
@CommonSkill({
  name: "s_mingjian_remover",
  description: "s_mingjian_remover_description",
})
export class MingJianRemover
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async whenDead(room: Room, player: Player) {
    player.hasShadowSkill(MingJianBuff.Name) &&
      (await room.loseSkill(player.Id, MingJianBuff.Name));
  ...
  public canUse(
    ...
    return (
      owner.Id === event.fromPlayer && event.from === PlayerPhase.PhaseFinish
    );
  }
