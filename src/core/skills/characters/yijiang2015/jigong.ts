...
export class JiGong extends TriggerSkill {
  ...
  public async beforeUse(
    ...
    room.notify(
      GameEventIdentifiers.AskForChoosingOptionsEvent,
      {
        ...
        conversation: TranslationPack.translationJsonPatcher(
          "{0}: please choose jigong options",
          this.Name
        ).extract(),
      },
      fromId
    ...
    if (selectedOption) {
      EventPacker.addMiddleware(
        {
          tag: this.Name,
          data: options.findIndex((option) => option === selectedOption) + 1,
        },
        event
      );
  ...
  public async onEffect(
    ...
    const damage = room.Analytics.getDamageRecord(
      event.fromId,
      "phase"
    ).reduce<number>((sum, event) => sum + event.damage, 0);
    room.setFlag<number>(
      event.fromId,
      this.Name,
      damage,
  ...
  implements OnDefineReleaseTiming
{
  ...
  public breakBaseCardHoldNumber(room: Room, owner: Player) {
    return owner.getFlag<number>(this.GeneralName) !== undefined
      ? owner.getFlag<number>(this.GeneralName)
      : -1;
  }
}
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.DamageEvent) {
      ...
      room.getFlag<number>(fromId, this.GeneralName) >=
        room.getFlag<number>(fromId, JiGong.JiGongNum) &&
        (await room.recover({
          toId: fromId,
          recoveredHp: 1,
          recoverBy: fromId,
