...
export class HuaiYi extends ActiveSkill {
  ...
  public async onEffect(
    ...
    if (
      !handcards.find(
        (id) =>
          Sanguosha.getCardById(handcards[0]).Color !==
          Sanguosha.getCardById(id).Color
      )
    ) {
      ...
      const color =
        response.selectedOption === options[1]
          ? CardColor.Red
          : CardColor.Black;

      const todrop = handcards.filter(
        (id) =>
          Sanguosha.getCardById(id).Color === color &&
          room.canDropCard(fromId, id)
      );
      if (todrop.length > 0) {
        ...
        if (players.length > 0) {
          ...
          for (const target of resp.selectedPlayers) {
            ...
            const options: CardChoosingOptions = {
              ...
              [PlayerCardsArea.HandArea]: to.getCardIds(
            ...
            const newResp = await room.askForChoosingPlayerCard(
              chooseCardEvent,
              fromId,
              false,
              true
            ...
            newResp &&
              (await room.moveCards({
                movingCards: [
                  { card: newResp.selectedCard!, fromArea: newResp.fromArea },
                ],
                fromId: target,
                toId: fromId,
