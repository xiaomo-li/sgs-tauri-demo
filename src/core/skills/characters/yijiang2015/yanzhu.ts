...
export class YanZhu extends ActiveSkill {
  ...
  public isAvailableTarget(
    ...
    return (
      target !== owner && room.getPlayerById(target).getPlayerCards().length > 0
    );
  }
  ...
  public async onEffect(
    ...
    const response = await room.askForCardDrop(
      ...
      room.getPlayerById(toIds[0]).getCardIds(PlayerCardsArea.EquipArea)
        .length === 0,
      undefined,
      this.Name,
      room.getPlayerById(toIds[0]).getCardIds(PlayerCardsArea.EquipArea)
...
@PersistentSkill()
@CommonSkill({
  name: "s_yanzhu_debuff",
  description: "s_yanzhu_debuff_description",
})
export class YanZhuDebuff
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.DamageEvent) {
      return (
        (event as ServerEventFinder<GameEventIdentifiers.DamageEvent>).toId ===
        owner.Id
      );
  ...
  public async onEffect(
    ...
    if (additionalDamage) {
      ...
      if (
        EventPacker.getIdentifier(unkownEvent) ===
        GameEventIdentifiers.DamageEvent
      ) {
        (
          unkownEvent as ServerEventFinder<GameEventIdentifiers.DamageEvent>
...
export class YanZhuEX extends YanZhu {
  ...
  public canUse(room: Room, owner: Player): boolean {
    return (
      !owner.hasUsedSkill(this.Name) && !owner.hasUsedSkill(this.GeneralName)
    );
  }
  ...
  public async onEffect(
    ...
    const originalDamage =
      room.getFlag<number>(toIds[0], this.GeneralName) || 0;
    room.setFlag<number>(
      toIds[0],
      this.GeneralName,
      originalDamage + 1,
