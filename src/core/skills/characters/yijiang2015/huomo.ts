...
export class HuoMo extends ViewAsSkill implements OnDefineReleaseTiming {
  public async whenObtainingSkill(room: Room, player: Player) {
    ...
    for (const event of records) {
      ...
      const usedCards =
        room.getFlag<string[]>(player.Id, this.GeneralName) || [];
  ...
  public canViewAs(room: Room, owner: Player): string[] {
    ...
    ).filter((name) => !usedCards.includes(name));
  }
  ...
  public isAvailableCard(
    ...
    return (
      !Sanguosha.getCardById(pendingCardId).is(CardType.Basic) &&
...
export class HuoMoShadow extends TriggerSkill {
  ...
  public isTriggerable(
    ...
    return (
      stage === CardUseStage.PreCardUse && Card.isVirtualCardId(event.cardId)
    );
  }
