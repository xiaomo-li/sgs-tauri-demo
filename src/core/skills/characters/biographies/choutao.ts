...
export class ChouTao extends TriggerSkill {
  ...
  public getSkillLog(
    ...
    return TranslationPack.translationJsonPatcher(
      `{0}: do you want to discard a card from {1} to set {2} Unoffsetable${
        event.fromId === owner.Id
          ? " and restore your limit of using slash"
          : ""
      }?`,
      this.Name,
  ...
  public async onEffect(
    ...
    const options: CardChoosingOptions = {
      [PlayerCardsArea.EquipArea]: userPlayer.getCardIds(
        PlayerCardsArea.EquipArea
      ),
      [PlayerCardsArea.HandArea]: userPlayer.getCardIds(
