...
export class DaoJi extends TriggerSkill implements OnDefineReleaseTiming {
        ...
        GameEventIdentifiers.AskForChoosingOptionsEvent,
        {
          ...
          conversation: TranslationPack.translationJsonPatcher(
            ...
            TranslationPack.patchPlayerInTranslation(
              room.getPlayerById(cardUseEvent.fromId)
            )
          ).extract(),
          toId: event.fromId,
  ...
  public async onEffect(
    ...
    if (chosen === this.DaoJiOptions[0]) {
      ...
      user.hasShadowSkill(DaoJiDebuff.Name) ||
        (await room.obtainSkill(user.Id, DaoJiDebuff.Name));
    }
...
@CommonSkill({
  name: "s_daoji_debuff",
  description: "s_daoji_debuff_description",
})
...
@PersistentSkill()
@CommonSkill({
  name: "s_daoji_remover",
  description: "s_daoji_remover_description",
})
export class DaoJiRemover
  extends TriggerSkill
  implements OnDefineReleaseTiming
{
  public async whenDead(room: Room, owner: Player) {
    owner.hasShadowSkill(DaoJiDebuff.Name) &&
      (await room.loseSkill(owner.Id, DaoJiDebuff.Name));
