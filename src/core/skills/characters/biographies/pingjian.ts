...
export class PingJian extends ActiveSkill {
  ...
  public canUse(room: Room, owner: Player): boolean {
    ...
    return (
      ...
      skillsUsed[0].length < PingJian.PingJianSkillPool[0].length
    );
  }
  ...
  public async onEffect(
    ...
    if (skillsUsed.length > 0) {
      newSkillPool = newSkillPool.filter(
        (skill) => !skillsUsed[0].includes(skill)
      );
    }
    ...
    for (let i = 0; i < Math.min(n, 3); i++) {
      const chosenSkill =
        newSkillPool[Math.floor(Math.random() * newSkillPool.length)];
      ...
      EventPacker.createUncancellableEvent<GameEventIdentifiers.AskForChoosingOptionsEvent>(
        {
          ...
          optionPrompt: options.map((option) => ({
            option,
            sideTip: Sanguosha.getSkillBySkillName(option).Description,
          })),
...
export class PingJianShadow extends TriggerSkill {
  ...
  public canUse(
    ...
    if (
      ...
      return (
        ...
        skillsUsed[1].length < PingJian.PingJianSkillPool[1].length
      );
    } else if (
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      realEvent =
        unknownEvent as ServerEventFinder<GameEventIdentifiers.PhaseStageChangeEvent>;
    }
    ...
    if (skillsUsed.length > 0) {
      newSkillPool = newSkillPool.filter(
        (skill) => !skillsUsed[index].includes(skill)
      );
    }
...
@PersistentSkill()
@CommonSkill({
  name: PingJianShadow.Name,
  description: PingJianShadow.Description,
})
export class PingJianLoseSkill
  ...
  implements OnDefineReleaseTiming
{
  ...
  public isTriggerable(
    ...
    return (
      stage === PhaseStageChangeStage.StageChanged ||
      stage === SkillEffectStage.BeforeSkillEffect
    );
  }
  ...
  public canUse(
    ...
    if (
      ...
      const skillEffectEvent =
        content as ServerEventFinder<GameEventIdentifiers.SkillEffectEvent>;
      return (
        skillEffectEvent.fromId === owner.Id &&
        skillsUsing.find(
          (skill) =>
            skill === skillEffectEvent.skillName ||
            (PingJian.PingJianSkillMap[skill] &&
              PingJian.PingJianSkillMap[skill].includes(
                skillEffectEvent.skillName
              ))
        ) !== undefined
      );
    }
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      ...
      const skillEffectEvent =
        unknownEvent as ServerEventFinder<GameEventIdentifiers.SkillEffectEvent>;
