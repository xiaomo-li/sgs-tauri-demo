  ...
  public async onEffect(
    ...
    for (const toId of event.toIds) {
      ...
      if (selectedCards.length > 0) {
          ...
          movingCards: [
            {
              ...
              fromArea: room.getPlayerById(toId).cardFrom(selectedCards[0]),
            },
          ],
          fromId: toId,
          toId: fromId,
        ...
        const targets =
          room.getFlag<PlayerId[]>(fromId, XueZhao.XueZhaoTargets) || [];
        targets.includes(toId) || targets.push(toId);
        room
          .getPlayerById(fromId)
          .setFlag<PlayerId[]>(XueZhao.XueZhaoTargets, targets);
