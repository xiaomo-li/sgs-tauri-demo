    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      ...
      return (
        ...
        room
          ...
          .find((player) => player.hasSkill(CiXiao.Name)) !== undefined
      );
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      ...
      const dad = room
        ...
        .filter((player) => player.hasSkill(CiXiao.Name));
      if (dad.length > 1) {
        const skillUseEvent =
          EventPacker.createUncancellableEvent<GameEventIdentifiers.AskForSkillUseEvent>(
            {
              ...
              toId: fromId,
              conversation:
                "panshi: please choose one hand card and one target",
            }
          );
        room.notify(
...
@CommonSkill({
  name: "shadow_panshi",
  description: "shadow_panshi_description",
})
