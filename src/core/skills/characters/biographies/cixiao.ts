...
export class CiXiao extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public canUse(
    ...
    return (
      ...
      room
        ...
        .find((player) => !player.getFlag<boolean>(this.Name)) !== undefined &&
      !(
  ...
  public cardFilter(room: Room, owner: Player, cards: CardId[]): boolean {
    return (
      cards.length ===
      (room.AlivePlayers.find((player) => player.getFlag<boolean>(this.Name))
        ? 1
        : 0)
    );
  }
  ...
  public getSkillLog(room: Room, owner: Player): PatchedTranslationObject {
    return room.AlivePlayers.find((player) =>
      player.getFlag<boolean>(this.Name)
    )
