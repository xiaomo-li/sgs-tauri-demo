...
export class ZhaXiang extends TriggerSkill {
  ...
  isRefreshAt(room: Room, owner: Player, phase: PlayerPhase) {
    return (
      phase === PlayerPhase.PlayCardStage && room.CurrentPlayer.Id === owner.Id
    );
  }
  ...
  public canUse(
    room: Room,
    owner: Player,
    content: ServerEventFinder<GameEventIdentifiers.LoseHpEvent>
  ) {
  ...
  public async onEffect(
    ...
    if (room.CurrentPlayer.Id === skillUseEvent.fromId) {
      const num =
        room.getFlag<number>(skillUseEvent.fromId, this.GeneralName) || 0;
...
@ShadowSkill
@CompulsorySkill({
  name: ZhaXiang.GeneralName,
  description: ZhaXiang.Description,
})
export class ZhaXiangShadow
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    return (
      room.getFlag<number>(owner.Id, this.GeneralName) !== 0 &&
      owner.Id === event.cardUserId
    );
  }
  ...
  public async onEffect(
    ...
    const currentEvent =
      triggeredOnEvent as ServerEventFinder<GameEventIdentifiers.AskForCardUseEvent>;
...
@ShadowSkill
@CompulsorySkill({
  name: ZhaXiangShadow.Name,
  description: ZhaXiang.Description,
})
export class ZhaXiangDistance
  ...
  implements OnDefineReleaseTiming
{
  ...
  public breakCardUsableDistance(
    ...
    if (
      ...
      !room.getFlag<number>(owner.Id, this.GeneralName)
    ) {
      return 0;
    }
    ...
    if (cardId instanceof CardMatcher) {
      match = cardId.match(
        new CardMatcher({
          suit: [CardSuit.Heart, CardSuit.Diamond],
          generalName: ["slash"],
        })
      );
    } else {
