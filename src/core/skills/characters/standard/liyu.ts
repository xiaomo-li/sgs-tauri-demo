  ...
  public async onEffect(
    ...
    await room.moveCards({
      ...
      translationsMessage: TranslationPack.translationJsonPatcher(
        ...
        TranslationPack.patchPlayerInTranslation(
          room.getPlayerById(chooseCardEvent.fromId)
        ),
        TranslationPack.patchCardInTranslation(response.selectedCard),
        TranslationPack.patchPlayerInTranslation(
          room.getPlayerById(chooseCardEvent.toId)
        )
      ).extract(),
    });
    ...
    if (responseCard.is(CardType.Equip)) {
      const targets = room
        ...
        .filter(
          (p) =>
            p.Id !== damageEvent.toId &&
            from.canUseCardTo(room, new CardMatcher({ name: ["duel"] }), p.Id)
        )
        .map((p) => p.Id);
