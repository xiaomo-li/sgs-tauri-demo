...
export class Rende extends ActiveSkill {
  ...
  isAvailableTarget(owner: PlayerId, room: Room, target: PlayerId): boolean {
    return (
      owner !== target &&
      !room.getPlayerById(target).getFlag<boolean>(this.Name)
    );
  }
  ...
  async onEffect(
    ...
    await room.moveCards({
      movingCards: skillUseEvent.cardIds!.map((card) => ({
        card,
    ...
    if (
      ...
      const hasLegionFightExt = room.Info.cardExtensions.includes(
      ...
      if (
        hasLegionFightExt &&
        from.canUseCard(room, new CardMatcher({ name: ["alcohol"] }))
      ) {
