...
export class LuoShen extends TriggerSkill {
  ...
  canUse(
    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      ...
      return (
        ...
        PlayerPhaseStages.PrepareStage === content.toStage
      );
  ...
  async onTrigger(
    ...
    const identifier =
      triggeredOnEvent && EventPacker.getIdentifier(triggeredOnEvent);
  ...
  async onEffect(
    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      ...
      if (Sanguosha.getCardById(judgeEvent.judgeCardId).isBlack()) {
        ...
        const originalLuoShenCards =
          room.getCardTag(skillUseEvent.fromId, this.Name) || [];
...
export class LuoShenShadow extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const otherHandCards = player
      ...
      .filter((card) => !luoshenCards.includes(card));
