...
@ShadowSkill
@CompulsorySkill({
  name: WuShuang.GeneralName,
  description: WuShuang.Description,
})
export class WuShuangShadow extends TriggerSkill {
  ...
  public canUse(
    ...
    switch (identifier) {
      case GameEventIdentifiers.CardUseEvent: {
        ...
        const slashEvent =
          responseToEvent as ServerEventFinder<GameEventIdentifiers.CardUseEvent>;
        canUse =
      ...
      case GameEventIdentifiers.CardResponseEvent: {
        const slashEvent =
          event as ServerEventFinder<GameEventIdentifiers.CardResponseEvent>;
        const { responseToEvent } = slashEvent;
        const duelEvent =
          responseToEvent as ServerEventFinder<GameEventIdentifiers.CardUseEvent>;
        canUse =
          Sanguosha.getCardById(duelEvent.cardId).GeneralName === "duel" &&
    ...
    return (
      canUse &&
      !!EventPacker.getMiddleware<boolean>(this.GeneralName, responseToEvent)
    );
  }
  ...
  async onEffect(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
          ...
          slashEvent.fromId !== undefined
            ? TranslationPack.translationJsonPatcher(
                ...
                TranslationPack.patchPlayerInTranslation(
                  room.getPlayerById(slashEvent.fromId)
                ),
      ...
      const duelResponseEvent =
        unknownEvent as ServerEventFinder<GameEventIdentifiers.CardResponseEvent>;
      ...
      const response = await room.askForCardResponse(
        askForResponseCardEvent,
        duelResponseEvent.fromId
      );
      if (response.cardId === undefined) {
