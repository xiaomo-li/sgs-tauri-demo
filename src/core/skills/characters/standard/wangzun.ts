...
import { PlayerId, PlayerRole } from "../../../player/player_props";
...
export class WangZun extends TriggerSkill {
  ...
  async onEffect(
    ...
    if (
      room.getPlayerById(phaseStageChangeEvent.playerId).Role ===
      PlayerRole.Lord
    ) {
  ...
  implements OnDefineReleaseTiming
{
  ...
  canUse(
    ...
    return (
      ...
      lord.getInvisibleMark(this.GeneralName) > 0
    );
  }
