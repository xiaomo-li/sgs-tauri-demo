...
export class HanZhan extends TriggerSkill {
  ...
  canUse(
    room: Room,
    owner: Player,
    content: ServerEventFinder<GameEventIdentifiers.PinDianEvent>
  ) {
  ...
  async onEffect(
    ...
    const pindianEvent =
      triggeredOnEvent as ServerEventFinder<GameEventIdentifiers.PinDianEvent>;
...
export class HanZhanHidden extends TriggerSkill {
  ...
  canUse(
    ...
    const involved =
      owner.Id === content.fromId || content.toIds.includes(owner.Id);
    if (!involved) {
      return false;
    }
    ...
    return (
      ...
      content.procedures.find(
        (result) => Sanguosha.getCardById(result.cardId).GeneralName === "slash"
      ) !== undefined
    );
  }
  ...
  async onEffect(
    ...
    const cards = pindianEvent.procedures.map((result) =>
      Sanguosha.getCardById(result.cardId)
    );
