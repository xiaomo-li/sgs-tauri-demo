...
export class CangZhuo extends TriggerSkill {
  ...
  canUse(
    ...
    return (
      ...
      room.Analytics.getUsedCard(owner.Id, "round", undefined, 1).filter(
        (cardId) => Sanguosha.getCardById(cardId).is(CardType.Trick)
      ).length === 0
    );
  }
...
@ShadowSkill
@CompulsorySkill({
  name: CangZhuo.GeneralName,
  description: CangZhuo.Description,
})
export class CangZhuoShadow
