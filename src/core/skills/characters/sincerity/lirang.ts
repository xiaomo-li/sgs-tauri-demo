  ...
  public async onEffect(
    ...
    if (
      ...
      room
        ...
        .setFlag<PlayerId>(this.Name, drawCardEvent.fromId);
    } else {
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      ...
      return (
        ...
        room.Analytics.getCardDropRecord(
          phaseStageChangeEvent.playerId,
          "phase",
          undefined,
          1
  ...
  public async beforeUse(
    ...
    if (
      ...
      const droppedCardIds = room.Analytics.getCardDropRecord(
        currentId,
        "phase",
        undefined
      ).reduce<CardId[]>((cardIds, dropEvent) => {
      ...
      if (droppedCardIds.length > 0) {
        const response =
          await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingCardWithConditionsEvent>(
            GameEventIdentifiers.AskForChoosingCardWithConditionsEvent,
            {
              ...
              customTitle:
                "lirang: you can choose at most 2 cards of these cards to gain",
              amount: [1, 2],
            },
            event.fromId
          );
        ...
        if (response.selectedCards && response.selectedCards.length > 0) {
          EventPacker.addMiddleware(
            { tag: this.Name, data: response.selectedCards },
            event
          );
          return true;
        }
  ...
  public async onEffect(
    ...
    if (
      ...
      const cardIdsChosen = EventPacker.getMiddleware<CardId[]>(
        this.Name,
        event
      );
      if (!cardIdsChosen) {
