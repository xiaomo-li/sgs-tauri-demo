...
export class MingShi extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const options = {
      [PlayerCardsArea.JudgeArea]: sourcePlayer.getCardIds(
        PlayerCardsArea.JudgeArea
      ),
      [PlayerCardsArea.EquipArea]: sourcePlayer.getCardIds(
        PlayerCardsArea.EquipArea
      ),
      [PlayerCardsArea.HandArea]: sourcePlayer.getCardIds(
    ...
    if (
      Sanguosha.getCardById(response.selectedCard!).isBlack() &&
      room.isCardInDropStack(response.selectedCard!)
    ) {
