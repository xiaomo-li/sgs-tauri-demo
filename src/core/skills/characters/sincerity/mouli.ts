      ...
      if (users && users.includes(owner.Id)) {
        if (users.length === 1) {
          ...
          if (
            room
              .getAlivePlayersFrom()
              .find((player) => player.getFlag<PlayerId[]>(MouLi.MouLiLi))
          ) {
            room.installSideEffectSkill(
              System.SideEffectSkillApplierEnum.MouLi,
              MouLiSide.Name,
              owner.Id
            );
          } else {
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      return (
        ...
        room
          ...
          .find((player) =>
            room
              .getFlag<PlayerId[]>(player.Id, MouLi.MouLiLi)
              ?.includes(owner.Id)
          ) !== undefined
      );
    }
      ...
      if (index !== -1) {
        ...
        if (targets.length === 1) {
          ...
          room
            ...
            .setFlag<PlayerId[]>(this.GeneralName, targets);
        }
      }
    } else {
      room.getPlayerById(fromId).removeFlag(this.GeneralName);
