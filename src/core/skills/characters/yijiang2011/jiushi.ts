...
export class JiuShiShadow extends TriggerSkill {
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.DamageEvent) {
      ...
      if (damageEvent.toId === owner.Id && !owner.isFaceUp()) {
        if (stage === DamageEffectStage.DamagedEffect) {
          EventPacker.addMiddleware(
            { tag: JiuShiShadow.faceDownTag, data: true },
            damageEvent
          );
        } else {
          return !!EventPacker.getMiddleware(
            JiuShiShadow.faceDownTag,
            damageEvent
          );
        }
      }
      ...
      const turnOverEvent =
        event as ServerEventFinder<GameEventIdentifiers.PlayerTurnOverEvent>;
      return (
        turnOverEvent.toId === owner.Id &&
        owner.getFlag<boolean>(JiuShi.levelUp)
      );
    }
  }
  ...
  public async obtainTrickRandomly(room: Room, fromId: PlayerId) {
    const pendingCardIds = room.findCardsByMatcherFrom(
    ...
    await room.moveCards({
      movingCards: [
        { card: pendingCardIds[randomIndex], fromArea: CardMoveArea.DrawStack },
      ],
      toId: fromId,
