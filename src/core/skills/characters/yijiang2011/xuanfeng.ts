...
export class XuanFeng extends TriggerSkill {
        ...
        const moveEvents =
          room.Analytics.getRecordEvents<GameEventIdentifiers.MoveCardEvent>(
            (event) =>
              ...
              event.infos.find(
                (info) =>
                  info.fromId === phaseStageChangeEvent.playerId &&
        ...
        moveCardEvent.infos.find(
          (info) =>
            ...
            info.movingCards.find(
              (card) => card.fromArea === CardMoveArea.EquipArea
            )
        ) !== undefined
      );
    }
  ...
  private async xuanFengDropCard(
    room: Room,
    fromId: PlayerId,
    toId: PlayerId
  ): Promise<void> {
  ...
  public async onEffect(
    ...
    const { fromId } = event;
    const targetIds = Precondition.exists(
      event.toIds,
      "Unable to get xuanfeng targets"
    );
    ...
    if (
      room.CurrentPlayer === room.getPlayerById(fromId) &&
      xuanfengTargets.length > 0
    ) {
