...
import { ActiveSkill, FilterSkill } from "../../skill";
...
export class JiYu extends ActiveSkill {
  ...
  public canUse(room: Room, owner: Player) {
    return !!owner
      ...
      .find((cardId) => owner.canUseCard(room, cardId));
  }
  ...
  public isAvailableTarget(
    ...
    return (
      room.getPlayerById(target).getCardIds(PlayerCardsArea.HandArea).length >
        0 &&
      !(
        room.getPlayerById(owner).getFlag<PlayerId[]>(this.Name) || []
      ).includes(target)
    );
  }
  ...
  public async onEffect(
    ...
    if (response.droppedCards) {
      const bannedSuits =
        room.getFlag<CardSuit[]>(event.fromId, JiYu.BannedSuits) || [];
      const suitDiscarded = Sanguosha.getCardById(
        response.droppedCards[0]
      ).Suit;
...
export class JiYuBlocker extends FilterSkill implements OnDefineReleaseTiming {
  ...
  public canUseCard(
    ...
    return (
      ...
      isCardResponse ||
      !(
        (room.getFlag<CardSuit[]>(owner, JiYu.BannedSuits) || []).includes(
