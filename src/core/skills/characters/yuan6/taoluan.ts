...
export class TaoLuan extends ViewAsSkill implements OnDefineReleaseTiming {
  ...
  public canViewAs(room: Room, owner: Player): string[] {
    ...
    return Sanguosha.getCardNameByType(
      ...
      (name) => !usedCards.includes(Sanguosha.getCardByName(name).GeneralName)
    );
  }
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    return (
      ...
      EventPacker.getMiddleware<PlayerId>(this.GeneralName, content) ===
        owner.Id
    );
  }
  ...
  public async onEffect(
    ...
    if (
      ...
      const response =
        await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
          GameEventIdentifiers.AskForChoosingPlayerEvent,
          {
            ...
            requiredAmount: 1,
            conversation:
              "taoluan: please choose another player to ask for a card",
      ...
      const typeExcept = [
        ...
        CardType.Trick,
        CardType.Equip,
      ].filter((type) => type !== card.BaseType);
      const resp =
      ...
      const usedCards =
        room.getFlag<string[]>(event.fromId, this.GeneralName) || [];
...
@PersistentSkill()
@CommonSkill({
  name: TaoLuanShadow.Name,
  description: TaoLuanShadow.Description,
})
export class TaoLuanClear
  extends TriggerSkill
  implements OnDefineReleaseTiming
{
  public afterLosingSkill(
  ...
  public canUse(
    ...
    return (
      ...
      owner.getFlag<boolean>(TaoLuan.Used) !== undefined
    );
  }
