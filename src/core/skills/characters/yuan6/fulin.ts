...
export class FuLin extends TriggerSkill implements OnDefineReleaseTiming {
  public async whenObtainingSkill(room: Room, player: Player) {
    ...
    const cardIdsGainedThisRound = room.Analytics.getCardObtainedRecord(
      player.Id,
      "round"
    ).reduce<CardId[]>((cardIds, moveCardEvent) => {
      for (const info of moveCardEvent.infos) {
        ...
        for (const cardInfo of info.movingCards) {
          cardIds.push(
            ...Algorithm.unique(
              VirtualCard.getActualCards([cardInfo.card]),
              cardIds
            )
          );
        }
  ...
  public canUse(
    ...
    return (
      ...
      !!content.infos.find(
        (info) =>
          info.toId === owner.Id && info.toArea === CardMoveArea.HandArea
      )
    );
  }
  ...
  public async onEffect(
    ...
    const cardIdsGained = (
      ...
      .filter(
        (info) =>
          info.toId === event.fromId && info.toArea === CardMoveArea.HandArea
      )
    ...
    room.setCardTag(
      event.fromId,
      this.Name,
      originalFuLinCardIds.concat(...cardIdsGained)
    );
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.AskForCardDropEvent) {
      ...
      const exceptCardIds = player
        ...
        .filter((cardId) =>
          player.getCardTag(this.GeneralName)!.includes(cardId)
        );
