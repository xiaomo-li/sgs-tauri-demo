...
export class WeiKui extends ActiveSkill {
  ...
  public async onEffect(
    ...
    if (
      handCards.find((id) => Sanguosha.getCardById(id).GeneralName === "jink")
    ) {
      ...
      room
        ...
        .canUseCardTo(room, virtualSlash, event.toIds[0], true) &&
        (await room.useCard(
          {
            fromId: event.fromId,
  ...
  implements OnDefineReleaseTiming
{
  ...
  public breakDistanceTo(room: Room, owner: Player, target: Player): number {
    return owner.getFlag<PlayerId[]>(this.GeneralName)?.includes(target.Id)
      ? 1
      : 0;
  }
}
