  ...
  public async onEffect(
    ...
    if (Sanguosha.getCardById(judgeEvent.judgeCardId).isBlack()) {
      ...
      const response =
        await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
          GameEventIdentifiers.AskForChoosingPlayerEvent,
          {
            ...
            requiredAmount: 1,
            conversation:
              "chouce: please choose a target to discard a card from his area",
      ...
      await room.dropCards(
        response.selectedPlayers[0] === fromId
            ...
            conversation: xianfuPlayer
              ? TranslationPack.translationJsonPatcher(
                  ...
                  TranslationPack.patchPlayerInTranslation(
                    room.getPlayerById(xianfuPlayer)
                  )
                ).extract()
              : TranslationPack.translationJsonPatcher(
                  "{0}: please choose a target to draw a card",
                  this.Name
                ).extract(),
            triggeredBySkills: [this.Name],
          },
      ...
      if (
        response.selectedPlayers[0] === xianfuPlayer &&
        !room.getFlag<boolean>(xianfuPlayer, XianFu.XianFuPlayer)
      ) {
