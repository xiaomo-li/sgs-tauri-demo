...
export class QingZhong extends TriggerSkill {
  ...
  public getSkillLog(): PatchedTranslationObject {
    return TranslationPack.translationJsonPatcher(
      "qingzhong {0}: do you want to draw 2 cards?",
      this.Name
    ).extract();
  }
  ...
  implements OnDefineReleaseTiming
{
  public afterLosingSkill(
    ...
    return (
      room.CurrentPlayerStage === PlayerPhaseStages.PlayCardStageEnd &&
      stage === PhaseStageChangeStage.StageChanged
    );
  }
  ...
  public canUse(
    ...
    return (
      owner.Id === content.playerId && owner.hasUsedSkill(this.GeneralName)
    );
  }
  ...
  public async onEffect(
    ...
    if (min <= handCards.length) {
      const targets = room
        .getOtherPlayers(event.fromId)
        .filter(
          (player) => player.getCardIds(PlayerCardsArea.HandArea).length === min
        );
      if (targets.length > 0) {
        ...
        if (targets.length > 1 || handCards.length === min) {
          ...
          toId =
            resp.selectedPlayers && resp.selectedPlayers.length > 0
              ? resp.selectedPlayers[0]
              : event.fromId;
        }

        if (toId !== event.fromId) {
          const firstCards = room
            .getPlayerById(event.fromId)
            .getCardIds(PlayerCardsArea.HandArea)
            .slice();
          const secondCards = room
            .getPlayerById(toId)
            .getCardIds(PlayerCardsArea.HandArea)
            .slice();
          await room.moveCards(
            {
              ...
              movingCards: firstCards.map((cardId) => ({
                card: cardId,
            ...
            {
              moveReason: CardMoveReason.PassiveMove,
              movingCards: secondCards.map((cardId) => ({
                card: cardId,
                fromArea: CardMoveArea.HandArea,
              })),
              fromId: toId,
