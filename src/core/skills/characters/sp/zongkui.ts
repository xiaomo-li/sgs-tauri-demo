...
export class ZongKui extends TriggerSkill {
  public isAutoTrigger(
    ...
    return (
      event !== undefined &&
      EventPacker.getIdentifier(event) === GameEventIdentifiers.CircleStartEvent
    );
  }
  ...
  public isTriggerable(
    ...
    return (
      ...
      stage === CircleStartStage.AfterCircleStarted
    );
  }
    ...
    if (identifier === GameEventIdentifiers.PhaseChangeEvent) {
      ...
      return (
        ...
        room
          ...
          .find((player) => player.getMark(MarkEnum.Kui) === 0) !== undefined
      );
    } else if (identifier === GameEventIdentifiers.CircleStartEvent) {
      const minimun = room
        .getOtherPlayers(owner.Id)
        .reduce<number>((min, player) => {
      ...
      return (
        room
          ...
          .find(
            (player) =>
              player.getMark(MarkEnum.Kui) === 0 && player.Hp === minimun
          ) !== undefined
      );
    }
        ...
        const resp =
          await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
            GameEventIdentifiers.AskForChoosingPlayerEvent,
            {
              ...
              requiredAmount: 1,
              conversation:
                "zongkui: please choose a target to gain 1 ‘Kui’ token",
