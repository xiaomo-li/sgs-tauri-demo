...
export class FuMan extends ActiveSkill {
  ...
  public async onEffect(
    ...
    const originalMappers =
      room.getFlag<FuManMapper[]>(fromId, FuMan.FuManMappers) || [];
    originalMappers[toIds[0]] = originalMappers[toIds[0]] || [];
    originalMappers[toIds[0]].includes(cardIds[0]) ||
      originalMappers[toIds[0]].push(cardIds[0]);
    room
      .getPlayerById(fromId)
      .setFlag<FuManMapper[]>(FuMan.FuManMappers, originalMappers);
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      if (phaseChangeEvent.fromPlayer === fromId) {
        ...
        const originalMappers = room.getFlag<FuManMapper[]>(
          fromId,
          FuMan.FuManMappers
        );
