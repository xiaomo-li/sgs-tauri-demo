...
export class TianSuan extends ActiveSkill implements OnDefineReleaseTiming {
  ...
  public async whenDead(room: Room, player: Player) {
    ...
    for (const target of targets) {
      ...
      if (
        !power.includes(TianSuan.TianSuanStricks[4]) &&
        room.getPlayerById(target).hasSkill(TianSuanDebuff.Name)
      ) {
        await room.loseSkill(target, TianSuanDebuff.Name);
      }
  ...
  public async onEffect(
    ...
    const originalPower =
      room.getFlag<string[]>(response.selectedPlayers[0], this.Name) || [];
    const originalTargets =
      room.getFlag<TianSuanMapper>(fromId, TianSuan.TianSuanTargets) || {};
    ...
    if (
      ...
      if (
        !room
          .getPlayerById(response.selectedPlayers[0])
          .hasSkill(TianSuanPower.Name)
      ) {
      ...
      if (
        (result === TianSuan.TianSuanStricks[0] ||
          result === TianSuan.TianSuanStricks[1]) &&
        ...
        const options: CardChoosingOptions = {
          [PlayerCardsArea.JudgeArea]: target.getCardIds(
            PlayerCardsArea.JudgeArea
          ),
          [PlayerCardsArea.EquipArea]: target.getCardIds(
        ...
        const resp =
          await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingCardFromPlayerEvent>(
        ...
        await room.moveCards({
          movingCards: [
            {
              card: resp.selectedCard,
              fromArea: target.cardFrom(resp.selectedCard),
            },
          ],
          fromId: response.selectedPlayers[0],
          toId: fromId,
    ...
    room
      ...
      .setFlag<TianSuanMapper>(TianSuan.TianSuanTargets, originalTargets);
...
@PersistentSkill()
@CommonSkill({
  name: "shadow_tiansuan",
  description: "shadow_tiansuan_description",
})
export class TianSuanPower
  ...
  implements OnDefineReleaseTiming
{
  public async whenDead(room: Room, player: Player) {
    ...
    player.hasSkill(TianSuanDebuff.Name) &&
      (await room.loseSkill(player.Id, TianSuanDebuff.Name));
  }
  ...
  public canUse(
    ...
    const canUse =
      ...
      !(
        stage === DamageEffectStage.DamagedEffect &&
        power.includes(TianSuan.TianSuanStricks[1]) &&
        content.damage < 2
      );
    if (canUse) {
...
@PersistentSkill()
@CommonSkill({
  name: TianSuanPower.Name,
  description: TianSuanPower.Description,
})
export class TianSuanDebuff extends FilterSkill {
  public canUseCard(
    ...
    if (
      !(room.getFlag<string[]>(owner, TianSuan.Name) || []).includes(
        TianSuan.TianSuanStricks[4]
      ) ||
      isCardResponse
    ) {
      return true;
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async onEffect(
    ...
    const { fromId } = event;
    const mappers = room.getFlag<TianSuanMapper>(
      fromId,
      TianSuan.TianSuanTargets
    );
    const targets = Object.keys(mappers);
