...
import { BaGuaZhenSkill, QiCai } from "../..";
...
export class LingLongShadow extends RulesBreakerSkill {
  ...
  public breakAdditionalCardHoldNumber(room: Room, owner: Player): number {
    return owner.getEquipment(CardType.DefenseRide) ||
      owner.getEquipment(CardType.OffenseRide)
      ? 0
      : 1;
  }
}
...
@ShadowSkill
@CompulsorySkill({
  name: LingLongShadow.Name,
  description: LingLongShadow.Description,
})
export class LingLongLose extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      ...
      (owner.getEquipment(CardType.Precious) !== undefined &&
  ...
  public async onEffect(
    ...
    if (!from.getEquipment(CardType.Precious) && !from.hasSkill(QiCai.Name)) {
      ...
      from.getEquipment(CardType.Precious) &&
      from.getInvisibleMark(this.GeneralName) > 0
    ) {
