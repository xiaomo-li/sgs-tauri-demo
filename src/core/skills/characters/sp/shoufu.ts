...
export class ShouFu extends ActiveSkill {
  ...
  public async onEffect(
    ...
    const targets = room
      ...
      .filter(
        (player) =>
          player.getCardIds(PlayerCardsArea.OutsideArea, this.Name).length === 0
      )
    ...
    if (handcards.length > 0 && targets.length > 0) {
      ...
      response.toIds = response.toIds || [
        targets[Math.floor(Math.random() * targets.length)],
      ];
      response.cardIds = response.cardIds || [
        handcards[Math.floor(Math.random() * handcards.length)],
      ];

      await room.moveCards({
        movingCards: [
          { card: response.cardIds[0], fromArea: CardMoveArea.HandArea },
        ],
        fromId,
      ...
      target.hasShadowSkill(ShouFuRemove.Name) ||
        (await room.obtainSkill(response.toIds[0], ShouFuRemove.Name));
      target.hasShadowSkill(ShouFuDebuff.Name) ||
        (await room.obtainSkill(response.toIds[0], ShouFuDebuff.Name));
    }
...
@CommonSkill({
  name: "choose_shoufu",
  description: "choose_shoufu_description",
})
  ...
  public isAvailableTarget(
    ...
    return (
      ...
      room
        ...
        .getCardIds(PlayerCardsArea.OutsideArea, ShouFu.Name).length === 0
    );
  }
...
@CommonSkill({
  name: "remove_shoufu",
  description: "remove_shoufu_description",
})
export class ShouFuRemove extends TriggerSkill {
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.DamageEvent) {
      ...
      return (
        ...
        owner.getCardIds(PlayerCardsArea.OutsideArea, ShouFu.Name).length > 0
      );
        ...
        moveCardEvent.infos.find(
          (info) =>
            ...
            info.movingCards.filter(
              (card) =>
                Sanguosha.getCardById(card.card).is(
                  Sanguosha.getCardById(Lu[0]).BaseType
                ) &&
  ...
  public async onEffect(
    ...
    from.hasShadowSkill(ShouFuDebuff.Name) &&
      (await room.loseSkill(fromId, ShouFuDebuff.Name));
...
@CommonSkill({
  name: "debuff_shoufu",
  description: "debuff_shoufu_description",
})
export class ShouFuDebuff extends FilterSkill {
  public canUseCard(
    ...
    const Lu = room
      .getPlayerById(owner)
      .getCardIds(PlayerCardsArea.OutsideArea, ShouFu.Name);
    if (Lu.length === 0) {
      return true;
    }
    ...
    return cardId instanceof CardMatcher
      ? !cardId.match(
          new CardMatcher({ type: [Sanguosha.getCardById(Lu[0]).BaseType] })
        )
