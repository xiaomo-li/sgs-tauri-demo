...
export class ZhouFu extends ActiveSkill {
  ...
  public isAvailableTarget(
    ...
    return (
      ...
      room
        ...
        .getCardIds(PlayerCardsArea.OutsideArea, this.Name).length === 0
    );
  }
  ...
  public async onEffect(
    ...
    await room.moveCards({
      movingCards: [
        { card: event.cardIds[0], fromArea: CardMoveArea.HandArea },
      ],
      fromId: event.fromId,
      toId: event.toIds[0],
...
@PersistentSkill()
@CommonSkill({
  name: "s_zhoufu_debuff",
  description: "s_zhoufu_debuff_description",
})
export class ZhouFuDebuff extends TriggerSkill {
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.PhaseChangeEvent) {
      ...
      return (
        phaseChangeEvent.to === PlayerPhase.PhaseFinish &&
        owner.getFlag<boolean>(this.Name) === true
      );
    } else if (identifier === GameEventIdentifiers.JudgeEvent) {
      const judgeEvent =
        event as ServerEventFinder<GameEventIdentifiers.JudgeEvent>;
      return (
        judgeEvent.toId === owner.Id &&
        owner.getCardIds(PlayerCardsArea.OutsideArea, ZhouFu.Name).length > 0
      );
        ...
        (
          ...
          (info) =>
            ...
            info.movingCards.find(
              (card) =>
                card.fromArea === CardMoveArea.OutsideArea &&
                owner.getOutsideAreaNameOf(card.card) === ZhouFu.Name
            )
        ) !== undefined
      );
    }
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.JudgeEvent) {
      const judgeEvent =
        unknownEvent as ServerEventFinder<GameEventIdentifiers.JudgeEvent>;
      ...
      room
        .getPlayerById(event.fromId)
        .getCardIds(PlayerCardsArea.OutsideArea, ZhouFu.Name).length === 0 &&
