...
export class BingZheng extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      ...
      room
        ...
        .find(
          (player) =>
            player.getCardIds(PlayerCardsArea.HandArea).length !== player.Hp
        ) !== undefined
    );
  }
  ...
  public async onEffect(
    ...
    if (to.Hp === to.getCardIds(PlayerCardsArea.HandArea).length) {
      ...
      if (
        fromId !== toIds[0] &&
        room.getPlayerById(fromId).getPlayerCards().length > 0
      ) {
        const response =
