...
export class ZhenWei extends TriggerSkill {
  ...
  public async beforeUse(
    ...
    if (
      !(
        !AimGroupUtil.getAllTargets(aimEvent.allTargets).includes(
          event.fromId
        ) &&
    ...
    response.selectedOption =
      response.selectedOption || options[options.length - 1];
  ...
  public async onEffect(
    ...
    if (chosen === this.ZhenWeiOptions[0]) {
      ...
      const originalTargets = TargetGroupUtil.getAllTargets(
...
@PersistentSkill()
@CommonSkill({
  name: "s_zhenwei_clear",
  description: "s_zhenwei_clear_description",
})
export class ZhenWeiClear
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async onEffect(
    ...
    const zheng = room
      ...
      .getCardIds(PlayerCardsArea.OutsideArea, ZhenWei.Name);
    zheng.length > 0 &&
      (await room.moveCards({
        movingCards: zheng.map((id) => ({
          card: id,
