...
export class FangTong extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const fang = room
      ...
      .getCardIds(PlayerCardsArea.OutsideArea, JiJun.Name);
    const response =
    ...
    response.selectedCards =
      response.selectedCards || fang[Math.floor(Math.random() * fang.length)];
    ...
    if (
      Sanguosha.getCardById(cardIds[0]).CardNumber +
        response.selectedCards.reduce<number>(
      ...
      const resp =
        await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
          GameEventIdentifiers.AskForChoosingPlayerEvent,
          {
            ...
            requiredAmount: 1,
            conversation:
              "fangtong: please choose a target to deal 3 thunder damage to him?",
            triggeredBySkills: [this.Name],
          },
          fromId,
          true
