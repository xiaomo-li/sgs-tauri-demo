...
export class ShiJian extends TriggerSkill implements OnDefineReleaseTiming {
  public async whenObtainingSkill(room: Room, owner: Player) {
    room.CurrentPlayerPhase === PlayerPhase.PlayCardStage &&
      ...
      owner.setFlag<number>(
        this.Name,
        room.Analytics.getCardUseRecord(room.CurrentPlayer.Id, "phase").length
      );
  }
  ...
  implements OnDefineReleaseTiming
{
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      return (
        (content as ServerEventFinder<GameEventIdentifiers.PhaseChangeEvent>)
          .from === PlayerPhase.PlayCardStage &&
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      let originTimes =
        room.getFlag<number>(event.fromId, this.GeneralName) || 0;
      originTimes++;
      room
        .getPlayerById(event.fromId)
        .setFlag<number>(this.GeneralName, originTimes);
      originTimes === 2 &&
...
@PersistentSkill()
@CommonSkill({
  name: "s_shijian_remover",
  description: "s_shijian_remover_description",
})
export class ShiJianRemover
  extends TriggerSkill
  implements OnDefineReleaseTiming
{
  public afterLosingSkill(
  ...
  public async onEffect(
    ...
    room.getPlayerById(event.fromId).hasSkill(YuXu.Name) &&
      (await room.loseSkill(event.fromId, YuXu.Name));
