...
export class ZiShu extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      content.infos.find(
        (info) =>
          !info.movedByReason?.includes(this.Name) &&
          info.toId === owner.Id &&
  ...
  public async onEffect(
    ...
    if (room.CurrentPlayer.Id === fromId) {
      ...
      if (moveCardEvent.infos.length === 1) {
        zishuCards.push(
          ...moveCardEvent.infos[0].movingCards.map((card) => card.card)
        );
      } else {
        const infos = moveCardEvent.infos.filter(
          (info) =>
            info.toId === fromId && info.toArea === CardMoveArea.HandArea
        );
...
export class ZiShuShadow extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public canUse(
    ...
    return (
      owner.getFlag<CardId[]>(this.GeneralName) &&
  ...
  public async onEffect(
    ...
    if (from.getCardIds(PlayerCardsArea.HandArea).length > 0) {
      ...
      const restCards = zishuCards.filter((cardId) =>
        from.getCardIds(PlayerCardsArea.HandArea).includes(cardId)
      );

      if (restCards.length > 0) {
        await room.moveCards({
          movingCards: restCards.map((card) => ({
            card,
