...
export class ChongZhen extends TriggerSkill {
  ...
  private findChongZhenTarget(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      if (card.GeneralName === "slash") {
        const targets = TargetGroupUtil.getRealTargets(
        ...
        if (
          ...
          EventPacker.getIdentifier(cardUseEvent.responseToEvent) !==
        ...
        const cardEffectEvent =
          cardUseEvent.responseToEvent as ServerEventFinder<GameEventIdentifiers.CardEffectEvent>;
        return cardEffectEvent.fromId;
      }
    } else if (identifier === GameEventIdentifiers.CardResponseEvent) {
      const cardResponseEvent =
        content as ServerEventFinder<GameEventIdentifiers.CardResponseEvent>;
      if (
        !cardResponseEvent.responseToEvent ||
        EventPacker.getIdentifier(cardResponseEvent.responseToEvent) !==
          GameEventIdentifiers.CardEffectEvent
      ) {
        return undefined;
      }
      ...
      if (
        Sanguosha.getCardById(cardEffectEvent.cardId).GeneralName === "duel"
      ) {
  ...
  public canUse(
    ...
    if (
      ChongZhen.LongDanName.find((name) =>
        virtualCard.findByGeneratedSkill(name)
      ) === undefined
    ) {
      return false;
    }
      ...
      const response =
        await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingCardFromPlayerEvent>(
          GameEventIdentifiers.AskForChoosingCardFromPlayerEvent,
          {
            ...
            toId: targetId,
            options: {
              [PlayerCardsArea.HandArea]: target.getCardIds(
