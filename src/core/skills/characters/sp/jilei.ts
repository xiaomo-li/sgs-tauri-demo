...
export class JiLei extends TriggerSkill {
  ...
  public async onEffect(
    ...
    if (!originalTypes.includes(type)) {
      ...
      room.getPlayerById(source).hasShadowSkill(JiLeiRemove.Name) ||
        (await room.obtainSkill(source, JiLeiRemove.Name));
    }
...
@PersistentSkill()
@CommonSkill({
  name: "s_jilei_blocker",
  description: "s_jilei_blocker_description",
})
export class JiLeiBlocker extends FilterSkill {
  public canUseCard(
    ...
    return cardId instanceof CardMatcher
      ? true
      : room.getPlayerById(owner).cardFrom(cardId) !==
...
@CommonSkill({
  name: "s_jilei_remove",
  description: "s_jilei_remove_description",
})
export class JiLeiRemove extends TriggerSkill implements OnDefineReleaseTiming {
  public async whenDead(room: Room, player: Player) {
    ...
    player.hasShadowSkill(JiLeiBlocker.Name) &&
      (await room.loseSkill(player.Id, JiLeiBlocker.Name));
  }
