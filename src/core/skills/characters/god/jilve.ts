...
import { PhaseChangeStage, PlayerPhase } from "../../../game/stage_processor";
...
export class JiLve extends ActiveSkill {
  ...
  public canUse(room: Room, owner: Player): boolean {
    return (
      (!owner.getFlag<boolean>(JiLve.ZhihengUsed) ||
        !owner.getFlag<boolean>(JiLve.WanshaUsed)) &&
  ...
  public async onEffect(
    ...
    if (selectedOption === ZhiHeng.Name) {
      ...
      if (
        selectedCards.filter((zhihengCard) => handCards.includes(zhihengCard))
...
export class JiLveJiZhi extends JiZhi {
  canUse(
    ...
    return (
      owner.getMark(MarkEnum.Ren) > 0 && super.canUse(room, owner, content)
    );
  }
