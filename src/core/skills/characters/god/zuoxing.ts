  ...
  public async whenLosingSkill(room: Room, player: Player) {
    if (player.getFlag<boolean>(this.Name)) {
      room.installSideEffectSkill(
        System.SideEffectSkillApplierEnum.ZuoXing,
        this.Name,
        player.Id
      );
    }
  ...
  public canUse(
    ...
    return (
      ...
      room
        .getAlivePlayersFrom()
        .find(
          (player) => player.Character.Name === "god_guojia" && player.MaxHp > 1
        ) !== undefined
    );
  }
  ...
  public async onEffect(
    ...
    if (godGuoJias.length > 1) {
      const resp =
        await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
          GameEventIdentifiers.AskForChoosingPlayerEvent,
          {
            ...
            requiredAmount: 1,
            conversation:
              "zuoxing: please choose a God Guo Jia to lose 1 max hp",
      ...
      resp.selectedPlayers = resp.selectedPlayers || [
        godGuoJias[Math.floor(Math.random() * godGuoJias.length)],
      ];
...
@CommonSkill({
  name: "side_zuoxing_s",
  description: "side_zuoxing_s_description",
})
