...
@CompulsorySkill({
  name: "liu_jieying",
  description: "liu_jieying_description",
})
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      ...
      return (
        ...
        room.getOtherPlayers(owner.Id).find((player) => !player.ChainLocked) !==
          undefined
      );
      ...
      return (
        chainLockedEvent.toId === owner.Id && chainLockedEvent.linked === false
      );
    }

    return (
      identifier === GameEventIdentifiers.GameBeginEvent && !owner.ChainLocked
    );
  }
