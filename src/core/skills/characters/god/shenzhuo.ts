  ...
  public async onEffect(
    ...
    if (response.selectedOption === options[0]) {
      ...
      room.setFlag<number>(
        event.fromId,
        this.Name,
        (room.getFlag<number>(event.fromId, this.Name) || 0) + 1
      );
    } else {
...
@PersistentSkill()
@CompulsorySkill({
  name: ShenZhuoExtra.Name,
  description: ShenZhuoExtra.Description,
})
export class ShenZhuoBlock
  extends FilterSkill
  implements OnDefineReleaseTiming
{
  public afterLosingSkill(
...
@PersistentSkill()
@CompulsorySkill({
  name: ShenZhuoBlock.Name,
  description: ShenZhuoBlock.Description,
})
export class ShenZhuoRemove
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    return (
      ...
      (owner.getFlag<number>(this.GeneralName) !== undefined ||
        owner.getFlag<boolean>(ShenZhuo.Block) !== undefined)
    );
  }
  ...
  public async onEffect(
    ...
    room.getFlag<boolean>(event.fromId, ShenZhuo.Block) !== undefined &&
      room.removeFlag(event.fromId, ShenZhuo.Block);
