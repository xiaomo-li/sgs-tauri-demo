...
export class PoWei extends TriggerSkill {
      ...
      return (
        ...
        ((current.getMark(MarkEnum.Wei) > 0 &&
          (owner
            ...
            .find((id) => room.canDropCard(owner.Id, id)) !== undefined ||
            (current !== owner &&
              current.Hp <= owner.Hp &&
    ...
    if (
      ...
      if (room.getMark(phaseChangeEvent.toPlayer, MarkEnum.Wei) > 0) {
        ...
        room
          ...
          options.push("powei:dropCard");

        const current = room.getPlayerById(phaseChangeEvent.toPlayer);
        if (
          current.Hp <= room.getPlayerById(fromId).Hp &&
          current.getCardIds(PlayerCardsArea.HandArea).length > 0
        ) {
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.GameBeginEvent) {
      ...
      const phaseChangeEvent =
        unknownEvent as ServerEventFinder<GameEventIdentifiers.PhaseChangeEvent>;
      const selectedOption = EventPacker.getMiddleware<string | undefined>(
        this.Name,
        event
      );
      ...
      for (const player of room.getAlivePlayersFrom()) {
        player.getMark(MarkEnum.Wei) > 0 &&
          room.removeMark(player.Id, MarkEnum.Wei);
      }
...
export class PoWeiRemove extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public canUse(
    ...
    return (
      ...
      owner.getFlag<PlayerId>(PoWei.Debuff) !== undefined
    );
  }
