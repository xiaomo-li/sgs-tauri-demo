...
export class JiuFa extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      ...
      (!owner.getFlag<string[]>(this.Name) ||
        !owner
          .getFlag<string[]>(this.Name)
          .includes(Sanguosha.getCardById(content.cardId).GeneralName))
    );
  }
  ...
  isAutoTrigger(
    ...
    return (
      !owner.getFlag<string[]>(this.Name) ||
      owner.getFlag<string[]>(this.Name).length < 8
    );
  }
        ...
        displayCardIds
          .filter(
            (cardId, _, allCardIds) =>
              allCardIds.filter(
                (id) =>
                  Sanguosha.getCardById(id).CardNumber ===
