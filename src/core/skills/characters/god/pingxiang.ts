...
export class PingXiang extends ActiveSkill {
  ...
  public async onEffect(
    ...
    let slashCount = 9;
    const virtualFireSlash = VirtualCard.create({
      cardName: "fire_slash",
      bySkill: this.Name,
    });
    while (slashCount-- > 0) {
      const availableTargets = room
        ...
        .filter(
          (player) =>
            room.canAttack(from, player, virtualFireSlash.Id) &&
            room.withinAttackDistance(from, player)
        );
...
@ShadowSkill
@CommonSkill({
  name: PingXiang.GeneralName,
  description: PingXiang.Description,
})
