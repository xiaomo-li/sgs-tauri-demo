  ...
  public async onEffect(
    ...
    if (!from.Dead) {
      ...
      await room.obtainSkill(
        fromId,
        judge ? FireLongNu.Name : ThunderLongNu.Name
      );
...
@CompulsorySkill({
  name: "fire_longnu",
  description: "fire_longnu_description",
})
export class FireLongNu
...
@CompulsorySkill({
  name: "thunder_longnu",
  description: "thunder_longnu_description",
})
export class ThunderLongNu
  extends TransformSkill
  implements OnDefineReleaseTiming
{
  async whenObtainingSkill(room: Room, owner: Player) {
...
@PersistentSkill()
@CompulsorySkill({
  name: LongNuShaodw.Name,
  description: LongNuShaodw.Description,
})
