...
import { AllStage, SkillUseStage } from "../../../game/stage_processor";
import { Player } from "../../../player/player";
import { Room } from "../../../room/room";
import { FlagEnum } from "../../../shares/types/flag_list";
import { ActiveSkill, Skill, TriggerSkill } from "../../skill";
import {
  CommonSkill,
  LimitSkill,
  PersistentSkill,
...
@LimitSkill({
  name: "god_huishi_sec",
  description: "god_huishi_sec_description",
})
export class GodHuiShiSec extends ActiveSkill {
  ...
  public async onEffect(
    ...
    if (
      room.getPlayerById(fromId).MaxHp >= room.AlivePlayers.length &&
      ...
      const originalSkillNames =
        to.getFlag<string[]>(FlagEnum.EnableToAwaken) || [];
      if (!originalSkillNames.includes(response.selectedOption)) {
        ...
        to.hasShadowSkill(GodHuiShiSecRemover.Name) ||
          (await room.obtainSkill(toIds[0], GodHuiShiSecRemover.Name));
      }
    } else {
...
@PersistentSkill()
@CommonSkill({
  name: "s_god_huishi_sec_remover",
  description: "s_god_huishi_sec_remover_description",
})
export class GodHuiShiSecRemover extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      ...
      owner
        .getFlag<string[]>(FlagEnum.EnableToAwaken)
        ?.includes(content.skillName)
    );
  }
  ...
  public async onEffect(
    ...
    const content =
      event.triggeredOnEvent as ServerEventFinder<GameEventIdentifiers.SkillUseEvent>;

    const originalSkillNames =
      room.getFlag<string[]>(content.fromId, FlagEnum.EnableToAwaken) || [];
    if (originalSkillNames.includes(content.skillName)) {
      const index = originalSkillNames.findIndex(
        (name) => name === content.skillName
      );
