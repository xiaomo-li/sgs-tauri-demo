...
export class ZhiTi extends TriggerSkill {
  public isTriggerable(
    event: ServerEventFinder<
      GameEventIdentifiers.DamageEvent | GameEventIdentifiers.PinDianEvent
    >,
    stage?: AllStage
  ): boolean {
    return (
    ...
    if (identifier === GameEventIdentifiers.DamageEvent) {
      ...
      return (
        ((damageEvent.fromId === owner.Id &&
          damageEvent.cardIds &&
          Sanguosha.getCardById(damageEvent.cardIds[0]).GeneralName ===
            "duel") ||
      ...
      const pindianEvent =
        content as ServerEventFinder<GameEventIdentifiers.PinDianEvent>;
  ...
  public async onEffect(
    ...
    resp.selectedOption =
      resp.selectedOption ||
      room.getPlayerById(fromId).DisabledEquipSections[0];

    await room.resumePlayerEquipSections(
      fromId,
      resp.selectedOption as CharacterEquipSections
    );
...
export class ZhiTiShadow extends GlobalRulesBreakerSkill {
  public breakAdditionalCardHold(
    ...
    return room.withinAttackDistance(owner, target) && target.LostHp > 0
      ? -1
      : 0;
  }
}
