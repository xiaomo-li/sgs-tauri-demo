...
export class WangGui extends TriggerSkill {
  ...
  public async beforeUse(
    ...
    if (
      room.getFlag<AllStage>(event.fromId, this.WangGuiStage) ===
      ...
      targets = room
        ...
        .filter(
          (player) =>
            player.Nationality !== room.getPlayerById(event.fromId).Nationality
        )
