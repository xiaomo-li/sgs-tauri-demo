...
export class YiJiao extends ActiveSkill {
  ...
  public isAvailableTarget(
    ...
    return (
      target !== owner && room.getPlayerById(target).getMark(MarkEnum.Yi) === 0
    );
  }
  ...
  public async onEffect(
    ...
    room.addMark(
      event.toIds[0],
      MarkEnum.Yi,
      parseInt(response.selectedOption, 10) * 10
    );
    room
      .getPlayerById(event.toIds[0])
      .setFlag<PlayerId>(this.Name, event.fromId);
...
@PersistentSkill()
@CommonSkill({
  name: "s_yijiao_handler",
  description: "s_yijiao_handler_description",
})
export class YiJiaoHandler
  ...
  implements OnDefineReleaseTiming
{
  public async whenDead(room: Room, player: Player) {
    ...
    player.getFlag<number>(this.Name) !== undefined &&
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      from.getFlag<number>(this.Name) !== undefined &&
