        ...
        ...infos[0].movingCards
          .filter(
            (cardInfo) =>
              !cardInfo.asideMove &&
              room
                .getCardTag(infos[0].fromId!, this.Name)!
                .includes(cardInfo.card) &&
              (cardInfo.fromArea === CardMoveArea.HandArea ||
                cardInfo.fromArea === CardMoveArea.EquipArea) &&
        ...
        if (
          ...
          info.movingCards.find(
            (cardInfo) =>
              !cardInfo.asideMove &&
              room
                .getCardTag(info.fromId!, this.Name)!
                .includes(cardInfo.card) &&
              (cardInfo.fromArea === CardMoveArea.HandArea ||
                cardInfo.fromArea === CardMoveArea.EquipArea)
          )
    ...
    return !!content.infos.find(
      (info) =>
        ...
        info.movingCards.find(
          (cardInfo) =>
            !room
              .getCardTag(info.toId!, this.GeneralName)
              ?.includes(cardInfo.card)
        )
    );
  }
  ...
  public async onEffect(
    ...
    for (const info of (
      ...
      if (
        ...
        const cardIdsRecorded =
          room.getCardTag(info.toId, this.GeneralName) || [];
        ...
        singletonCardIds > cardIdsRecorded &&
          room.setCardTag(info.toId, this.GeneralName, singletonCardIds);
      }
    }
...
@ShadowSkill
@CompulsorySkill({
  name: LiuZhuanShadow.Name,
  description: LiuZhuanShadow.Description,
})
...
@PersistentSkill()
@CommonSkill({
  name: LiuZhuanProhibited.Name,
  description: LiuZhuanProhibited.Description,
})
