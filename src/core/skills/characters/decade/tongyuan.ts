...
@CommonSkill({
  name: "s_tongyuan_buff",
  description: "s_tongyuan_buff_description",
})
export class TongYuanBuff extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      ...
      (stage === CardUseStage.BeforeCardUseEffect
        ...
        : cardUsed.is(CardType.Basic) &&
          TargetGroupUtil.getRealTargets(content.targetGroup).length > 0)
    );
  }
  ...
  public async beforeUse(
    ...
    if (Sanguosha.getCardById(cardUseEvent.cardId).is(CardType.Basic)) {
      ...
      if (availableTarget) {
        const exclude = TargetGroupUtil.getRealTargets(
        ...
        if (
          ...
          exclude.push(
            ...room
              ...
              .filter(
                (player) =>
                  !room.canAttack(room.getPlayerById(event.fromId), player)
              )
              .map((player) => player.Id)
          );
        }
