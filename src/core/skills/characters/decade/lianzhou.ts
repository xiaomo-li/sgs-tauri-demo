...
export class LianZhou extends TriggerSkill {
  ...
  public async onEffect(
    ...
    room.getPlayerById(event.fromId).ChainLocked ||
      (await room.chainedOn(event.fromId));

    const targets = room.AlivePlayers.filter(
      (player) =>
        player.Hp === room.getPlayerById(event.fromId).Hp && !player.ChainLocked
    ...
    if (targets.length > 0) {
      const response =
        await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
          GameEventIdentifiers.AskForChoosingPlayerEvent,
          {
            ...
            conversation:
              "lianzhou: do you want to choose targets to chain on?",
      ...
      if (response.selectedPlayers && response.selectedPlayers.length > 0) {
        for (const player of response.selectedPlayers) {
          room.getPlayerById(player).ChainLocked ||
            (await room.chainedOn(player));
        }
      }
    }
