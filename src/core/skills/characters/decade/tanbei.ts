...
export class TanBei extends ActiveSkill {
  ...
  public async onEffect(
    ...
    if (response.selectedOption === "tanbei:prey") {
      ...
      const randomCard =
        wholeCards[Math.floor(Math.random() * wholeCards.length)];
      await room.moveCards({
        movingCards: [
          {
            card: randomCard,
            fromArea: room.getPlayerById(fromId).cardFrom(randomCard),
          },
        ],
        fromId: toIds[0],
        toId: fromId,
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUseCardTo(
    ...
    return !(
      owner === from && to.Id === owner.getFlag<PlayerId>(this.GeneralName)
    );
  }
}
