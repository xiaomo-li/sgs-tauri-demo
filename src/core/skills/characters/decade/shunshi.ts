...
export class ShunShi extends TriggerSkill {
    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      ...
      const canUse =
        ...
        room
          ...
          .find((player) => player.Id !== damageEvent.fromId) !== undefined;
  ...
  public isAvailableTarget(owner: string, room: Room, target: string): boolean {
    return (
      target !== owner && target !== room.getFlag<PlayerId>(owner, this.Name)
    );
  }
  ...
  public getSkillLog(
    ...
    return owner.getFlag<PlayerId>(this.Name)
      ...
      : TranslationPack.translationJsonPatcher(
          "{0}: do you want to give another player a card?",
          this.Name
        ).extract();
  }
      ...
      movingCards: [
        {
          ...
          fromArea: room.getPlayerById(fromId).cardFrom(event.cardIds[0]),
        },
      ],
      fromId,
      toId: event.toIds[0],
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.DrawCardEvent) {
      const drawCardEvent =
        unkownEvent as ServerEventFinder<GameEventIdentifiers.DrawCardEvent>;
      drawCardEvent.drawAmount += room.getFlag<number>(
        event.fromId,
        ShunShi.ShunShiBuff
      );
    } else {
...
@PersistentSkill()
@CommonSkill({
  name: ShunShiShadow.Name,
  description: ShunShiShadow.Description,
})
export class ShunShiBuff
