    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      ...
      return (
        ...
        ((phaseStageChangeEvent.toStage ===
          ...
          owner.getCardIds(PlayerCardsArea.OutsideArea, this.Name).length ===
            0 &&
          ...
          (info) =>
            ...
            info.movingCards.find(
              (cardInfo) =>
                ...
                Sanguosha.getCardById(cardInfo.card).Suit ===
                  Sanguosha.getCardById(biao[0]).Suit &&
                Sanguosha.getCardById(cardInfo.card).CardNumber ===
                  Sanguosha.getCardById(biao[0]).CardNumber
            )
        ) !== undefined
      );
    }
    ...
    if (identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      if (
        ...
        await room.moveCards({
          movingCards: [
            {
              card: event.cardIds[0],
              fromArea: room
                .getPlayerById(event.fromId)
                .cardFrom(event.cardIds[0]),
            },
          ],
          fromId: event.fromId,
          toId: event.fromId,
          ...
          movingCards: [
            {
              card: room
                .getPlayerById(event.fromId)
                .getCardIds(PlayerCardsArea.OutsideArea, this.Name)[0],
        ...
        const diff =
          most -
          room
            .getPlayerById(resp.selectedPlayers[0])
