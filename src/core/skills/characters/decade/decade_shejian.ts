...
@CommonSkill({
  name: "decade_shejian",
  description: "decade_shejian_description",
})
      ...
      if (toDiscard.length > event.cardIds.length) {
        const resp = await room.doAskForCommonly(
          GameEventIdentifiers.AskForChoosingCardWithConditionsEvent,
          {
            toId: event.fromId,
            customCardFields: {
              [PlayerCardsArea.EquipArea]: user.getCardIds(
                PlayerCardsArea.EquipArea
              ),
              [PlayerCardsArea.HandArea]: user.getCardIds(
        ...
        if (
          (resp.selectedCards || []).length +
            (resp.selectedCardsIndex || []).length <
          event.cardIds.length
        ) {
          toDiscard = Algorithm.randomPick(
            event.cardIds.length,
            user.getPlayerCards()
          );
        } else {
          ...
          toDiscard.push(
            ...Algorithm.randomPick(
              event.cardIds.length - toDiscard.length,
              user.getCardIds(PlayerCardsArea.HandArea)
            )
          );
        }
      }
