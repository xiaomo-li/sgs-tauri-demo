...
export class XuHe extends TriggerSkill {
      ...
      (content.toStage === PlayerPhaseStages.PlayCardStageStart ||
        (content.toStage === PlayerPhaseStages.PlayCardStageEnd &&
          !room
            ...
            .find((player) => player.MaxHp > owner.MaxHp)))
    );
  }
  ...
  public async beforeUse(
    ...
    if (room.CurrentPlayerStage === PlayerPhaseStages.PlayCardStageStart) {
      const targets = room
        ...
        .filter(
          (player) =>
            room.distanceBetween(room.getPlayerById(fromId), player) <= 1
        );
  ...
  public async onEffect(
    ...
    if (
      room.CurrentPlayerStage === PlayerPhaseStages.PlayCardStageStart &&
      toIds
    ) {
