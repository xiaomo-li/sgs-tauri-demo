  ...
  public async onEffect(
    ...
    if (
      to
        ...
        .find((id) => Sanguosha.getCardById(id).GeneralName === "jink")
    ) {
      const toGain = to
        .getCardIds(PlayerCardsArea.HandArea)
        .filter((id) => Sanguosha.getCardById(id).GeneralName === "jink");
      to.getEquipment(CardType.Shield) &&
        toGain.push(to.getEquipment(CardType.Shield)!);

      await room.moveCards({
        movingCards: toGain.map((card) => ({
          card,
          fromArea:
            card === to.getEquipment(CardType.Shield)
      ...
      if (selectedCards.length > toGain.length) {
        ...
        selectedCards =
          resp.selectedCards.length > 0
            ? resp.selectedCards
            : Algorithm.randomPick(toGain.length, selectedCards);
      }
