...
export class HuGuan extends TriggerSkill {
  ...
  public async whenObtainingSkill(room: Room, owner: Player) {
    ...
    const records = room.Analytics.getCardUseRecord(
      owner.Id,
      "phase",
      undefined,
      1
    ...
    if (records.length > 0) {
      owner.getFlag<boolean>(HuGuanShadow.Name) ||
        owner.setFlag<boolean>(HuGuanShadow.Name, true);
  ...
  public async onEffect(
    ...
    const suit = Functional.convertSuitStringToSuit(
    ...
    room.setFlag<CardSuit[]>(
      ...
      TranslationPack.translationJsonPatcher(
        "huguan suits: {0}",
        originalSuits.reduce<string>(
          (suitString, suit) =>
            suitString + Functional.getCardSuitCharText(suit),
          ""
        )
      ).toString()
    );

    room.getPlayerById(user).hasShadowSkill(HuGuanBuff.Name) ||
      (await room.obtainSkill(user, HuGuanBuff.Name));
...
@CommonSkill({
  name: "s_huguan_buff",
  description: "s_huguan_buff_description",
})
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.AskForCardDropEvent) {
      ...
      const tricks = player
        ...
        .filter((cardId) =>
          player
            .getFlag<CardSuit[]>(HuGuan.Name)
            .includes(Sanguosha.getCardById(cardId).Suit)
        );

      if (tricks.length > 0) {
        const otherHandCards = player
          .getCardIds(PlayerCardsArea.HandArea)
          .filter((card) => !tricks.includes(card));
        const discardAmount =
          otherHandCards.length - player.getMaxCardHold(room);
