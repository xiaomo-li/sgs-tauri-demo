...
export class ZhenGe extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const targets =
      room.getFlag<PlayerId[]>(fromId, ZhenGe.ZhenGeTargets) || [];
    targets.includes(toIds[0]) || targets.push(toIds[0]);
    room
      .getPlayerById(fromId)
      .setFlag<PlayerId[]>(ZhenGe.ZhenGeTargets, targets);
    ...
    if (
      !room
        ...
        .find(
          (player) =>
            !room.withinAttackDistance(room.getPlayerById(toIds[0]), player)
        )
    ) {
      const targets = room
        .getOtherPlayers(toIds[0])
