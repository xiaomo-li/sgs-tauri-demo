    ...
    if (typeNum > 0) {
      ...
      slashs.length > 0 &&
        (await room.moveCards({
          movingCards: [
            {
              card: slashs[Math.floor(Math.random() * slashs.length)],
...
export class GuoWuShadow extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      return (
        ...
        (owner.hasShadowSkill(GuoWuUnlimited.Name) ||
          owner.getFlag<boolean>(this.GeneralName))
      );
    }
  ...
  public async beforeUse(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      const allTargets = TargetGroupUtil.getAllTargets(
...
@PersistentSkill()
@CommonSkill({
  name: "s_guowu_unlimited",
  description: "s_guowu_unlimited_description",
})
