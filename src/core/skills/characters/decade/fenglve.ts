...
export class FengLve extends ActiveSkill {
  ...
  public async onEffect(
    ...
    if (pindianRecord[0].winner === fromId) {
      ...
      pindianRecord[0].winner === toIds[0] &&
      pindianCardId &&
      room.isCardInDropStack(pindianCardId)
    ) {
      await room.moveCards({
        movingCards: [
          { card: pindianCardId, fromArea: CardMoveArea.DropStack },
        ],
        toId: toIds[0],
        toArea: CardMoveArea.HandArea,
