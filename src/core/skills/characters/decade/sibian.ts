...
export class SiBian extends TriggerSkill {
  ...
  public async onEffect(
    ...
    toGain.push(
      ...displayCards.filter(
        (id) => Sanguosha.getCardById(id).CardNumber === max
      )
    );
    min !== max &&
      toGain.push(
        ...displayCards.filter(
          (id) => Sanguosha.getCardById(id).CardNumber === min
        )
      );
    ...
    if (leftCards.length > 0) {
      if (
        leftCards.length === 2 &&
        Math.abs(
          Sanguosha.getCardById(leftCards[0]).CardNumber -
            Sanguosha.getCardById(leftCards[1]).CardNumber
        ...
        const players = room.AlivePlayers.filter(
          (player) =>
            player.getCardIds(PlayerCardsArea.HandArea).length === minimun
