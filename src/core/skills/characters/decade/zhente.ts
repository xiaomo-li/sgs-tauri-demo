  ...
  public async onEffect(
      ...
      await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingOptionsEvent>(
        GameEventIdentifiers.AskForChoosingOptionsEvent,
        {
          ...
          conversation: TranslationPack.translationJsonPatcher(
            ...
            this.Name,
            Functional.getCardColorRawText(
              Sanguosha.getCardById(aimEvent.byCardId).Color
            ),
    ...
    if (response.selectedOption === options[0]) {
      ...
      for (const skillName of [ZhenTeBlocker.Name, ZhenTeRemover.Name]) {
        room.getPlayerById(user).hasShadowSkill(skillName) ||
          (await room.obtainSkill(user, skillName));
      }
    } else {
...
@PersistentSkill()
@CommonSkill({
  name: "s_zhente_blocker",
  description: "s_zhente_blocker_description",
})
export class ZhenTeBlocker extends FilterSkill {
  public canUseCard(
    ...
    if (cardId instanceof CardMatcher) {
      const suits = colors.includes(CardColor.Black)
        ? [CardSuit.Spade, CardSuit.Club]
        : [];
...
@PersistentSkill()
@CommonSkill({
  name: "s_zhente_remover",
  description: "s_zhente_remover_description",
})
export class ZhenTeRemover
  ...
  implements OnDefineReleaseTiming
{
  public async whenDead(room: Room, player: Player) {
    ...
    player.hasShadowSkill(ZhenTeBlocker.Name) &&
      (await room.loseSkill(player.Id, ZhenTeBlocker.Name));
  }
  ...
  public canUse(
    ...
    return (
      ...
      owner.getFlag<CardColor[]>(ZhenTe.Name) !== undefined
    );
  }
