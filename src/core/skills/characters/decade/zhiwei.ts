...
export class ZhiWei extends TriggerSkill {
  ...
  public isAutoTrigger(
    ...
    return (
      !!event &&
      EventPacker.getIdentifier(event) === GameEventIdentifiers.GameBeginEvent
    );
  }
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.PhaseChangeEvent) {
      ...
      return (
        ...
        (!owner.getFlag<PlayerId>(this.Name) ||
          room.getPlayerById(owner.getFlag<PlayerId>(this.Name)).Dead)
      );
    }
  ...
  public async onEffect(
      ...
      const response =
        await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
          GameEventIdentifiers.AskForChoosingPlayerEvent,
          {
            ...
            requiredAmount: 1,
            conversation:
              "zhiwei: please choose another player to be your ‘Zhi Wei’ player",
    ...
    room.setFlag<boolean>(toId, ZhiWei.ZhiWeiPlayer, false, this.Name, [
      event.fromId,
    ]);
      ...
      const canUse =
        (stage === DamageEffectStage.AfterDamageEffect &&
          (content as ServerEventFinder<GameEventIdentifiers.DamageEvent>)
            .fromId === owner.getFlag<PlayerId>(this.GeneralName)) ||
        (stage === DamageEffectStage.AfterDamagedEffect &&
          (content as ServerEventFinder<GameEventIdentifiers.DamageEvent>)
            .toId === owner.getFlag<PlayerId>(this.GeneralName));
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.DamageEvent) {
      ...
      await room.moveCards({
        movingCards: availableIds.map((card) => ({
          card,
