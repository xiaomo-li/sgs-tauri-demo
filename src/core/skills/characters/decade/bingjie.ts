  ...
  implements OnDefineReleaseTiming
{
  ...
  public isTriggerable(
    ...
    return (
      stage === AimStage.AfterAim || stage === PhaseChangeStage.PhaseChanged
    );
  }
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.AimEvent) {
      ...
      return (
        ...
        !!AimGroupUtil.getAllTargets(aimEvent.allTargets).find(
          (playerId) => playerId !== owner.Id
        )
      );
  ...
  public async onEffect(
    ...
    if (
      ...
      for (const toId of allTargets) {
        if (
          toId === event.fromId ||
          room.getPlayerById(toId).getPlayerCards().length === 0
        ) {
          continue;
        }
