...
export class YeChou extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      ...
      room.getOtherPlayers(owner.Id).find((player) => player.LostHp > 1) !==
        undefined
    );
  }
...
@PersistentSkill()
@CommonSkill({
  name: "s_yechou_debuff",
  description: "s_yechou_debuff_description",
})
export class YeChouDebuff
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async onEffect(
    ...
    if (
      ...
      phaseChangeEvent.toPlayer === event.fromId
    ) {
