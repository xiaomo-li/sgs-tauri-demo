...
export class ShiCai extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      const cardIds = card.isVirtualCard()
        ? (card as VirtualCard).getRealActualCards()
        : [cardUseEvent.cardId];
      return (
        ...
        EventPacker.getMiddleware<boolean>(
          ShiCai.ShiCaiTypeUsed,
          cardUseEvent
        ) === true
      );
      ...
      return (
        ...
        EventPacker.getMiddleware<boolean>(ShiCai.ShiCaiTypeUsed, aimEvent) ===
          true
      );
    }
  ...
  public async onEffect(
    ...
    if (cardIds.length > 1) {
      const { top } =
        await room.doAskForCommonly<GameEventIdentifiers.AskForPlaceCardsInDileEvent>(
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      return (
        cardUseEvent.fromId === owner.Id &&
        !shicaiUsed.includes(
          Sanguosha.getCardById(cardUseEvent.cardId).BaseType
        )
      );
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      from.setFlag<CardType[]>(this.GeneralName, [
        ...shicaiUsed,
        Sanguosha.getCardById(cardUseEvent.cardId).BaseType,
      ]);
      EventPacker.addMiddleware(
        { tag: ShiCai.ShiCaiTypeUsed, data: true },
        cardUseEvent
      );
    } else {
