...
export class KuiZhu extends TriggerSkill {
  ...
  public canUse(
    ...
    if (canUse) {
      const droppedCardsNum = room.Analytics.getCardLostRecord(
        owner.Id,
        "phase"
        ...
        if (event.infos.length === 1) {
          ...
          const infos = event.infos.filter(
            (info) => info.moveReason === CardMoveReason.SelfDrop
          );
    ...
    if (selectedOption) {
      ...
      if (response.toIds) {
        EventPacker.addMiddleware(
          { tag: this.Name, data: selectedOption },
          event
        );
...
export class KuiZhuDraw extends TriggerSkill {
  ...
  public targetFilter(room: Room, owner: Player, targets: PlayerId[]): boolean {
    return (
      targets.length > 0 &&
      targets.length <= owner.getFlag<number>(KuiZhu.KuiZhuDroppedNum)
    );
  }
...
@CommonSkill({
  name: "damage_kuizhu",
  description: "damage_kuizhu_description",
})
  ...
  public targetFilter(room: Room, owner: Player, targets: PlayerId[]): boolean {
    return (
      targets.reduce<number>(
        (sum, target) => sum + room.getPlayerById(target).Hp,
        0
