...
import { SwitchSkill } from "../../skill_wrappers";
import {
...
export class JuZhanYin extends TriggerSkill {
  ...
  public canUse(
    ...
    const canUse =
      ...
      allTargets.find(
        (playerId) => room.getPlayerById(playerId).getPlayerCards().length > 0
      ) !== undefined;
...
export class JuZhanShadow
  extends GlobalFilterSkill
  implements OnDefineReleaseTiming
{
  ...
  public canUseCardTo(
    ...
    return (
      !(fromTargets && fromTargets.includes(to.Id)) &&
      !(toTargets && toTargets.includes(from.Id))
    );
  }
}
