  ...
  public async onEffect(
    ...
    if (resp.selectedPlayers && resp.selectedPlayers.length > 0) {
      const originalPlayers =
        room.getFlag<PlayerId[]>(resp.selectedPlayers[0], this.Name) || [];
      ...
      let selected: string = "";
      if (
        target.getCardIds(PlayerCardsArea.EquipArea).length >
          from.getCardIds(PlayerCardsArea.EquipArea).length &&
          ...
          movingCards: [
            {
              ...
              fromArea: target.cardFrom(response.selectedCards[0]),
            },
          ],
