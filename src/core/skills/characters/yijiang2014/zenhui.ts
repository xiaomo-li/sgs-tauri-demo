...
export class ZenHui extends TriggerSkill {
      ...
      room
        ...
        .find(
          (playerId) =>
            ...
            (
              Sanguosha.getCardById(event.byCardId)
  ...
  public getAnimationSteps(
    ...
    if (event.toIds!.length > 1) {
      step.push({
        from: event.toIds![0],
        tos: event.toIds!.slice(1, event.toIds!.length),
      });
    }
  ...
  public async onEffect(
    ...
    let option2 = true;
    if (
      room.getPlayerById(fromId).getCardIds(PlayerCardsArea.HandArea).length > 0
    ) {
      const { selectedCards } =
          ...
          movingCards: [
            {
              ...
              fromArea: room.getPlayerById(toIds[0]).cardFrom(selectedCards[0]),
            },
          ],
          fromId: toIds[0],
          toId: fromId,
