...
export class ZhongYong extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      ...
      ((VirtualCard.getActualCards([content.cardId]).length > 0 &&
        ...
        (content.cardIdsResponded !== undefined &&
          content.cardIdsResponded.find(
            (id) =>
              ...
              VirtualCard.getActualCards([id]).find(
                (cardId) => !room.isCardInDropStack(cardId)
              ) === undefined
          ) !== undefined))
    );
  }
  ...
  public async beforeUse(
    ...
    room.isCardOnProcessing(cardUseEvent.cardId) &&
      allCards.push(...VirtualCard.getActualCards([cardUseEvent.cardId]));
    ...
    const response =
      await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
        GameEventIdentifiers.AskForChoosingPlayerEvent,
        {
          ...
          requiredAmount: 1,
          conversation:
            "zhongyong: do you want to choose a target to gain these cards?",
  ...
  public async onEffect(
    ...
    await room.moveCards({
      movingCards: cardIds.map((card) => ({
        card,
        fromArea: room.isCardOnProcessing(card)
    ...
    if (
      VirtualCard.getActualCards([cardId]).length > 0 &&
      room.isCardOnProcessing(cardId)
    ) {
