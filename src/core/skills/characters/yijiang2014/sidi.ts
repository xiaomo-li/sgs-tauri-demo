...
export class SiDi extends TriggerSkill {
  ...
  public async onEffect(
    ...
    if (phaseStageChangeEvent.toStage === PlayerPhaseStages.FinishStageStart) {
      ...
      const originalColor =
        room.getFlag<CardColor[]>(phaseStageChangeEvent.playerId, this.Name) ||
        [];
      ...
      room
        .getPlayerById(phaseStageChangeEvent.playerId)
        .hasShadowSkill(SiDiBlocker.Name) ||
      ...
      room
        .getPlayerById(phaseStageChangeEvent.playerId)
        .hasShadowSkill(SiDiRemover.Name) ||
        (await room.obtainSkill(
          phaseStageChangeEvent.playerId,
          SiDiRemover.Name
        ));
...
export class SiDiShadow extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public canUse(
    ...
    return (
      owner.getFlag<boolean>(SiDi.SiDiTarget) &&
  ...
  public async onEffect(
    ...
    if (
      !records.find(
        (event) => Sanguosha.getCardById(event.cardId).GeneralName === "slash"
      )
    ) {
      ...
      room
        ...
        .canUseCardTo(room, slash, current, true) &&
        (await room.useCard({
          fromId: event.fromId,
          targetGroup: [[current]],
          cardId: slash,
...
@CommonSkill({
  name: "s_sidi_blocker",
  description: "s_sidi_blocker_description",
})
export class SiDiBlocker extends FilterSkill {
  public canUseCard(
    ...
    if (cardId instanceof CardMatcher) {
      ...
      colors.includes(CardColor.Red) &&
        suits.push(...[CardSuit.Diamond, CardSuit.Heart]);
      colors.includes(CardColor.Black) &&
        suits.push(...[CardSuit.Spade, CardSuit.Club]);
...
@CommonSkill({
  name: "s_sidi_remover",
  description: "s_sidi_remover_description",
})
export class SiDiRemover extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public async whenDead(room: Room, player: Player) {
    ...
    room.getPlayerById(player.Id).hasShadowSkill(this.Name) &&
  ...
  public async onEffect(
    ...
    room.getPlayerById(event.fromId).hasShadowSkill(this.Name) &&
