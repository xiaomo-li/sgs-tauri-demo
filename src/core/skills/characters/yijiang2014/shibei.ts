...
export class ShiBei extends TriggerSkill {
  public async whenObtainingSkill(room: Room, owner: Player) {
    const records = room.Analytics.getDamagedRecord(
      owner.Id,
      "round",
      undefined,
      1
  ...
  public async onEffect(
    ...
    if (EventPacker.getMiddleware<boolean>(this.Name, damagedEvent)) {
      await room.recover({
        recoveredHp: 1,
        recoverBy: damagedEvent.toId,
        toId: damagedEvent.toId,
      });
    } else {
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.DamageEvent) {
      ...
      return (
        damageEvent.toId === owner.Id && !owner.getFlag<boolean>(this.Name)
      );
