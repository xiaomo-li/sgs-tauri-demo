...
export class PinDi extends ActiveSkill {
  ...
  public isAvailableCard(owner: PlayerId, room: Room, cardId: CardId): boolean {
    return (
      ...
      !room
        .getFlag<CardType[]>(owner, PinDi.PinDiType)
  ...
  public async onEffect(
    ...
    const originalTypes =
      room.getFlag<CardType[]>(fromId, PinDi.PinDiType) || [];
    ...
    room.getPlayerById(toIds[0]).getPlayerCards().length > 0 &&
      options.push("pindi:discard");

    let chosen = options[0];
    const skillUsedTimes = room
    ...
    if (
      !room.getPlayerById(toIds[0]).Dead &&
      room.getPlayerById(toIds[0]).LostHp > 0
    ) {
...
export class PinDiShadow extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public async onEffect(
    ...
    room.getFlag<CardType[]>(event.fromId, PinDi.PinDiType) &&
      room.removeFlag(event.fromId, PinDi.PinDiType);
