  ...
  public async onEffect(
    ...
    if (options.length > 1) {
      ...
      response.selectedOption =
        response.selectedOption ||
        askForChooseEvent.options[options.length - 1];
      ...
      if (response.selectedOption === "qieting:prey") {
        ...
        if (to.getCardIds(PlayerCardsArea.HandArea).length > 2) {
          options[PlayerCardsArea.HandArea] = Algorithm.randomPick(
            2,
            to.getCardIds(PlayerCardsArea.HandArea)
          );
        }
