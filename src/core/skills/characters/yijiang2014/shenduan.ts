...
import { BingLiangCunDuan } from "../../../cards/legion_fight/bingliangcunduan";
...
export class ShenDuan extends TriggerSkill {
  ...
  public async beforeUse(
      ...
      availableIds = moveCardEvent.infos.reduce<CardId[]>((cardIds, info) => {
        ...
        for (const movingCard of info.movingCards) {
          ...
          if (
            ...
            (movingCard.fromArea === PlayerCardsArea.HandArea ||
              movingCard.fromArea === PlayerCardsArea.EquipArea) &&
    ...
    if (
      response.selectedCards === undefined ||
    ...
    from.setFlag<CardId[]>(
      ...
      from
        .getFlag<CardId[]>(this.shenDuanBannedIds)
    ...
    if (
      newResponse.selectedPlayers === undefined ||
      newResponse.selectedPlayers.length === 0
    ) {
      from.removeFlag(this.shenDuanBannedIds);
      return false;
    }
  ...
  public async onEffect(
    ...
    const { fromId } = event;
    const target = EventPacker.getMiddleware<PlayerId>(
      this.shenDuanTarget,
      event
    );
