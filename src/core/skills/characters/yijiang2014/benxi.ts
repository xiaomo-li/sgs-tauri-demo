...
export class BenXi extends TriggerSkill {
  ...
  public isTriggerable(
    ...
    return (
      ...
      stage === CardUseStage.AfterCardTargetDeclared
    );
  }
      ...
      canUse =
        ...
        (Sanguosha.getCardById(content.cardId).GeneralName === "slash" ||
          (Sanguosha.getCardById(content.cardId).is(CardType.Trick) &&
            !Sanguosha.getCardById(content.cardId).is(
              CardType.DelayedTrick
            ))) &&
        room
          .getOtherPlayers(owner.Id)
          .find((player) => room.distanceBetween(owner, player) > 1) ===
          undefined;
    }
  ...
  async onEffect(
    ...
    if (stage === CardUseStage.CardUsing) {
      ...
      const availableTargets: PlayerId[] = room
        ...
        .filter(
          (playerId) =>
            !TargetGroupUtil.getRealTargets(cardUseEvent.targetGroup).includes(
              playerId
            ) &&
      ...
      for (let i = 0; i < 2; i++) {
        ...
        if (resp.selectedOption) {
          ...
          const index = options.findIndex(
            (selected) => selected === resp.selectedOption
          );
            ...
            const response =
              await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingCardAvailableTargetEvent>(
                GameEventIdentifiers.AskForChoosingCardAvailableTargetEvent,
                {
                  ...
                  exclude: TargetGroupUtil.getRealTargets(
                    cardUseEvent.targetGroup
                  ),
                  conversation:
                    "benxi: please select a player to append to card targets",
                  triggeredBySkills: [this.Name],
                },
                fromId,
                true
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.PhaseChangeEvent) {
      ...
      return (
        ...
        damageEvent.triggeredBySkills.includes(this.Name)
      );
    }
...
@PersistentSkill()
@CompulsorySkill({
  name: BenXiShadow.Name,
  description: BenXiShadow.Description,
})
export class BenXiCharge
