...
export class DuoJiShadow extends TriggerSkill {
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      return (
        from.getCardIds(PlayerCardsArea.OutsideArea, this.GeneralName).length >
          0 &&
      ...
      return (
        room
          ...
          0 && phaseChangeEvent.from === PlayerPhase.PhaseFinish
      );
    }
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      if (
        user.getCardIds(PlayerCardsArea.EquipArea).includes(cardUseEvent.cardId)
      ) {
        await room.moveCards({
          movingCards: [
            { card: cardUseEvent.cardId, fromArea: CardMoveArea.EquipArea },
          ],
