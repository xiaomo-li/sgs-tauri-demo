...
export class ZuiCi extends TriggerSkill {
    ...
    if (stage === PhaseStageChangeStage.StageChanged) {
      return (
        (event as ServerEventFinder<GameEventIdentifiers.PhaseStageChangeEvent>)
          .toStage === PlayerPhaseStages.PrepareStage
      );
    }
  ...
  public async onEffect(
    ...
    const options = from.AvailableEquipSections.filter(
      (section) =>
        from.getEquipment(this.equipSectionToCardTypeMapper[section]) !==
        undefined
    );
    ...
    await room.abortPlayerEquipSections(
      fromId,
      selectedOption as CharacterEquipSections
    );
    await room.recover({
      toId: fromId,
      recoveredHp: 2,
    ...
    const markedPlayer = room
      .getOtherPlayers(fromId)
      .find((player) => player.getMark(MarkEnum.Fu) > 0);
    const players = room
      .getOtherPlayers(fromId)
