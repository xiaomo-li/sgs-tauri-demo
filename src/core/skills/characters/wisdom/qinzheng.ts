...
export class QinZheng extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public canUse(
    ...
    return (
      ...
      factors.find((factor) => cardUsedNum % factor === 0) !== undefined
    );
  }
  ...
  private async gainQinZhengCard(
    room: Room,
    owner: PlayerId,
    patterns: string[]
  ) {
    ...
    for (const card of patterns) {
      const cards = room.findCardsByMatcherFrom(
        new CardMatcher({ generalName: [card] })
      );
      if (cards.length > 0) {
    ...
    if (toGain.length > 0) {
      await room.moveCards({
        movingCards: [
          {
            card: toGain[Math.floor(Math.random() * toGain.length)],
