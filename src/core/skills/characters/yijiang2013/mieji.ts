  ...
  async onEffect(
    ...
    const nonTrickCards: CardId[] = to
      ...
      .filter(
        (cardId) =>
          !Sanguosha.getCardById(cardId).is(CardType.Trick) &&
          room.canDropCard(fromId, cardId)
      );
    if (
      to
        .getPlayerCards()
        .filter((id) => Sanguosha.getCardById(id).is(CardType.Trick)).length > 0
    ) {
    ...
    if (selectedOption === "mieji:trick") {
      ...
      await room.moveCards({
        movingCards: [
          { card: selectedCards[0], fromArea: CardMoveArea.HandArea },
        ],
        fromId: toId,
      ...
      while (droppedCards < Math.min(nonTrickCards.length, 2)) {
        const response = await room.askForCardDrop(
          ...
          to
            ...
            .filter(
              (cardId) =>
                Sanguosha.getCardById(cardId).BaseType === CardType.Trick
            ),
          this.Name
        );
