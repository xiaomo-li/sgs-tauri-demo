...
export class ZongXuan extends TriggerSkill {
    ...
    return (
      content.infos.find(
        (info) =>
          ...
          info.movingCards.find((node) => room.isCardInDropStack(node.card)) !==
            undefined
      ) !== undefined
    );
  }
  ...
  public async onEffect(
    ...
    if (moveCardEvent.infos.length === 1) {
      cardIds.push(
        ...moveCardEvent.infos[0].movingCards
          .filter((node) => room.isCardInDropStack(node.card))
          .map((node) => node.card)
      );
    } else {
    ...
    const tricks = cardIds.filter((id) =>
      Sanguosha.getCardById(id).is(CardType.Trick)
    );
    if (tricks.length > 0) {
      ...
      if (response.selectedCards && response.selectedCards.length > 0) {
        ...
        if (resp.selectedPlayers && resp.selectedPlayers.length > 0) {
          ...
          const index = cardIds.findIndex(
            (id) => id === response.selectedCards![0]
          );
