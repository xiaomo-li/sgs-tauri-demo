...
export class QiaoShuoWin extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public isRefreshAt(room: Room, owner: Player, stage: PlayerPhase): boolean {
    return (
      stage === PlayerPhase.FinishStage && room.CurrentPhasePlayer === owner
    );
  }

  public whenRefresh(room: Room, owner: Player): void {
    owner.getFlag<boolean>(QiaoShuo.WIN) &&
      room.removeFlag(owner.Id, QiaoShuo.WIN);
  }
  ...
  public async onEffect(
    ...
    if (
      ...
      const realTargets = TargetGroupUtil.getRealTargets(
      ...
      const pendingTargets = room
        ...
        .filter(
          (playerId) =>
            ...
            cardSkill.isCardAvailableTarget(
              fromId,
              room,
              playerId,
              [],
        ...
        const { selectedPlayers } =
          await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingPlayerEvent>(
            GameEventIdentifiers.AskForChoosingPlayerEvent,
            {
              ...
              requiredAmount: 1,
              conversation:
                "qiaoshuo: please select a player to reduce from card targets",
              triggeredBySkills: [this.Name],
            },
            fromId,
            true
        ...
        room.broadcast(GameEventIdentifiers.CustomGameDialog, {
          translationsMessage: TranslationPack.translationJsonPatcher(
            ...
            TranslationPack.patchPlayerInTranslation(
              room.getPlayerById(selectedPlayers![0])
            ),
        ...
        const { selectedPlayers } =
          await room.doAskForCommonly<GameEventIdentifiers.AskForChoosingCardAvailableTargetEvent>(
            GameEventIdentifiers.AskForChoosingCardAvailableTargetEvent,
            {
              ...
              exclude: realTargets,
              conversation:
                "qiaoshuo: please select a player to append to card targets",
              triggeredBySkills: [this.Name],
            },
            fromId
          );
  ...
  implements OnDefineReleaseTiming
{
  ...
  public whenRefresh(room: Room, owner: Player) {
    owner.getFlag<boolean>(QiaoShuo.LOSE) &&
      room.removeFlag(owner.Id, QiaoShuo.LOSE);
  }
