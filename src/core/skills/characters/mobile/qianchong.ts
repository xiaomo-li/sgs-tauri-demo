...
export class QianChong extends TriggerSkill implements OnDefineReleaseTiming {
  ...
  public async whenLosingSkill(room: Room, player: Player) {
    ...
    for (const skillName of this.RelatedSkills) {
      player.getFlag<string>(this.Name) === skillName &&
        (await room.loseSkill(player.Id, skillName));
    }
  ...
  private async handleQianChongSkills(
    room: Room,
    player: Player
  ): Promise<void> {
    ...
    let originalSkillName: string | undefined = player.getFlag<string>(
      this.Name
    );
        ...
        (info) =>
          (info.fromId === owner.Id &&
            info.movingCards.find(
              (cardInfo) => cardInfo.fromArea === CardMoveArea.EquipArea
            )) ||
...
export class QianChongShadow extends TriggerSkill {
      ...
      (equips.length === 0 ||
        !!equips.find(
          (cardId) =>
            ...
            Sanguosha.getCardById(equips[0]).Color
        ))
    );
  }
  ...
  public async onEffect(
    ...
    const originalTypes =
      room.getFlag<CardType[]>(event.fromId, this.Name) || [];
    if (!originalTypes.includes(mapper[response.selectedOption])) {
      ...
      room.setFlag<CardType[]>(
        ...
        TranslationPack.translationJsonPatcher(
          originalText,
          this.GeneralName,
          ...originalTypes.map((type) =>
            Functional.getCardBaseTypeAbbrRawText(type)
          )
        ).toString()
      );
    }
...
@PersistentSkill()
@CommonSkill({
  name: QianChongShadow.Name,
  description: QianChongShadow.Description,
})
export class QianChongBuff
  ...
  implements OnDefineReleaseTiming
{
  ...
  public breakCardUsableDistance(
    ...
    if (cardId instanceof CardMatcher) {
      return cardId.match(
        new CardMatcher({
          type: owner.getFlag<CardType[]>(QianChongShadow.Name),
        })
      )
        ? INFINITE_DISTANCE
        : 0;
      ...
      return owner
        .getFlag<CardType[]>(QianChongShadow.Name)
        .includes(Sanguosha.getCardById(cardId).BaseType)
        ? INFINITE_DISTANCE
        : 0;
    }
  }
...
@PersistentSkill()
@CommonSkill({
  name: QianChongBuff.Name,
  description: QianChongBuff.Description,
})
export class QianChongRemover
