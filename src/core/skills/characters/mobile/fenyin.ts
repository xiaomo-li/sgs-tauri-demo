...
export class FenYin extends TriggerSkill implements OnDefineReleaseTiming {
  public async whenObtainingSkill(room: Room, player: Player) {
    if (
      ...
      room.CurrentPhasePlayer !== player
    ) {
      return;
    }
    ...
    if (records.length > 0) {
      const lastColor = Sanguosha.getCardById(
        records[records.length - 1].cardId
      ).Color;
      if (lastColor !== CardColor.None) {
        player.setFlag<CardColor>(this.Name, lastColor);
        const precolor = Sanguosha.getCardById(
          records[records.length - 2].cardId
        ).Color;
        if (
          records.length > 1 &&
          precolor !== CardColor.None &&
          lastColor !== precolor
        ) {
  ...
  public async onEffect(
    ...
    if (
      ...
      if (card.Color === CardColor.None) {
        ...
        const precolor = room.getFlag<CardColor>(
          event.fromId,
          this.GeneralName
        );
        ...
        room
          ...
          .setFlag<CardColor>(this.GeneralName, card.Color);
      }
    } else {
      room.getPlayerById(event.fromId).removeFlag(this.GeneralName);
    }
