...
export class ZhaoXin extends ActiveSkill {
  ...
  public cardFilter(room: Room, owner: Player, cards: CardId[]): boolean {
    return (
      cards.length > 0 &&
      cards.length <=
        3 - owner.getCardIds(PlayerCardsArea.OutsideArea, this.Name).length
    );
  }
  ...
  public async onEffect(
    ...
    await room.moveCards({
      movingCards: cardIds.map((card) => ({
        card,
        fromArea: room.getPlayerById(fromId).cardFrom(card),
      })),
      fromId,
      toId: fromId,
...
export class ZhaoXinShadow extends TriggerSkill {
  ...
  public isTriggerable(
    ...
    return (
      ...
      event.toStage === PlayerPhaseStages.DrawCardStageEnd
    );
  }
  ...
  public canUse(
    ...
    return (
      ...
      (event.playerId === owner.Id ||
        room.withinAttackDistance(owner, room.getPlayerById(event.playerId)))
    );
  }
  ...
  public async onEffect(
    ...
    await room.moveCards({
      movingCards: [
        { card: event.cardIds[0], fromArea: CardMoveArea.OutsideArea },
      ],
      fromId: event.fromId,
      toId: playerId,
