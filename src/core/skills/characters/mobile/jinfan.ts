...
export class JinFan extends TriggerSkill {
  ...
  public cardFilter(room: Room, owner: Player, cards: CardId[]): boolean {
    return (
      cards.length > 0 &&
      cards.length <=
        4 - owner.getCardIds(PlayerCardsArea.OutsideArea, this.Name).length
    );
  }
  ...
  public isAvailableCard(
    ...
    const suits = [
      ...ownerPlayer.getCardIds(PlayerCardsArea.OutsideArea, this.Name),
      ...selectedCards,
    ].reduce<CardSuit[]>((allSuits, id) => {
...
export class JinFanShadow extends TriggerSkill {
  ...
  public canUse(
    ...
    if (stage === CardMoveStage.CardMoving) {
            ...
            allCards.push(
              ...info.movingCards
                .filter(
                  (cardInfo) =>
                    cardInfo.fromArea === PlayerCardsArea.OutsideArea &&
                    owner.getOutsideAreaNameOf(cardInfo.card) ===
                      this.GeneralName
                )
      ...
      if (ling.length > 0) {
        EventPacker.addMiddleware(
          { tag: this.GeneralName, data: ling },
          content
        );
      }
    }
    ...
    return (
      stage === CardMoveStage.AfterCardMoved &&
      EventPacker.getMiddleware<CardId[]>(this.GeneralName, content) !==
        undefined
    );
  }
  ...
  public async onEffect(
    ...
    for (const suit of suits) {
      const randomCards = room.findCardsByMatcherFrom(
        new CardMatcher({ suit: [suit] })
      );
