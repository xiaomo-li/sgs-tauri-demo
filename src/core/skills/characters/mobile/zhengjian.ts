...
export class ZhengJian extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const toId = (response.selectedPlayers || [
      players[Math.floor(Math.random() * players.length)],
    ])[0];

    const originalTargets =
      room.getFlag<PlayerId[]>(event.fromId, ZhengJian.Targets) || [];
    originalTargets.includes(toId) || originalTargets.push(toId);
    room
      .getPlayerById(event.fromId)
      .setFlag<PlayerId[]>(ZhengJian.Targets, originalTargets);
...
@PersistentSkill()
@CommonSkill({
  name: "s_zhengjian_recorder",
  description: "s_zhengjian_recorder_description",
})
export class ZhengJianRecorder
  extends TriggerSkill
  implements OnDefineReleaseTiming
{
  ...
  public isTriggerable(
    ...
    return (
      ...
      stage === CardResponseStage.BeforeCardResponseEffect
    );
  }
  ...
  public canUse(
    ...
    return (
      owner.getFlag<number>(ZhengJian.Name) !== undefined &&
  ...
  public async onEffect(
    ...
    const originalCount =
      room.getFlag<number>(event.fromId, ZhengJian.Name) || 0;
    room.setFlag<number>(
      event.fromId,
      ZhengJian.Name,
      originalCount + 1,
