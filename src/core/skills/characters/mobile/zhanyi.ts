...
import { ActiveSkill, TriggerSkill, ViewAsSkill } from "../../skill";
    ...
    if (!room.getPlayerById(event.fromId).Dead) {
      ...
      if (typeDiscarded === CardType.Basic) {
        room.installSideEffectSkill(
          System.SideEffectSkillApplierEnum.ZhanYi,
          ZhanYiSide.Name,
          event.fromId
        );
  ...
  implements OnDefineReleaseTiming
{
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      cardUseEvent.additionalRecoveredHp =
        (cardUseEvent.additionalRecoveredHp || 0) + 1;
      ...
      cardEffectEvent.disresponsiveCards =
        cardEffectEvent.disresponsiveCards || [];
      ...
      if (response.droppedCards.length > 0) {
        ...
        const toGain = response.droppedCards.filter((cardId) =>
          room.isCardInDropStack(cardId)
        );
        if (toGain.length > 0) {
          const resp =
          ...
          await room.moveCards({
            movingCards: [
              { card: resp.selectedCards[0], fromArea: CardMoveArea.DropStack },
            ],
            toId: event.fromId,
      ...
      if (
        room.getFlag<CardType>(event.fromId, this.GeneralName) ===
        CardType.Basic
      ) {
...
@CommonSkill({
  name: "side_zhanyi_s",
  description: "side_zhanyi_s_description",
})
    ...
    if (identifier === GameEventIdentifiers.AskForCardUseEvent) {
      return (
        Sanguosha.getCardNameByType((types) =>
          ...
          owner.canUseCard(
            room,
            new CardMatcher({ name: [name] }),
            new CardMatcher(event!.cardMatcher)
          )
        ) !== undefined
      );
    }
