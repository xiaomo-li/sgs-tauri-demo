...
@CommonSkill({
  name: "mobile_yizheng",
  description: "mobile_yizheng_description",
})
export class MobileYiZheng extends ActiveSkill {
  ...
  public isAvailableTarget(
    ...
    return (
      room.getPlayerById(owner).Hp >= room.getPlayerById(target).Hp &&
  ...
  public async onEffect(
    ...
    if (pindianReport.pindianRecord[0].winner === fromId) {
      room.getFlag<boolean>(toIds[0], this.Name) ||
...
@PersistentSkill()
@CommonSkill({
  name: "s_myizheng_debuff",
  description: "s_myizheng_debuff_description",
})
export class MobileYiZhengDebuff extends TriggerSkill {
  ...
  public canUse(
    ...
    return (
      content.toPlayer === owner.Id && content.to === PlayerPhase.DrawCardStage
    );
  }
