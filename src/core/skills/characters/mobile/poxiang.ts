...
export class PoXiang extends ActiveSkill {
  ...
  public async onEffect(
    ...
    const cardIdsDrawn = await room.drawCards(
      3,
      fromId,
      "top",
      fromId,
    ...
    room
      ...
      .map((card) => ({ card, fromArea: CardMoveArea.OutsideArea })).length >
      0 &&
      (await room.moveCards({
        movingCards: room
          .getPlayerById(fromId)
...
@PersistentSkill()
@CompulsorySkill({
  name: PoXiang.GeneralName,
  description: PoXiang.Description,
})
export class PoXiangShadow
    ...
    if (identifier === GameEventIdentifiers.AskForCardDropEvent) {
      ...
      if (cardIdsDrawn.length > 0) {
        const otherHandCards = player
          ...
          .filter((card) => !cardIdsDrawn.includes(card));
        const discardAmount =
