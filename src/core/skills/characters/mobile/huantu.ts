  ...
  public getSkillLog(
    ...
    return TranslationPack.translationJsonPatcher(
      ...
      TranslationPack.patchPlayerInTranslation(
        room.getPlayerById(event.toPlayer)
      )
    ).extract();
  }
      ...
      movingCards: [
        {
          ...
          fromArea: room.getPlayerById(toId).cardFrom(event.cardIds[0]),
        },
      ],
      fromId: event.fromId,
      toId,
  ...
  public async onEffect(
    ...
    if (
      ...
      if (response.selectedOption === "huantu:recover") {
        ...
        if (
          room.getPlayerById(fromId).getCardIds(PlayerCardsArea.HandArea)
            .length > 1
        ) {
