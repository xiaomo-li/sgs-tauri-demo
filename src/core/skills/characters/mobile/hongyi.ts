...
export class HongYi extends ActiveSkill {
  ...
  public async onEffect(
    ...
    const originalTargets =
      room.getFlag<PlayerId[]>(event.fromId, HongYi.Targets) || [];
...
@PersistentSkill()
@CommonSkill({
  name: "s_hongyi_debuff",
  description: "s_hongyi_debuff_description",
})
export class HongYiDebuff
  extends TriggerSkill
  implements OnDefineReleaseTiming
{
  public isAutoTrigger(): boolean {
  ...
  public async whenLosingSkill(room: Room, owner: Player) {
    owner.getFlag<boolean>(HongYi.Name) &&
      room.removeFlag(owner.Id, HongYi.Name);
  }
  ...
  public async onEffect(
    ...
    if (Sanguosha.getCardById(judgeEvent.judgeCardId).isRed()) {
      ...
      room.getPlayerById(victim).Dead ||
        (await room.drawCards(1, victim, "top", event.fromId, HongYi.Name));
