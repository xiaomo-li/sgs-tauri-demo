...
export class JingLve extends ActiveSkill {
  ...
  public canUse(room: Room, owner: Player) {
    return (
      ...
      !room.AlivePlayers.find((player) =>
        player.getFlag<JingLveMapper>(JingLve.JingLveMapper)
      )
    );
  }
  ...
  public async onEffect(
    ...
    const originalPlayers =
      room.getFlag<PlayerId[]>(event.fromId, this.Name) || [];
    originalPlayers.includes(event.toIds[0]) ||
      originalPlayers.push(event.toIds[0]);
    ...
    room.setFlag<JingLveMapper>(
      ...
      TranslationPack.translationJsonPatcher(
        "sishi: {0} {1}",
        sishi.Name,
        Functional.getCardSuitCharText(sishi.Suit) +
          Functional.getCardNumberRawText(sishi.CardNumber)
      ).toString(),
      [event.fromId]
    );
...
@PersistentSkill()
@CommonSkill({
  name: "s_jinglve_sishi",
  description: "s_jinglve_sishi_description",
})
export class JingLveSiShi
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      return (
        ...
        VirtualCard.getActualCards([cardUseEvent.cardId]).includes(
          Object.values(sishi)[0]
        ) &&
        !(
          Sanguosha.getCardById(cardUseEvent.cardId).Skill instanceof
            ResponsiveSkill && !cardUseEvent.toCardIds
        )
      );
        ...
        (
          ...
          (info) =>
            ...
            info.movingCards.find(
              (cardInfo) => cardInfo.card === Object.values(sishi)[0]
            )
        ) !== undefined
      );
      ...
      return (
        ...
        owner
          ...
          .find(
            (id) =>
              VirtualCard.getActualCards([id])[0] === Object.values(sishi)[0]
          ) !== undefined
      );
    }
  ...
  public async onEffect(
    ...
    if (identifier === GameEventIdentifiers.CardUseEvent) {
      ...
      const sishi = Object.entries(
        room.getFlag<JingLveMapper>(event.fromId, JingLve.JingLveMapper)
      )[0];
      if (
        !room.getPlayerById(sishi[0]).Dead &&
        (
          unknownEvent as ServerEventFinder<GameEventIdentifiers.MoveCardEvent>
        ).infos.find(
          (info) =>
        ...
        toGain &&
          (await room.moveCards({
            movingCards: [
              {
                card: toGain,
                fromArea: room.getPlayerById(event.fromId).cardFrom(toGain),
              },
            ],
            fromId: event.fromId,
            toId: sishi[0],
