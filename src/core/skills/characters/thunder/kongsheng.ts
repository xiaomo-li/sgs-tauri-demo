...
export class KongShengShadow extends TriggerSkill {
  ...
  public async onEffect(
    ...
    const useableEquips = kong.filter(
      (card) =>
        Sanguosha.getCardById(card).is(CardType.Equip) &&
        from.canUseCardTo(room, card, fromId)
    );
    while (useableEquips.length > 0) {
      ...
      const index = useableEquips.findIndex(
        (equip) => equip === response.selectedCards[0]
      );
      useableEquips.splice(index, 1);
    }

    const leftCards = from.getCardIds(
