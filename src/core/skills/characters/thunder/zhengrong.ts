...
export class ZhengRong extends TriggerSkill {
  ...
  public canUse(
    ...
    const canUse =
      ...
      (Object.values(DamageCardEnum) as string[]).includes(
        Sanguosha.getCardById(content.byCardId).GeneralName
      );
    ...
    if (canUse) {
      targets = AimGroupUtil.getAllTargets(content.allTargets).filter(
        (playerId) =>
          room.getPlayerById(playerId).getCardIds(PlayerCardsArea.HandArea)
  ...
  public async onEffect(
    ...
    await room.moveCards({
      movingCards: [
        { card: response.selectedCard, fromArea: CardMoveArea.HandArea },
      ],
      fromId: toIds[0],
      toId: fromId,
