...
export class JueYan extends ActiveSkill {
  ...
  public canUse(room: Room, owner: Player) {
    return (
      !owner.hasUsedSkill(this.Name) && owner.AvailableEquipSections.length > 0
    );
  }
  ...
  public async onEffect(
    ...
    if (options.length < from.AvailableEquipSections.length) {
      const index = options.findIndex(
        (section) => section === CharacterEquipSections.Precious
      );
      if (index !== -1) {
    ...
    if (response.selectedOption === "ride section") {
      ...
      await room.abortPlayerEquipSections(
        fromId,
        response.selectedOption as CharacterEquipSections
      );
  ...
  implements OnDefineReleaseTiming
{
  ...
  public breakCardUsableDistance(
    ...
    return owner.getFlag<string[]>(this.GeneralName)?.includes("ride section")
      ? INFINITE_DISTANCE
      : 0;
  }
  ...
  public breakCardUsableTimes(
    ...
    if (
      !owner
        .getFlag<string[]>(this.GeneralName)
        ?.includes(CharacterEquipSections.Weapon)
    ) {
      return 0;
    }
  ...
  public breakAdditionalCardHoldNumber(room: Room, owner: Player): number {
    return owner
      .getFlag<string[]>(this.GeneralName)
      ?.includes(CharacterEquipSections.Shield)
      ? 3
      : 0;
  }
}
  ...
  implements OnDefineReleaseTiming
{
  ...
  public canUse(
    ...
    return (
      (owner.Id === event.fromPlayer &&
        ...
        sections.find(
          (section) => section !== CharacterEquipSections.Shield
        ) !== undefined) ||
      (event.from === PlayerPhase.PhaseFinish &&
        sections.includes(CharacterEquipSections.Shield))
    );
  }
  ...
  public async onEffect(
    ...
    const phaseFrom = (
      triggeredOnEvent as ServerEventFinder<GameEventIdentifiers.PhaseChangeEvent>
    ).from;
    ...
    if (phaseFrom === PlayerPhase.PlayCardStage) {
      ...
      if (
        sections.includes(CharacterEquipSections.Precious) &&
        room.getPlayerById(fromId).hasSkill("jizhi")
      ) {
