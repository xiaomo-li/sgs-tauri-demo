...
export class SlashSkill extends ActiveSkill implements ExtralCardSkillProperty {
  ...
  public canUse(room: Room, owner: Player, contentOrContainerCard: CardId) {
    return (
      room
        ...
        .find(
          (player) =>
            room.CommonRules.getCardUsableTimes(
              room,
              owner,
              Sanguosha.getCardById(contentOrContainerCard),
              player
            ) >
  ...
  async onEffect(
    ...
    const addtionalDrunkDamage =
      EventPacker.getMiddleware<number>(TagEnum.DrunkTag, event) || 0;
