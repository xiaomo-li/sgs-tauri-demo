import { GuoHeChaiQiaoSkillTrigger } from "../../../ai/skills/cards/guohechaiqiao";
...
@CommonSkill({
  name: "guohechaiqiao",
  description: "guohechaiqiao_description",
})
export class GuoHeChaiQiaoSkill
  extends ActiveSkill
  implements ExtralCardSkillProperty
{
  ...
  public async onEffect(
    ...
    const to = room.getPlayerById(
      Precondition.exists(event.toIds, "Unknown targets in guohechaiqiao")[0]
    );
    if (
      (to.Id === event.fromId &&
        to.getCardIds().filter((id) => room.canDropCard(to.Id, id)).length ===
          0) ||
