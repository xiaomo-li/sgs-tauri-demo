...
export class CardMatcher {
  ...
  public static match(
    matcher: CardMatcherSocketPassenger,
    card: Card | CardMatcher
  ) {
    Precondition.assert(
      matcher.tag && matcher.tag === "card-matcher",
      "Invalid card matcher props"
    );

    const { suit, cardNumber, name, generalName, type, cards, reverseMatch } =
      matcher;
    let matched = true;
    ...
    if (card instanceof Card) {
      ...
      if (suit && cardMatcher.suit) {
        ...
        matched =
          matched &&
          cardMatcher.suit.length > 0 &&
          cardMatcher.suit.every((cardSuit) => suit.includes(cardSuit));
      }
      if (cardNumber && cardMatcher.cardNumber) {
        ...
        matched =
          ...
          cardMatcher.cardNumber.every((cardNum) =>
            cardNumber.includes(cardNum)
          );
      }
      if (name) {
        if (cardMatcher.name) {
          ...
          matched =
            matched &&
            cardMatcher.name.length > 0 &&
            Algorithm.intersection(name, cardMatcher.name).length > 0;
        }
        if (cardMatcher.generalName && cardMatcher.generalName.length > 0) {
          hasMatchedCondition = true;
          const allNames = cardMatcher.generalName.reduce<string[]>(
            (target, currentGeneralName) => {
          ...
          matched =
            matched && Algorithm.intersection(allNames, name).length > 0;
        }
      }
      if (generalName) {
        if (cardMatcher.generalName) {
          ...
          matched =
            ...
            Algorithm.intersection(generalName, cardMatcher.generalName)
              .length > 0;
        }
        if (cardMatcher.name) {
          hasMatchedCondition = true;
          const allNames = generalName.reduce<string[]>(
            (target, currentGeneralName) => {
              if (this.generalNameMaps[currentGeneralName]) {
                target.push(...this.generalNameMaps[currentGeneralName]);
              }
          ...
          matched =
            matched &&
            Algorithm.intersection(allNames, cardMatcher.name).length > 0;
        }
      }
      if (type && cardMatcher.type) {
        ...
        matched =
          matched &&
          cardMatcher.type.length > 0 &&
          cardMatcher.type.every((cardType) => type.includes(cardType));
      }
      if (cards && cardMatcher.cards) {
        hasMatchedCondition = true;
        matched =
          matched &&
          cardMatcher.cards.length > 0 &&
          cardMatcher.cards.every((innerCard) => cards.includes(innerCard));
      }
    }
