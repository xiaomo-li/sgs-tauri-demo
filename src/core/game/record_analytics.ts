...
export class RecordAnalytics {
  ...
  public record<T extends GameEventIdentifiers>(
    ...
    if (currentPhase) {
      this.currentRoundEvents[currentPhase] =
        this.currentRoundEvents[currentPhase] || [];
      this.currentRoundEvents[currentPhase]!.push(event);

      this.currentCircleEvents[currentPhase] =
        this.currentCircleEvents[currentPhase] || [];
  ...
  public getRecordEvents<T extends GameEventIdentifiers>(
    ...
    if (current) {
      if (inPhase !== undefined && current !== "phase") {
        ...
        events = inPhase.reduce<ServerEventFinder<T>[]>(
          (selectedEvents, phase) => {
        ...
        if (current === "phase") {
          ...
          events = Object.values(this.currentPhaseEvents).reduce<
            ServerEventFinder<T>[]
          >((allEvents, phaseEvent) => {
          ...
          events = Object.values(currentEvents).reduce<ServerEventFinder<T>[]>(
            (allEvents, phaseEvents) => {
              phaseEvents &&
                allEvents.push(...(phaseEvents as ServerEventFinder<T>[]));
              return allEvents;
            },
            []
          );
    ...
    return this.getRecordEvents<GameEventIdentifiers.RecoverEvent>(
      (event) =>
        EventPacker.getIdentifier(event) ===
          GameEventIdentifiers.RecoverEvent && event.toId === player,
      undefined,
      current,
      inPhase,
      num
  ...
  public getLostHpRecord(
    ...
    return this.getRecordEvents<GameEventIdentifiers.LoseHpEvent>(
      (event) =>
        EventPacker.getIdentifier(event) === GameEventIdentifiers.LoseHpEvent &&
        event.toId === player,
      undefined,
      current,
      inPhase,
  ...
  public getCardUseRecord(
    ...
    return this.getRecordEvents<GameEventIdentifiers.CardUseEvent>(
      (event) =>
        EventPacker.getIdentifier(event) ===
          GameEventIdentifiers.CardUseEvent && event.fromId === player,
      undefined,
      current,
      inPhase,
      num
  ...
  public getCardResponseRecord(
    ...
    return this.getRecordEvents<GameEventIdentifiers.CardResponseEvent>(
      (event) =>
        EventPacker.getIdentifier(event) ===
          GameEventIdentifiers.CardResponseEvent && event.fromId === player,
      undefined,
      current,
      inPhase,
      num
  ...
  public getCardObtainedRecord(
    ...
    return this.getRecordEvents<GameEventIdentifiers.MoveCardEvent>(
      (event) =>
        ...
        event.infos.find(
          (info) =>
            info.toId === player && info.toArea === CardMoveArea.HandArea
        ) !== undefined,
      undefined,
      current,
      inPhase,
  ...
  public getCardDrawRecord(
    ...
    return this.getRecordEvents<GameEventIdentifiers.MoveCardEvent>(
      (event) =>
        ...
        event.infos.find(
          (info) =>
            info.toId === player && info.moveReason === CardMoveReason.CardDraw
        ) !== undefined,
      undefined,
      current,
      inPhase,
  ...
  public getRecoveredHp(
    player: PlayerId,
    current?: RecordCurrentType,
    inPhase?: PlayerPhase[],
    num: number = 0
  ) {
    return this.getRecoveredHpRecord(
      player,
      current,
      inPhase,
      num
    ).reduce<number>((totalAmount, event) => {
  ...
  public getDamage(
    ...
    return this.getDamageRecord(player, current, inPhase, num).reduce<number>(
      (totalAmount, event) => {
  ...
  public getDamaged(
    ...
    return this.getDamagedRecord(player, current, inPhase, num).reduce<number>(
      (totalAmount, event) => {
        totalAmount += event.damage;
        return totalAmount;
      },
  ...
  public getLostHp(
    ...
    return this.getLostHpRecord(player, current, inPhase, num).reduce<number>(
      (totalAmount, event) => {
  ...
  public getUsedCard(
    ...
    return this.getCardUseRecord(player, current, inPhase, num).reduce<
      CardId[]
    >((allCards, event) => {
  ...
  public getResponsedCard(
    player: PlayerId,
    current?: RecordCurrentType,
    inPhase?: PlayerPhase[],
    num: number = 0
  ) {
    return this.getCardResponseRecord(player, current, inPhase, num).reduce<
      CardId[]
    >((allCards, event) => {
      allCards.push(event.cardId);
      return allCards;
  ...
  public getLostCard(
    ...
    return this.getCardLostRecord(player, current, inPhase, num).reduce<
      CardId[]
    >((allCards, event) => {
  ...
  public getObtainedCard(
    player: PlayerId,
    current?: RecordCurrentType,
    inPhase?: PlayerPhase[],
    num: number = 0
  ) {
    return this.getCardObtainedRecord(player, current, inPhase, num).reduce<
      CardId[]
    >((allCards, event) => {
      if (event.infos.length === 1) {
        for (const cardInfo of event.infos[0].movingCards) {
  ...
  public getDrawedCard(
    player: PlayerId,
    current?: RecordCurrentType,
    inPhase?: PlayerPhase[],
    num: number = 0
  ) {
    return this.getCardDrawRecord(player, current, inPhase, num).reduce<
      CardId[]
    >((allCards, event) => {
      if (event.infos.length === 1) {
        for (const cardInfo of event.infos[0].movingCards) {
  ...
  public getDroppedCard(
    player: PlayerId,
    current?: RecordCurrentType,
    inPhase?: PlayerPhase[],
    num: number = 0
  ) {
    return this.getCardDropRecord(player, current, inPhase, num).reduce<
      CardId[]
    >((allCards, event) => {
      if (event.infos.length === 1) {
        for (const cardInfo of event.infos[0].movingCards) {
  ...
  public getMovedCard(
    ...
    return this.getCardMoveRecord(player, current, inPhase, num).reduce<
      CardId[]
    >((allCards, event) => {
      if (event.infos.length === 1) {
        for (const cardInfo of event.infos[0].movingCards) {
