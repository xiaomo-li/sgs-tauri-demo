...
export class GameCommonRules {
  ...
  private isBannedBySideRules = (
    player: Player,
    cardOrMatcher: Card | CardMatcher
  ) => {
  ...
  public getCardUsableTimes(
    ...
    const baseRule = this.commonCardUseRules.find((rule) =>
    ...
    const additionalRule = this.userRules[user.Id].cards.filter((rule) =>
      rule.cardMatcher.match(card)
    );
    if (additionalRule) {
    ...
    for (const skill of user.getSkills<RulesBreakerSkill>("breaker")) {
      availableUseTimes += skill.breakCardUsableTimes(
        card instanceof Card ? card.Id : card,
        room,
        user
      );
      if (target) {
        availableUseTimes += skill.breakCardUsableTimesTo(
          card instanceof Card ? card.Id : card,
          room,
          user,
          target
  ...
  public getAdditionalHoldCardNumber(room: Room, user: Player) {
    ...
    for (const owner of room.getAlivePlayersFrom()) {
      for (const skill of owner.getSkills<GlobalRulesBreakerSkill>(
        "globalBreaker"
      )) {
  ...
  public addCardUsableTimes(
    cardMatcher: CardMatcher,
    times: number,
    user: Player
  ) {
    this.preCheck(user);
    const matchedRules = this.userRules[user.Id].cards.filter((cardProp) =>
      cardProp.cardMatcher.match(cardMatcher)
    );
  ...
  public addAdditionalUsableNumberOfTargets(
    card: Card,
    user: Player,
    additional: number
  ) {
  ...
  public removeCardRules(cardMatcher: CardMatcher, user: Player) {
    this.userRules[user.Id].cards = this.userRules[user.Id].cards.filter(
      (rule) => rule.cardMatcher.match(cardMatcher)
    );
  }
  ...
  public getCardAdditionalUsableDistance(
    ...
    user.getSkills<RulesBreakerSkill>("breaker").forEach((skill) => {
      times +=
        skill.breakCardUsableDistance(
          card instanceof Card ? card.Id : card,
          room,
          user
        ) +
        (target
          ? skill.breakCardUsableDistanceTo(
              card instanceof Card ? card.Id : card,
              room,
              user,
              target
    ...
    for (const player of room.getAlivePlayersFrom()) {
      let count = 0;
      player
        .getSkills<GlobalRulesBreakerSkill>("globalBreaker")
        .forEach((skill) => {
          count += skill.breakGlobalCardUsableDistance(
            card instanceof Card ? card.Id : card,
            room,
            player,
            user
  ...
  public getCardAdditionalNumberOfTargets(
    room: Room,
    user: Player,
    card: Card | CardMatcher
  ) {
    ...
    user.getSkills<RulesBreakerSkill>("breaker").forEach((skill) => {
      times += skill.breakCardUsableTargets(
        card instanceof Card ? card.Id : card,
        room,
        user
      );
  ...
  public getCardAdditionalAttackDistance(
    room: Room,
    user: Player,
    card?: Card | CardMatcher,
    target?: Player
    ...
    user.getSkills<RulesBreakerSkill>("breaker").forEach((skill) => {
      distance += skill.breakAttackDistance(
        card instanceof Card ? card.Id : card,
        room,
        user
      );
