...
import { PlayerInfo, PlayerRole } from "../../player/player_props";
import { Algorithm } from "../../shares/libs/algorithm";
import { pveLongShenSkills } from "../../skills/game_mode/pve/pve_longshen_skills";
...
export class PveLongshenGameProcessor extends PveClassicGameProcessor {
  ...
  protected async nextLevel() {
    ...
    const originSkills = Sanguosha.getCharacterByCharaterName(
      "pve_longshen"
    ...
    const bossPropertiesChangeEvent: ServerEventFinder<GameEventIdentifiers.PlayerPropertiesChangeEvent> =
      {
        changedProperties: [
          {
            toId: playersInfo.find((info) => info.Role === PlayerRole.Rebel)!
              .Id,
            characterId:
              Sanguosha.getCharacterByCharaterName("pve_longshen").Id,
          },
        ],
      };
