...
export class TwoVersusTwoGameProcessor extends StandardGameProcessor {
  ...
  public getWinners(players: Player[]) {
    const rebels = players.filter(
      (player) => player.Role === PlayerRole.Rebel && player.Dead
    );
    const loyalists = players.filter(
      (player) => player.Role === PlayerRole.Loyalist && player.Dead
    );
  ...
  protected async drawGameBeginsCards(playerInfo: PlayerInfo) {
    const cardIds = this.room.getCards(
      playerInfo.Position === 3 ? 5 : 4,
      "top"
    );
  ...
  protected async chooseCharacters(
    ...
    for (const response of await Promise.all(sequentialAsyncResponse)) {
      ...
      changedProperties.push({
        ...
        maxHp:
          playerInfo.Role === PlayerRole.Lord ? character.MaxHp + 1 : undefined,
  ...
  protected async onHandlePlayerDiedEvent(
    ...
    if (!this.room.isGameOver()) {
      ...
      const teammate = this.room.AlivePlayers.find(
        (player) => player.Role === deadPlayer.Role
      );
      if (teammate) {
