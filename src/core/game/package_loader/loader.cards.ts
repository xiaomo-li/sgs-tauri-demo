...
import { SkillsGeneratedCardPackage } from "../../cards/character_skills";
import { LegionFightCardPackage } from "../../cards/legion_fight";
import { StandardCardPackage } from "../../cards/standard";
    ...
    for (const [packageName, loader] of Object.entries(allPackageLoaders)) {
      ...
      for (const card of cards) {
        if (!card.isUniqueCard()) {
          ...
          const bySkill = Precondition.exists(
            card.generatedBySkill(),
            `unknown unique card generator: ${card.Name}`
          );
