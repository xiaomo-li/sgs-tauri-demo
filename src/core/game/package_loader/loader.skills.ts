...
import * as SkillList from "../../skills";
import { Skill, SkillPrototype } from "../../skills/skill";
...
export class SkillLoader {
  ...
  public getShadowSkillsByName<S extends Skill = Skill>(
    skillName: string
  ): S[] {
    const skills = this.shadowSkills.filter(
      (skill) => skill.GeneralName === skillName
    );
    return Precondition.exists(
      skills,
      `Unable to get shadow skills ${skillName}`
    ) as S[];
  }
