...
import { ServerPlayer } from "../player/player.server";
...
export class ServerSocket extends Socket<WorkPlace.Server> {
  ...
  constructor(
    ...
    this.socket.on("connection", (socket) => {
            ...
            switch (identifier) {
              ...
              case GameEventIdentifiers.UserMessageEvent:
                this.onPlayerMessage(
                  identifier,
                  content as ClientEventFinder<GameEventIdentifiers.UserMessageEvent>
                );
                break;
      ...
      serverResponsiveListenerEvents.forEach((identifier) => {
        socket.on(
          ...
          (content: ClientEventFinder<GameEventIdentifiers>) => {
            const mappedPlayerId = Object.entries(
              this.mapSocketIdToPlayerId
            ).find(([playerId, socketId]) => socketId === socket.id);
            if (!mappedPlayerId) {
              this.logger.info(
                `Unable to find playerId of socket ${socket.id}`
              );
              return;
            }
            ...
            content.status &&
              this.room!.updatePlayerStatus(content.status, playerId);

            const asyncResolver =
              this.asyncResponseResolver[identifier] &&
              this.asyncResponseResolver[identifier][playerId];
            if (asyncResolver) {
      ...
      socket.on("disconnect", () => {
        ...
        if (
          this.room &&
          this.room.Players.find((player) => player.Id === playerId) ===
            undefined
        ) {
          return;
        }
        ...
        if (
          room.Players.every(
            (player) => !player.isOnline() || player.isSmartAI()
          ) ||
        ...
        if (!room.isPlaying()) {
          ...
          const { identifier: awaitIdentifier, content } =
            this.room?.AwaitingResponseEvent[playerId]!;
          if (awaitIdentifier === undefined) {
            throw new Error(
              `Unknown event without identifier: ${JSON.stringify(
                this.room?.AwaitingResponseEvent[playerId]
              )}`
            );
          }
          ...
          const result = toPlayer.AI.onAction(
            this.room!,
            awaitIdentifier,
            content
          );
  ...
  private async onPlayerMessage(
    ...
    if (content.message) {
      ...
      this.broadcast(
        identifier,
        content as unknown as ServerEventFinder<GameEventIdentifiers.UserMessageEvent>
      );
    }
  }
  ...
  private async onObserverRequestChangingView(
    ...
    for (const player of this.room!.Players) {
      if (
        this.mapPlayerIdToObservers[player.Id]?.includes(content.observerId)
      ) {
        this.mapPlayerIdToObservers[player.Id] = this.mapPlayerIdToObservers[
          player.Id
        ].filter((id) => id !== content.observerId);
        break;
      }
    }

    this.mapPlayerIdToObservers[content.toObserverId] =
      this.mapPlayerIdToObservers[content.toObserverId] || [];
  ...
  private async onPlayerEnter(
    ...
    if (event.coreVersion !== Sanguosha.Version) {
      ...
      !event.joinAsObserver &&
      (room.isPlaying() || room.Info.numberOfPlayers <= room.Players.length)
    ) {
      joinFailed = true;
    }
    ...
    if (!event.joinAsObserver) {
      ...
      this.mapPlayerIdToObservers[observePlayer.Id] =
        this.mapPlayerIdToObservers[observePlayer.Id] || [];
  ...
  private async onPlayerLeave(
    ...
    if (
      room.Players.every((player) => !player.isOnline()) ||
      room.Players.length === 0
    ) {
      room.close();
      return;
  ...
  public notify<I extends GameEventIdentifiers>(
    ...
    if (!toPlayer.isOnline()) {
      ...
      this.socket
        .to(this.mapSocketIdToPlayerId[to])
        .emit(type.toString(), content);

      const observers = this.mapPlayerIdToObservers[to];
      if (observers) {
        for (const observer of observers) {
          this.socket
            .to(this.mapSocketIdToPlayerId[observer])
            .emit(type.toString(), content);
        }
      }
    }
