...
export class AimGroupUtil {
  ...
  public static getAllTargets(aimGroup: AimGroup): PlayerId[] {
    return [...aimGroup[AimStatus.Undone], ...aimGroup[AimStatus.Done]].reduce<
      PlayerId[]
    >((targets, target) => {
  ...
  public static getUndoneOrDoneTargets(
    aimGroup: AimGroup,
    done?: boolean
  ): PlayerId[] {
  ...
  public static addTargets(
    ...
    aimEvent.targetGroup &&
      TargetGroupUtil.pushTargets(aimEvent.targetGroup, playerIds);
  }

  public static cancelTarget(
    aimEvent: ServerEventFinder<GameEventIdentifiers.AimEvent>,
    playerId: PlayerId
  ) {
    for (const key of Object.keys(aimEvent.allTargets)) {
      aimEvent.allTargets[key] = (
        aimEvent.allTargets[key] as PlayerId[]
      ).filter((id) => id !== playerId);
    }
    aimEvent.allTargets[AimStatus.Cancelled].push(playerId);
    aimEvent.targetGroup &&
      TargetGroupUtil.removeTarget(aimEvent.targetGroup, playerId);
  }

  public static removeDeadTargets(
    room: Room,
    aimEvent: ServerEventFinder<GameEventIdentifiers.AimEvent>
  ) {
    aimEvent.allTargets[AimStatus.Undone] = room.deadPlayerFilters(
      aimEvent.allTargets[AimStatus.Undone]
    );
    aimEvent.allTargets[AimStatus.Done] = room.deadPlayerFilters(
      aimEvent.allTargets[AimStatus.Done]
    );
