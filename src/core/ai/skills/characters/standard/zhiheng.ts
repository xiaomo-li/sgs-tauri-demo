...
import type { ZhiHeng } from "../../../../skills";
  ...
  skillTrigger = (room: Room, ai: Player, skill: ZhiHeng) => {
    ...
    if (
      enemies.filter((enemy) => {
        ...
        return (
          equip !== undefined && Sanguosha.getCardById(equip).Name !== "tengjia"
        );
      }).length >=
      enemies.length / 2
    ) {
    ...
    const availableCards = zhihengCards.filter(
      (card) => !mustKeepCardNames.includes(Sanguosha.getCardById(card).Name)
    );
    return {
      fromId: ai.Id,
