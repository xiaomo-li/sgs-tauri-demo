  ...
  public onAskForCardUseEvent(
    ...
    if (target) {
      const filteredAvailableCards = availableCards.filter((card) =>
        room.canUseCardTo(card, ai, target)
      );

      if (filteredAvailableCards.length > 0) {
        const card =
          content.byCardId != null && Sanguosha.getCardById(content.byCardId);
        if (
          card &&
          !AiLibrary.areTheyFriendly(ai, target, room.Info.gameMode)
        ) {
          return {
            cardId: [
              "wugufengdeng",
      ...
      return {
        cardId: AiLibrary.areTheyFriendly(
          ai,
          room.getPlayerById(cardUserId),
          room.Info.gameMode
        )
          ? undefined
