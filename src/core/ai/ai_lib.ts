...
} from "../cards/libs/card_matcher";
import {
  CardChoosingOptions,
  CardColor,
  CardId,
  CardValue,
} from "../cards/libs/card_props";
import {
  ClientEventFinder,
...
export abstract class AiLibrary {
  ...
  static getCardValueofCard(cardId: CardId): CardValue {
    return (
      AiLibrary.standardCardValue.find(
        (cardValue) => cardValue.cardName === Sanguosha.getCardById(cardId).Name
      ) || {
        value: 50,
        wane: 0.5,
        priority: 50,
  ...
  static sortCardsValuePriority(cardIds: CardId[]): CardId[] {
    return cardIds.sort(
      (a, b) =>
        AiLibrary.getCardValueofCard(a).value -
        AiLibrary.getCardValueofCard(b).value
    );
  }
  ...
  static sortTargetEquipsInDefense(room: Room, ai: Player, to: Player) {
    const equipValues: [CardId, number][] = to
      ...
      .map((equip) => [
        equip,
        AiLibrary.equipStaticDefenseValue[Sanguosha.getCardById(equip).Name],
      ]);
    ...
    for (const skill of skills) {
      actionItems =
        skill
          ...
          ?.dynamicallyAdjustSkillUsePriority(room, ai, skill, actionItems) ||
        actionItems;
    }
  ...
  static shouldUseWuXieKeJi(
    ...
    if (askedByCard.Name === "wanjianqifa") {
      if (
        cardUseFrom &&
        room.CurrentPhasePlayer.Id === cardUseFrom &&
        !room.CurrentPhasePlayer.hasUsed("slash")
      ) {
        return (
          AiLibrary.findCardsByMatcher(
            room,
            player,
    ...
    return (
      cardUseFrom !== undefined &&
      !this.areTheyFriendly(
        room.getPlayerById(cardUseFrom),
        player,
        room.Info.gameMode
      )
    );
  ...
  static shouldUseJink(
    ...
    if (askedByCard) {
      ...
      if (askedByCard.GeneralName === "slash") {
        ...
        if (
          useFrom &&
          useFrom.getCardIds(PlayerCardsArea.HandArea).length > 3
        ) {
          return availableCards.length >= 1;
        }
        return true;
  ...
  static getUsableSlashesTo(room: Room, ai: Player, to: Player) {
    return AiLibrary.findAvailableCardsToUse(
      room,
      ai,
      new CardMatcher({ generalName: ["slash"] })
    ).filter(
      (cardId) =>
        room.canAttack(ai, to, cardId) && room.canUseCardTo(cardId, ai, to)
    );
  }

  static getAttackWillEffectSlashesTo(
    room: Room,
    ai: Player,
    to: Player,
    slashes?: CardId[]
    ...
    const shield =
      shieldId !== undefined ? Sanguosha.getCardById(shieldId) : undefined;
    slashes = slashes || AiLibrary.getUsableSlashesTo(room, ai, to);

    const weaponId = ai.getEquipment(CardType.Weapon);
    const weapon =
      weaponId !== undefined ? Sanguosha.getCardById(weaponId) : undefined;
    ...
    if (shield) {
      if (shield.Name === "tengjia") {
        slashes = slashes.filter(
          (slash) => Sanguosha.getCardById(slash).Name !== "slash"
        );
      } else if (shield.Name === "renwangdun") {
        slashes = slashes.filter(
          (slash) => Sanguosha.getCardById(slash).Color !== CardColor.Black
        );
      }
    }
  ...
  static askAiUseCard(
    ...
    if (byCardId !== undefined) {
      if (cardMatcher.name?.includes("wuxiekeji")) {
        return AiLibrary.shouldUseWuXieKeJi(
          room,
          aiPlayer,
          availableCards,
          byCardId,
        ...
        return AiLibrary.shouldUseJink(
          room,
          aiPlayer,
          availableCards,
          byCardId,
  ...
  static askAiChooseCardFromPlayer<
  ...
  >(
    ...
    const areas = Object.keys(options).map(
      (area) => Number(area) as PlayerCardsArea
    );
        ...
        selectedCard = cardIds.find((cardId) =>
          Sanguosha.getCardById(cardId).is(
            equipTypePriority.find(
              (equipType) =>
                cardIds.filter((c) => Sanguosha.getCardById(c).is(equipType))
                  .length > 0
            )!
          )
        );
  ...
  static sortCardbyValue(cards: CardId[], descend: boolean = true): CardId[] {
    const cardIds = cards.reduce<CardsValue[]>((allCardsValue, cardId) => {
      ...
      const value =
        cardValue.value *
        cardValue.wane **
          allCardsValue.filter(
            (s) =>
              Sanguosha.getCardById(s.cardId).Name ===
              Sanguosha.getCardById(cardId).Name
          ).length;

      allCardsValue.push({ cardId, value });
      allCardsValue.sort((a, b) =>
        descend ? b.value - a.value : a.value - b.value
      );
  ...
  static findCardsByMatcher(
    ...
    const cards = fromAreas.reduce<CardId[]>((savedCards, area) => {
      const areaCards = player
        ...
        .filter((card) =>
          cardMatcher ? cardMatcher.match(Sanguosha.getCardById(card)) : true
        );
  ...
  static findAvailableCardsToUse(
    room: Room,
    player: Player,
    cardMatcher?: CardMatcher
  ) {
    let cards = AiLibrary.findCardsByMatcher(room, player, cardMatcher).filter(
      (cardId) => player.canUseCard(room, cardId, cardMatcher)
    );

    if (!cardMatcher) {
      cards = cards.filter(
        (card) =>
          !(Sanguosha.getCardById(card).Skill instanceof ResponsiveSkill)
      );
    }

    for (const skill of player.getSkills<FilterSkill>("filter")) {
      cards = cards.filter((cardId) =>
        skill.canUseCard(cardId, room, player.Id)
      );
    }
    ...
    for (const skill of viewAsSkills) {
      ...
      if (avaiableViewAs) {
        const canViewAs = skill.canViewAs(
          room,
          player,
          avaiableViewAs,
          cardMatcher
  ...
  static findAvailableCardsToResponse(
    ...
    for (const skill of player.getSkills<FilterSkill>("filter")) {
      cards = cards.filter((cardId) =>
        skill.canUseCard(cardId, room, player.Id, onResponse)
      );
    }
  ...
  static getPlayerAbsoluteDefenseValue(player: Player) {
    ...
    if (
      equips.find((card) => Sanguosha.getCardById(card).is(CardType.Shield)) ||
  ...
  static getPlayerRelativeDefenseValue(from: Player, to: Player) {
    ...
    if (
      equips.find((card) =>
        Sanguosha.getCardById(card).is(CardType.OffenseRide)
      )
    ) {
    ...
    if (
      equips.find((card) => Sanguosha.getCardById(card).is(CardType.Weapon))
    ) {
      targetDefenseValue -= 2;
    }
    if (
      equips.find((card) => Sanguosha.getCardById(card).Name === "qinggang")
    ) {
  ...
  static sortEnemiesByRole(room: Room, from: Player) {
    let enemies = room
      ...
      .filter(
        (other) => !this.areTheyFriendly(other, from, room.Info.gameMode)
      );

    if (
      room.Info.gameMode === GameMode.Standard &&
      from.Role === PlayerRole.Renegade
    ) {
      enemies = enemies.filter((enemy) => enemy.Role === PlayerRole.Lord);
    }

    return enemies.sort((enemyA, enemyB) => {
      const defenseValueA = AiLibrary.getPlayerRelativeDefenseValue(
        from,
        enemyA
      );
      const defenseValueB = AiLibrary.getPlayerRelativeDefenseValue(
        from,
        enemyB
      );
  ...
  static areTheyFriendly(playerA: Player, playerB: Player, mode: GameMode) {
    ...
    switch (mode) {
      ...
      case GameMode.OneVersusTwo: {
        if (
          playerA.Role === PlayerRole.Lord ||
          playerB.Role === PlayerRole.Lord
        ) {
          return false;
        }
      ...
      case GameMode.Standard: {
        if (
          playerA.Role === PlayerRole.Lord ||
          playerA.Role === PlayerRole.Loyalist
        ) {
          return (
            playerB.Role === PlayerRole.Lord ||
            playerB.Role === PlayerRole.Loyalist
          );
        } else if (
          playerA.Role === PlayerRole.Rebel ||
          playerA.Role === PlayerRole.Renegade
        ) {
          return (
            playerB.Role === PlayerRole.Rebel ||
            playerB.Role === PlayerRole.Renegade
          );
        }
        break;
      }
