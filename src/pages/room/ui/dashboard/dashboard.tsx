...
export class Dashboard extends React.Component<DashboardProps> {
  ...
  componentDidUpdate() {
    ...
    if (this.handBoardRef.current && presenter.ClientPlayer) {
      ...
      const numOfHandCards =
        presenter.ClientPlayer.getCardIds(PlayerCardsArea.HandArea).length +
        this.AvailableOutsideCards.length;
      if (width < numOfHandCards * (this.handCardWidth + this.cardMargin)) {
        this.cardOffset =
          -(numOfHandCards * (this.handCardWidth + this.cardMargin) - width) /
          (numOfHandCards - 1);
      } else {
  ...
  getEquipCardsSection() {
    ...
    const abortedSections =
      this.props.presenter.ClientPlayer?.DisabledEquipSections;
      ...
      <div className={styles.equipSection}>
        ...
        {equipCards?.map((card) => (
          <EquipCardItem
            ...
            disabled={
              !this.props.cardSkillEnableMatcher ||
              !this.props.cardSkillEnableMatcher(card)
            }
  ...
  get AvailableOutsideCards() {
    ...
    for (const [areaName, cards] of Object.entries(
      this.props.presenter.ClientPlayer.getOutsideAreaCards()
    )) {
      ...
      availableCards.push(
        ...cards
          .filter((card) =>
            this.props.outsideCardEnableMatcher!(Sanguosha.getCardById(card))
          )
          .map((cardId) => ({
            areaName,
  ...
  getHandCardTags(cardId: CardId) {
    ...
    for (const [cardTag, cardIds] of Object.entries(
      this.props.presenter.ClientPlayer.getAllCardTags()
    )) {
  ...
  getCardXOffset(index: number) {
    ...
    if (
      this.cardOffset !== this.cardMargin &&
      this.onFocusCardIndex >= 0 &&
      index > this.onFocusCardIndex
    ) {
      offsetX =
        index * (this.handCardWidth + this.cardOffset) -
        this.cardOffset +
        this.cardMargin;
    }
    return offsetX;
  ...
  get AllClientHandCards() {
    const outsideCards = this.AvailableOutsideCards.map((cardInfo, index) => {
      const isSelected = this.props.store.selectedCards.includes(
        cardInfo.card.Id
      );
      const isDisabled =
        !this.props.outsideCardEnableMatcher ||
      ...
      return (
        <ClientCard
          ...
          highlight={this.props.store.highlightedCards}
          onMouseEnter={
            isSelected || isDisabled ? undefined : this.onFocusCard(index)
          }
    ...
    const handcards =
      this.props.presenter.ClientPlayer?.getCardIds(PlayerCardsArea.HandArea)
        .filter(
          (cardId) =>
            !this.props.handcardHiddenMatcher ||
            !this.props.handcardHiddenMatcher(Sanguosha.getCardById(cardId))
        )
        .map((cardId, index) => {
            ...
            <ClientCard
              ...
              onMouseEnter={
                isSelected || isDisabled
                  ? undefined
                  : this.onFocusCard(index + outsideCards.length)
              }
              onMouseLeave={this.onFocusCard(-2)}
  ...
  getPlayerJudgeCards() {
    const judgeAreaDisabled =
      this.props.presenter.ClientPlayer?.judgeAreaDisabled();
      ...
      <div className={styles.handBoard} ref={this.handBoardRef}>
        {this.props.store.inAction && (
          <PlayingBar
            ...
            playTime={this.props.store.notificationTime}
          />
        )}
        {this.getPlayerJudgeCards()}
        <div className={styles.userActionsButtons}>
          {!this.props.observerMode && buttons}
        </div>
  ...
  private readonly onSortHandcards = () => {
    const handcards =
      this.props.presenter.ClientPlayer?.getCardIds(PlayerCardsArea.HandArea) ||
      [];
    this.props.presenter.ClientPlayer?.setupCards(
      PlayerCardsArea.HandArea,
      Functional.sortCards(handcards)
    );
      ...
      <div className={styles.actionButtons}>
        ...
        <Button
          ...
          disabled={
            ...
            this.props.store.room.CurrentPlayerPhase !==
  ...
  render() {
    ...
    return (
      <div className={styles.dashboard} id={this.props.store.clientPlayerId}>
        ...
        {this.props.store.room.CurrentPlayer === player &&
          this.props.store.room.CurrentPlayerPhase !== undefined && (
            <PlayerPhaseBadge
        ...
        <PlayerAvatar
          ...
          delight={
            this.props.store.delightedPlayers !== undefined
              ? this.props.store.delightedPlayers
              : undefined
          }
          disabled={
