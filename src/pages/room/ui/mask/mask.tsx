  ...
  return (
    ...
    >
      <img
        ...
        alt={""}
        src={getMaskImage(gameMode, role)}
      />
    </div>
  );
};
...
export const Mask = (props: MaskProps) => {
  const {
    hideDisplay,
    displayedRole = PlayerRole.Unknown,
    className,
    lockedRole,
    gameMode,
  } = props;
    ...
    <div className={className} onClick={onMaskClick}>
      <div
        className={classNames(styles.displayedRole, {
          [styles.lord]:
            lockedRole === PlayerRole.Lord || role === PlayerRole.Lord,
          [styles.loyalist]:
            lockedRole === PlayerRole.Loyalist || role === PlayerRole.Loyalist,
          [styles.rebel]:
            lockedRole === PlayerRole.Rebel || role === PlayerRole.Rebel,
          [styles.renegade]:
            lockedRole === PlayerRole.Renegade || role === PlayerRole.Renegade,
        })}
      >
        <img
          className={styles.maskImage}
          alt={""}
          src={getMaskImage(gameMode, lockedRole || role)}
        />
      </div>
      <AllMasks
        onClick={lockedRole ? undefined : onClick}
        opened={!lockedRole && maskSwitch}
        gameMode={gameMode}
      />
    </div>
  );
