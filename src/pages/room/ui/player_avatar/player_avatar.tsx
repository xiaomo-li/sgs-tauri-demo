...
export class PlayerAvatar extends React.Component<PlayerAvatarProps> {
  ...
  private readonly onClickSkill = (skill: Skill) => () => {
    if (
      this.props.store.selectedSkill !== undefined &&
      skill !== this.props.store.selectedSkill
    ) {
      return;
    }
    ...
    this.props.onClickSkill &&
      this.props.onClickSkill(skill, this.skillSelected);
  };
    ...
    return (
      <>
        <div className={styles.playerSkills}>
          {skills.map((skill, index) => (
            <SkillButton
              ...
              skill={skill}
              selected={
                this.getSkillSelected() &&
                this.props.store.selectedSkill === skill
              }
              size={
                skills.length % 2 === 0
                  ? "normal"
        ...
        <div className={styles.userSideEffectSkillList}>
          {this.getSideEffectSkills()}
        </div>
      </>
    );
  }

  private readonly onClickUniqueSkillTag =
              ...
              <span
                ...
                onClick={
                  items && items.length > 0
                    ? this.onClickUniqueSkillTag(flag, items)
                    : undefined
                }
              >
                {translator.trx(flag)}
  ...
  private readonly onOutsideAreaTagClicked =
    (name: string, cards: (CardId | CharacterId)[]) => () => {
      ...
      if (
        ...
        (this.openedDialog === undefined &&
          this.props.store.selectorDialog !== undefined)
      ) {
        return;
      }
      ...
      .map((skillName, index) => {
        ...
        return (
          <SkillButton
            ...
            disabled={
              this.props.store.room.isGameOver() || isSkillDisabled(skill)
            }
            onClick={this.onClickSkill(skill)}
          />
        );
      ...
      ? this.hideRelatedSkills
        ? skills.length > 0 && (
            <span
              ...
              onClick={this.hideOrShowRelatedSkills}
            >
              ...
              <span
                dangerouslySetInnerHTML={{
                  __html: translator.tr(
                    skill.dynamicDescription(presenter.ClientPlayer!)
                  ),
                }}
              />
            </div>
  ...
  private renderCharacterImage() {
    if (
      ...
      if (this.props.skinData) {
        ...
        this.props.imageLoader
          .getCharacterImage(this.props.presenter.ClientPlayer.Character.Name)
          .then((image) => {
      ...
      const huashenCharacterId =
        this.props.presenter.ClientPlayer.getHuaShenInfo()?.characterId;
  ...
  private getOnceSkillMarks() {
    ...
    marks.push(
      ...limitSkills.map((skill) => (
        <LimitSkillMark
          ...
          hasUsed={this.props.store.onceSkillUsedHistory[
            clientPlayer.Id
          ]?.includes(skill.Name)}
          key={skill.Name}
          tagPosition="left"
    ...
    marks.push(
      ...switchSkills.map((skill) => (
        <SwitchSkillMark
          skillName={this.props.translator.tr(skill.Name)}
          state={this.props.store.switchSkillState[clientPlayer.Id]?.includes(
            skill.Name
          )}
          key={skill.Name}
          tagPosition="left"
        ...
        >
          ...
          {!clientPlayer?.isFaceUp() && (
            <Picture
              ...
              image={this.props.imageLoader.getTurnedOverCover()}
            />
          )}
          ...
          {clientPlayer && clientPlayer.ChainLocked && (
            <Picture
              ...
              image={this.props.imageLoader.getChainImage()}
            />
          )}
