...
export class SeatsLayout extends React.Component<SeatsLayoutProps> {
  ...
  constructor(props: SeatsLayoutProps) {
    ...
    this.topNumberOfPlayers =
      this.numberOfPlayers - this.sideNumberOfPlayers * 2;
  }
  ...
  private readonly renderRequestViewButton = (player: Player) =>
    this.props.observerMode && (
      <Button
        variant="primary"
        onClick={this.onRequestView(player)}
  ...
  private getLeftPlayers() {
    ...
    while (numberOfPlayers > 0) {
      const player = this.props.store.room.Players[playerIndex] as
        | ClientPlayer
        | undefined;
      ...
      players.unshift(
        <div className={styles.playerCard}>
          <PlayerCard
            ...
            inAction={
              player !== undefined &&
              this.props.store.notifiedPlayers.includes(player.Id)
            }
            incomingMessage={
              player && this.props.store.incomingUserMessages[player.Id]
            }
            onCloseIncomingMessage={
              player && this.onCloseIncomingMessage(player)
            }
