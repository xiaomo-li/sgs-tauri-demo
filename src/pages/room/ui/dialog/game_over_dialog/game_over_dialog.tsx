    ...
    <div className={classNames(styles.players, props.className)}>
      <div className={styles.title}>
        <span className={styles.username}>
          {props.translator.tr(Messages.playerName())}
        </span>
        <span className={styles.characterName}>
          {props.translator.tr(Messages.characterName())}
        </span>
        <span className={styles.role}>
          {props.translator.tr(Messages.role())}
        </span>
        <span className={styles.status}>
          {props.translator.tr(Messages.status())}
        </span>
        <span className={styles.handcards}>
          {props.translator.tr(Messages.handcards())}
        </span>
      </div>
      {props.players.map((player) => (
        <div className={styles.player} key={player.Id}>
          ...
          <span className={styles.characterName}>
            {props.translator.tr(player.Character.Name)}
          </span>
          <span className={styles.role}>
            {props.translator.tr(
              Functional.getPlayerRoleRawText(player.Role, props.gameMode)
            )}
          </span>
          <span className={styles.status}>
            {props.translator.tr(
              player.Dead ? Messages.dead() : Messages.alive()
            )}
          </span>
          <span className={styles.handcards}>
            <Button
              variant="primary"
              onMouseEnter={onSetHandCards(player)}
              onMouseLeave={onClearHandCards}
            >
    ...
    <div className={styles.gameOverBoard}>
      <div className={styles.winners}>
        <span className={styles.title}>
          {translator.tr(Messages.winners())}
        </span>
        <PlayerInfoTable
          imageLoader={imageLoader}
          players={winners}
