...
const CardSlot = (props: {
  ...
  return (
    <ClientCard
      ...
      className={classNames(styles.selectorCard, {
        [styles.selected]: selected,
      })}
      card={props.card}
      translator={props.translator}
      disabled={
        props.card && props.isCardDisabled
          ? props.isCardDisabled(props.card)
          : false
      }
...
const CardSelector = (props: CardSelectorProps) => {
  ...
  if (options instanceof Array || typeof options === "number") {
    ...
    for (const [area, cardIds] of Object.entries(options)) {
      if (
        cardIds === undefined ||
        (cardIds instanceof Array && cardIds.length === 0)
      ) {
        continue;
      }
      ...
      optionCardsLine.push(
        <div className={styles.cardLine} key={optionCardsLine.length}>
          <span className={styles.listName}>
            {translator.tr(
              Number.isNaN(fromArea)
                ? area
                : Functional.getPlayerCardAreaText(fromArea)
            )}
          </span>
