...
export class GuanXingCardSlots extends React.Component<GuanXingDialogProps> {
  ...
  private cardStyles: React.CSSProperties[] = new Array(
  ...
  UNSAFE_componentWillMount() {
    if (this.props.onConfirm) {
      this.props.presenter.defineConfirmButtonActions(
        this.props.onConfirm(this.topCards, this.bottomCards)
      );
    }
  ...
  private updateCardOffset(
    ...
    if (from === to) {
      const minMovingRange = this.getCardLeftOffset(
        Math.min(originalIndex, targetIndex)
      );
      const maxMovingRange = this.getCardLeftOffset(
      ...
      for (const cardId of Object.keys(this.cardPositions)) {
        ...
        if (
          this.cardPositions[cardId].top ===
            this.cardPositions[movingCard.Id].top &&
      ...
      for (const cardId of Object.keys(this.cardPositions)) {
        ...
        if (
          this.cardPositions[cardId].top ===
            this.cardPositions[movingCard.Id].top &&
          this.cardPositions[cardId].left >=
            this.cardPositions[movingCard.Id].left
        ) {
          this.cardPositions[cardId].left -= this.getCardLeftOffset(1);
        } else if (
          this.cardPositions[cardId].top !==
  ...
  private getCardPositionIndex(card: Card) {
    return Math.floor(
      this.cardPositions[card.Id].left / (this.cardWidth + this.cardMargin * 2)
    );
  }
  ...
  private calculateMovingPosition(
    card: Card,
    top: number,
    left: number
    ...
    let maxLength =
      to === "top" ? this.topCards.length : this.bottomCards.length;
    if (from === to) {
      maxLength--;
    }
    ...
    const index = Math.max(
      Math.min(
        currentIndex + Math.round(left / (this.cardMargin + this.cardWidth)),
        maxLength
      ),
      0
    );
  ...
  private addToStack(card: Card, place: "top" | "bottom", index: number) {
    if (place === "top") {
      if (place !== this.getCardStack(card)) {
        ...
        const deleteIndex = this.bottomCards.findIndex(
        ...
        const swapIndex = this.topCards.findIndex(
          (seekingCard) => seekingCard === card
        );
        if (swapIndex >= 0) {
          [this.topCards[swapIndex], this.topCards[index]] = [
            this.topCards[index],
            this.topCards[swapIndex],
          ];
        }
      }
    } else {
      if (place !== this.getCardStack(card)) {
        ...
        const deleteIndex = this.topCards.findIndex(
          (seekingCard) => seekingCard === card
        );
        if (deleteIndex >= 0) {
        ...
        const swapIndex = this.bottomCards.findIndex(
          (seekingCard) => seekingCard === card
        );
        if (swapIndex >= 0) {
    ...
    (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
      ...
      if (this.bottomCards.includes(card)) {
        ...
        if (
          ...
          this.bottomCards.length === bottomMaxCard
        ) {
          canMove = false;
        }
      }
  ...
  onMouseUp =
    ...
    (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
      ...
      const left = e.clientX - this.movingCardPosition.x;
      const top = e.clientY - this.movingCardPosition.y;
      const { to: place, index: targetIndex } = this.calculateMovingPosition(
        card,
        top,
        left
      );
  ...
  private canConfirm() {
    ...
    if (
      bottomMinCard !== undefined &&
      this.bottomCards.length < bottomMinCard
    ) {
      canConfirm = false;
    }
        ...
        <div className={styles.topCards}>
          {cards.map((card, index) => (
            <ClientCard
              ...
              onMouseLeave={
                movable ? this.onMouseLeave(card, index) : undefined
              }
              offsetLeft={
                this.cardPositions[card.Id] && this.cardPositions[card.Id].left
              }
              offsetTop={
                this.cardPositions[card.Id] && this.cardPositions[card.Id].top
              }
