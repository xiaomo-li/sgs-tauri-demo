...
export class MessageDialog extends React.Component<MessageDialogProps> {
  ...
  private readonly onClickSendButton = (
    ...
    if (this.currentTab === "room") {
      ...
      this.props.connectionService.Chat.send(
        this.textMessage!,
        this.props.presenter.ClientPlayer!.Name
      );
    }
  ...
  componentDidMount() {
    this.chatMessages = this.props.connectionService.Chat.chatHistory().map(
      (message, index) => {
  ...
  componentDidUpdate() {
    if (this.userMessageDialogElementRef.current) {
      this.userMessageDialogElementRef.current.scrollTop =
        this.userMessageDialogElementRef.current.scrollHeight;
    }
  }
  ...
  private getSkillAudios() {
    return this.props.presenter.ClientPlayer !== undefined &&
      this.props.presenter.ClientPlayer.Character
  ...
  render() {
    return (
      <div className={styles.chat}>
        ...
        <div
          ...
          style={{
            ...
            height: this.userMessageDialogElementRef.current
              ? `${
                  this.userMessageDialogElementRef.current.clientHeight +
        ...
        <form
          className={classNames(styles.inputLabel, this.props.className)}
          onSubmit={this.onClickSendButton}
        >
          {!this.hideQuickChatItems ? (
            <div className={styles.quickChat}>
              {[...this.getSkillAudios(), ...this.getQuickChatContents()].map(
                (content, index) => (
                  <div>
                    <span
                      key={index}
                      className={styles.quickChatItems}
                      onClick={this.onClickQuickChatItem(content)}
                    >
          ...
          <Input
            ...
            placeholder={this.props.translator.tr(
          ...
          <Button
            ...
            variant="primary"
            type="button"
            onClick={this.onClickQuickChatButton}
          >
            💬
          </Button>
          <Button
