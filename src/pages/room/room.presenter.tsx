...
export class RoomPresenter {
    ...
    if (this.store.displayedCards.length > 7) {
      ...
      setTimeout(
        mobx.action(() => {
          ...
          this.store.displayedCards = this.store.displayedCards.filter(
            (card) => !deletedCards.includes(card)
          );
        }),
        600
      );
  ...
  buryCards(...cards: CardId[]) {
    for (const cardId of cards) {
      const exist = this.store.displayedCards.find(
        (_card) => cardId === _card.card.Id
      );
      if (exist) {
        exist.buried = true;
      }
  ...
  private getCardElementPosition(cardId: CardId): Point {
    ...
    const offset =
      this.store.displayedCards.length * 120 > 600
        ? 300
        : this.store.displayedCards.length * 60;
    return {
  ...
  createCardCategoryDialog = (
    ...
    this.store.selectorDialog = (
      <CardCategoryDialog imageLoader={this.imageLoader} {...props} />
    );
  };
  ...
  unselectPlayer(player: ClientPlayer) {
    const index = this.store.selectedPlayers.findIndex(
  ...
  switchSkillStateChanged(
    ...
    if (!this.store.switchSkillState[player]) {
      ...
      const index = this.store.switchSkillState[player].findIndex(
        (name) => name === skillName
      );
