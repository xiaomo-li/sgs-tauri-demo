...
export class MoveCard extends React.Component<MoveCardProps> {
  ...
  private calculateDisplayedCardOffset(totalCards: number, index: number) {
    ...
    const innerOffset =
      Math.min(
        this.cardWidth * totalCards + this.cardMargin * (totalCards + 1),
        containerWidth
      ) /
        2 -
      this.cardWidth / 2;
    if (containerWidth < totalCards * (this.cardWidth + this.cardMargin)) {
      const offset =
        (totalCards * (this.cardWidth + this.cardMargin) - containerWidth) /
        (totalCards - 1);
      return (
        (totalCards - index - 1) * (this.cardMargin + this.cardWidth - offset) -
        innerOffset
      );
    } else {
      return (
        (totalCards - index - 1) * (this.cardMargin + this.cardWidth) +
        this.cardMargin * 2 -
        innerOffset
      );
    }
      ...
      <div className={styles.displayedCards} ref={this.displayedCardsRef}>
        {this.props.store.displayedCards.map((displayCard, index) => (
          <ClientCard
            ...
            offsetLeft={this.calculateDisplayedCardOffset(
              this.props.store.displayedCards.length,
              index
            )}
            ...
            style={
              this.props.store.displayedCardsAnimationStyles[
                displayCard.card.Id
              ]
            }
          />
