...
export class MoveInstantCardAnimation extends UiAnimation {
  ...
  private async animateCardMove(
    ...
    for (const info of content.infos) {
      ...
      if (
        ...
        movingCards.find(
          (card) => card.fromArea === CardMoveArea.ProcessingArea
        )
      ) {
        return;
      }
      ...
      if (fromId) {
        const position = this.store.animationPosition.getPosition(
          fromId,
          fromId === this.store.clientPlayerId
        );
        animationStyles.transform = `translate(${position.x - leftOffset}px, ${
          position.y - this.cardHeight / 2
        }px)`;
      } else {
        animationStyles.transform = `translate(${
          this.CentralPosition.x - leftOffset
        }px, ${this.CentralPosition.y - this.cardHeight / 2}px)`;
      }
      ...
      await UiAnimation.play(100, () => {
        const toPosition = this.store.animationPosition.getPosition(
          toId,
          toId === this.store.clientPlayerId
        );
        const leftOffset = (this.cardWidth + (elements.length - 1) * 16) / 2;
        ...
        ReactDOM.render(
          <div
            ...
            style={{
              opacity: 1,
              transform: `translate(${toPosition.x - leftOffset}px, ${
                toPosition.y
              }px)`,
            }}
          >
  ...
  async animateCardUse(
    ...
    }px, ${position.y - this.cardHeight / 2}px)`;
    ...
    await UiAnimation.play(100, () => {
      ReactDOM.render(
        <div
          className={styles.movingCards}
          style={{ ...animationStyles, opacity: 1 }}
        >
          {elements}
        </div>,
        container
