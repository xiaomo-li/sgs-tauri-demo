...
export class GameClientProcessor {
  ...
  protected eventFilter<T extends GameEventIdentifiers>(
    ...
    if (
      ...
      this.store.room.Analytics.record(
        event,
        this.store.room.CurrentPlayerPhase
      );
  ...
  protected record<T extends GameEventIdentifiers>(
    ...
    if (
      ...
      this.electron.sendReplayEventFlow(event, {
        ...
        playersInfo: this.store.room.Players.map((p) => ({
          Id: p.Id,
          Name: p.Name,
          Position: p.Position,
        })),
  ...
  public onPlayTrustedAction() {
    ...
    if (identifier === undefined) {
      ...
      const result = this.presenter.ClientPlayer!.AI.onAction(
        this.store.room,
        identifier,
        event!
      );
  ...
  private doTrustedAction() {
    this.onPlayTrustedActionTimer = setTimeout(
      ...
      this.store.notificationTime *
        (this.presenter.ClientPlayer!.isTrusted() ? 0 : 1000)
    );
  }
  ...
  >(type: T, content: ServerEventFinder<T>) {
    ...
    if (matchArray) {
      ...
      this.presenter.onIncomingMessage(
        content.playerId,
        this.translator.tr(content.originalMessage)
      );
  ...
  >(type: T, content: ServerEventFinder<T>) {
    if (content.randomPinDianCard) {
      const handcards = this.presenter.ClientPlayer!.getCardIds(
  ...
  >(type: T, content: ServerEventFinder<T>) {
    if (content.skillName !== undefined) {
      this.store.room.installSideEffectSkill(
  ...
  >(type: T, content: ServerEventFinder<T>) {
    Precondition.assert(
      this.store.clientRoomInfo !== undefined,
    ...
    if (
      ...
      this.store.room.broadcast(GameEventIdentifiers.PlayerReadyEvent, {
  ...
  >(type: T, content: ServerEventFinder<T>) {
    ...
    this.store.room.CommonRules.syncSocketObject(
  ...
  >(type: T, content: ServerEventFinder<T>) {
    await this.store.room.obtainSkill(
      content.toId,
      content.skillName,
      false,
      content.insertIndex
        ...
        if (skillsUsed.length > 0) {
          for (const skill of skillsUsed) {
            if (player.hasUsedSkill(skill)) {
              ...
              reaSkill.isRefreshAt(this.store.room, player, content.to) &&
                ...
                movedByReason === CardMovedBySpecifiedReason.JudgeProcess
                  ? TranslationPack.translationJsonPatcher(
                      ...
                      TranslationPack.patchPlayerInTranslation(
                        this.store.room.getPlayerById(proposer!)
                      )
                    ).toString()
                  : toArea === CardMoveArea.DropStack
                  ? "move to drop stack"
  ...
  protected onHandleJudgeEvent<T extends GameEventIdentifiers.JudgeEvent>(
    ...
    const displayedCard = this.store.displayedCards.find(
  ...
  >(type: T, content: ServerEventFinder<T>) {
    ...
    const involvedTargets = content.involvedTargets?.map((target) =>
      this.store.room.getPlayerById(target)
    );
    const matcher =
    ...
    this.presenter.createDialog(
      <GameOverDialog
        ...
        onBackingToWaitingRoom={
          this.store.room.Info.campaignMode
            ? undefined
  ...
  >(type: T, content: ServerEventFinder<T>) {
    ...
    this.presenter.createDialog(
      <WuGuFengDengDialog
        ...
        translator={this.translator}
        onClick={
          this.store.clientPlayerId === content.toId && !this.isObserver
            ? onClick
            : undefined
        }
      />
