...
class ClickableSeat extends React.Component<{
  ...
  private async doPlayersUpdate() {
    if (
      !this.props.seatInfo.seatDisabled &&
      this.props.seatInfo.playerAvatarId
    ) {
      const latestAvatar = await this.props.avatarService.getAvatarByIndex(
        this.props.seatInfo.playerAvatarId
      );
      ...
      >
        {!this.props.seatInfo.seatDisabled && (
          <span className={styles.playerName}>
            {this.props.seatInfo.playerName}
          </span>
        )}
        <Picture
        ...
        />
        {!this.props.seatInfo.seatDisabled &&
          this.props.seatInfo.playerReady && (
            <StatusBadge
              className={styles.userGetReady}
              text={this.props.translator.tr(Messages.ready())}
            />
          )}
      </span>
    );
...
export class Seats extends React.Component<SeatsProps> {
  ...
  onClickSeat = (
    ...
    if (
      this.props.store.selfPlayerId === kickedPlayerId ||
      !this.props.isHost
    ) {
      return;
    }

    this.props.senderService.kickPlayerOrCloseSeat(
      seatId,
      closeSeat,
      kickedPlayerId
    );
  ...
  @mobx.computed
  get isEveryoneReady() {
    return this.props.store.seats.every((seat) =>
  ...
  get countPlayers() {
    ...
    return this.props.store.seats.filter(
      (seat) => !seat.seatDisabled && seat.playerId != null
    ).length;
  }

  private readonly transferHost = (playerId: PlayerId) => () => {
    this.props.senderService.giveHostTo(
      this.props.store.selfPlayerId,
      playerId
    );
  };
  ...
  createSeats = () => {
    ...
    for (const seat of this.props.store.seats) {
      const hostControl =
        this.props.isHost &&
        this.props.store.selfPlayerId !== (seat as any).playerId;
      seatComponents.push(
  ...
  private readonly getReady = () => {
    ...
    this.props.senderService.getReady(
      this.props.store.selfPlayerId,
      this.isReady
    );
  };
