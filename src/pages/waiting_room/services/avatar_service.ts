...
export class RoomAvatarService {
  ...
  private readonly avatarIndexMap = Sanguosha.getAllCharacters().map(
  ...
  public getAvatarByIndex(index: number) {
    Precondition.assert(
      index < this.avatarIndexMap.length,
      "unacceptable avatar index"
    );
  ...
  public getRandomAvatar() {
    return this.imageLoader.getCharacterImage(
      this.avatarIndexMap[
        Math.floor(Math.random() * this.avatarIndexMap.length)
      ]
    );
  }
