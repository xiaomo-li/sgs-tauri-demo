...
export class WaitingRoomSender {
  ...
  broadcast<I extends WaitingRoomEvent>(
    identifier: I,
    event: WaitingRoomClientEventFinder<I>
  ) {
  ...
  giveHostTo(from: PlayerId, to: PlayerId) {
    this.socket.emit(WaitingRoomEvent.ChangeHost, {
      prevHostPlayerId: from,
      newHostPlayerId: to,
    });
  }
  ...
  requestGameStart(
    roomInfo: WaitingRoomClientEventFinder<WaitingRoomEvent.GameStart>["roomInfo"]
  ) {
    this.socket.emit(WaitingRoomEvent.GameStart, { roomInfo });
  }

  kickPlayerOrCloseSeat(
    seatId: number,
    close: boolean,
