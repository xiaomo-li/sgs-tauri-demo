...
import { AudioService } from "../audio/install";
    ...
    this.props.skinData
      ...
      ?.infos.find((skinInfo) =>
        skinInfo.images?.forEach((imageInfo) =>
          this.skinNameList.push(imageInfo.name)
        )
      );
...
export class CharacterSpec extends React.Component<CharacterSpecProps> {
  ...
  @mobx.action
  getSkillName() {
    this.skills = this.props.character.Skills.filter(
  ...
  onPlaySkillAudio = (skillName: string, skinName: string) => () => {
    ...
    if (
      this.props.skinName &&
      this.props.skinName !== this.props.character.Name
    ) {
  ...
  onPlayDeathAudio = (characterName: string, skinName?: string) => () => {
    if (this.props.skinName && this.props.skinName !== characterName) {
      this.props.audioService.playDeathAudio(
        characterName,
        true,
        this.props.skinData,
        this.props.skinName
    ...
    return (
      <div className={styles.characterSpec}>
        <span
          ...
          onClick={this.onPlayDeathAudio(this.props.character.Name)}
        >
            ...
            <div className={styles.skill} key={skill.Name}>
              ...
              <span
                ...
                dangerouslySetInnerHTML={{
                  __html: this.props.translator.tr(skill.Description),
                }}
              />
            </div>
          ))}
        {relatedSkills.length > 0 && (
          <span className={styles.relatedSkillTiltle}>
            {this.props.translator.trx("related skill")}
          </span>
        )}
        {relatedSkills.length > 0 &&
