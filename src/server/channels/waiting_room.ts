...
export class WaitingRoomSocket {
  ...
  private broadcast<Event extends WaitingRoomEvent>(
    e: Event,
    content: WaitingRoomServerEventFinder<Event>
  ) {
  ...
  private getAvailabeSeatId() {
    for (let i = 0; i < this.defaultNumberOfPlayers; i++) {
      if (
        !this.waitingRoomInfo.closedSeats.includes(i) &&
        !this.waitingRoomInfo.players.find((p) => p.seatId === i)
      ) {
        return i;
      }
    }
  ...
  private readonly onGameStart =
    ...
    (evt: WaitingRoomClientEventFinder<WaitingRoomEvent.GameStart>) => {
      ...
      this.broadcast(WaitingRoomEvent.GameStart, {
        roomId,
        otherPlayersId: this.waitingRoomInfo.players.map(
  ...
  private readonly onSendMessage =
    ...
    (evt: WaitingRoomClientEventFinder<WaitingRoomEvent.PlayerChatMessage>) => {
      this.broadcast(WaitingRoomEvent.PlayerChatMessage, {
        ...evt,
        timestamp: Date.now(),
      });
    };
    ...
    (evt: WaitingRoomClientEventFinder<WaitingRoomEvent.PlayerEnter>) => {
      ...
      if (
        ...
        this.broadcast(WaitingRoomEvent.PlayerEnter, {
          ...
          playerInfo,
          otherPlayersInfo: this.waitingRoomInfo.players.filter(
  ...
  private readonly onPlayerLeave =
    ...
    (evt: WaitingRoomClientEventFinder<WaitingRoomEvent.PlayerLeave>) => {
      this.waitingRoomInfo.players = this.waitingRoomInfo.players.filter(
        (p) => p.playerId !== evt.leftPlayerId
      );

      let newHostPlayerId: PlayerId | undefined;
      if (
        evt.leftPlayerId === this.waitingRoomInfo.hostPlayerId &&
        this.waitingRoomInfo.players.length > 0
      ) {
  ...
  private readonly onPlayerReady =
    ...
    (evt: WaitingRoomClientEventFinder<WaitingRoomEvent.PlayerReady>) => {
      const player = this.waitingRoomInfo.players.find(
        (p) => p.playerId === evt.readyPlayerId
      );
      if (player) {
  ...
  private readonly onSeatDisabled =
    ...
    (evt: WaitingRoomClientEventFinder<WaitingRoomEvent.SeatDisabled>) => {
      if (evt.disabled) {
        ...
        this.waitingRoomInfo.closedSeats =
          this.waitingRoomInfo.closedSeats.filter((s) => s !== evt.seatId);
        this.waitingRoomInfo.roomInfo.numberOfPlayers++;
      }

      if (evt.kickedPlayerId) {
        this.broadcast(WaitingRoomEvent.PlayerLeave, {
          leftPlayerId: evt.kickedPlayerId,
          byKicked: true,
        });
        this.waitingRoomInfo.players = this.waitingRoomInfo.players.filter(
          (p) => p.playerId !== evt.kickedPlayerId
        );
      }
  ...
  public readonly reassigHost = (
    prevHostPlayerId: PlayerId,
    newHostPlayerId: PlayerId
  ) => {
    this.broadcast(WaitingRoomEvent.ChangeHost, {
      prevHostPlayerId,
      newHostPlayerId,
    });
  };
