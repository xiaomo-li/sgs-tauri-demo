...
export class LobbyEventChannel {
  ...
  constructor(
    ...
    this.registerEventHandler(
      ...
      this.onQueryRoomsInfo
    );
    this.registerEventHandler(
      LobbySocketEvent.CreateWaitingRoom,
      this.onCreatingWaitingRoom
    );
    this.registerEventHandler(
      LobbySocketEvent.QueryVersion,
      this.matchCoreVersion
    );
    this.registerEventHandler(
      LobbySocketEvent.CheckRoomExist,
      this.onCheckingRoomExist
    );
  }
  ...
  private readonly installEventHandlers = (socket: SocketIO.Socket) => {
    for (const [eventEnum, handler] of Object.entries<
      (socket: SocketIO.Socket) => (...args: any) => void
    >(this.eventHandlers)) {
  ...
  private readonly matchCoreVersion =
    (socket: SocketIO.Socket) => (content: { version: string }) => {
      socket.emit(
  ...
  private readonly onCheckingRoomExist =
    (socket: SocketIO.Socket) => (id: RoomId) => {
      socket.emit(
        LobbySocketEvent.CheckRoomExist.toString(),
        this.roomService.checkRoomExist(id)
      );
    };
  ...
  private readonly onCreatingWaitingRoom =
    ...
    (content: {
      ...
      if (
        !this.roomService.isValidToCreateWaitingRoom(socket.handshake.address)
      ) {
        socket.emit(LobbySocketEvent.CreateWaitingRoom.toString(), {
  ...
  private readonly onGameCreated =
    (socket: SocketIO.Socket) => (content: GameInfo) => {
