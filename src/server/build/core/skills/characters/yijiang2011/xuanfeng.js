...
let XuanFeng = class XuanFeng extends skill_1.TriggerSkill {
    ...
    async onEffect(room, event) {
        ...
        if (room.CurrentPlayer === room.getPlayerById(fromId) && xuanfengTargets.length > 0) {
            const askForPlayerChoose = {
                ...
                requiredAmount: 1,
                conversation: translation_json_tool_1.TranslationPack.translationJsonPatcher('{0}: do you want to choose a XuanFeng target to deal 1 damage?', this.Name).extract(),
                triggeredBySkills: [this.Name],
            };
...
XuanFeng = tslib_1.__decorate([
    skill_1.CommonSkill({ name: 'xuanfeng', description: 'xuanfeng_description' })
], XuanFeng);
exports.XuanFeng = XuanFeng;
