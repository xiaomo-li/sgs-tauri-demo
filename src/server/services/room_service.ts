...
export class RoomService {
  ...
  constructor(
    ...
    private createGameServerSocket: (
      roomChannel: SocketIO.Namespace,
      roomId: RoomId
    ) => ServerSocket,
    ...
    private createGameWaitingRoom: (
      info: TemporaryRoomCreationInfo,
      roomId: RoomId
    ) => WaitingRoomInfo,
    private createWaitingRoomSocket: (
      socket: SocketIO.Namespace,
      roomInfo: WaitingRoomInfo
    ) => WaitingRoomSocket,
    private createDifferentModeGameProcessor: (
      gameMode: GameMode
    ) => GameProcessor
  ) {}
  ...
  checkRoomExist(roomId: RoomId) {
    return (
      ...
      this.waitingRooms.find(
        (room) => room.roomId === roomId && !room.isPlaying
      ) !== undefined
    );
  }
  ...
  getRoomsInfo(): readonly RoomInfo[] {
    return [
      ...
      ...this.waitingRooms
        ...
        .map((room) => this.getWaitingRoomInfo(room)),
    ];
  }
  ...
  createRoom(
    ...
    const roomSocket = this.createGameServerSocket(
    ...
    room.onClosed(async () => {
      ...
      const waitingRoomIndex = this.waitingRooms.findIndex(
        (r) => r.roomId === room.WaitingRoomInfo.roomId
      );
      ...
      if (waitingRoom?.players.length === 0) {
        ...
        waitingRoom &&
        waitingRoom.players.find(
          (p) => p.playerId === waitingRoom.hostPlayerId
        ) == null
      ) {
        this.waitingRoomMaps
          ...
          ?.reassigHost(
            ...
            waitingRoom.players[0].playerId
          );
      }
    });
  ...
  createWaitingRoom(
    ...
    const room = this.createGameWaitingRoom(
      roomInfo,
      roomInfo.roomId || Date.now()
    );
    const roomSocket = this.createWaitingRoomSocket(
  ...
  isValidToCreateWaitingRoom(playerIp: string): boolean {
    return (
      this.hostPlayerIps.get(playerIp) === undefined ||
      this.hostPlayerIps.get(playerIp)! < 2
    );
  }
}
